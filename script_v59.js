...
function renderListaLotesPicker() {
  ...
  cont.innerHTML = datos.map(l => {
    ...
    // "Bloqueado" (de Orange) NO impide seleccionar — puede ser un flag administrativo interno
    // de Orange que no significa que el lote no se pueda trabajar. Solo Reservado/Vendido/No
    // disponible (cargados en NUESTRO sistema) bloquean la selección de verdad.
...
function renderResumenCierres(mes, metaC, anio) {
  try {
    const cierres = (todosRegs||[]).filter(r => r.conclusion === 'Reserva' && r.precioVenta > 0 && r.mes === mes
      && (!anio || !r.ts || new Date(r.ts).getFullYear() === anio));
...
function renderDetalleCierres() {
  ...
  if (!tbody) return;
  // Usa el mismo filtro global de Año/Mes de arriba del panel — no uno propio separado.
  let cierres = todosRegs.filter(r => r.conclusion === 'Reserva' && r.precioVenta > 0
    && r.mes === ventasFilMes && (!r.ts || new Date(r.ts).getFullYear() === ventasFilAnio));

  cierres = cierres.sort((a,b)=>(b.ts||0)-(a.ts||0));

  if (!cierres.length) {
    tbody.innerHTML = '<tr><td colspan="13"><div class="empty-state">Sin cierres para ' + ventasFilMes + ' ' + ventasFilAnio + '.</div></td></tr>';
...
// ── Filtro global del panel de Ventas — Año + Mes. Todo lo que se ve "del mes"
// en este panel usa ESTO, no la fecha de hoy, así se puede mirar cualquier mes pasado.
...
function renderDashboard() {
  ...
  const vMes = soloInternos(todosRegs.filter(r => r.mes === mes && (!r.ts || new Date(r.ts).getFullYear() === anioMeta)));
  ...
  setText('v-kpi-mes-sub', 'registros en ' + mes + ' ' + anioMeta);
  ...
  window._fbListenMetas(anioMeta, mes, metas => {
    ...
    renderResumenCierres(mes, metaC, anioMeta);
  });
  ...
  renderResumenCierres(mes, 0, anioMeta); // pinta los conteos ya mismo; la meta se actualiza abajo cuando llegue de Firebase
