...
function renderDetalleCierres() {
  ...
  tbody.innerHTML = filasCalc.map(({r, metraje, m2Lista, m2Cierre}) => `<tr${r.esExterno?' style="background:#fdf6e8;"':''}>
      <td class="wrap-ok" style="font-weight:600;max-width:150px;overflow:hidden;text-overflow:ellipsis;" title="${esc(r.nombre||'')}">${esc(r.nombre||'—')} ${r.esExterno?'<span class="badge badge-def" style="background:#fef3c7;color:#92400e;">🤝 Ext.</span>':''}</td>
      ...
      <td>${metraje ? Number(metraje).toLocaleString('es-BO')+' m²' : '<span style="color:var(--gris);">falta</span>'}</td>
