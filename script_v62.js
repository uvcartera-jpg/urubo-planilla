...
function renderCobranzaCarteraTendencia() {
  ...
  _chartCobranzaCarteraTendencia = new Chart(canvas, {
    ...
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{display:false}, tooltip:{callbacks:{label:ctx=>'$'+ctx.parsed.y.toLocaleString('es-BO')}} },
...
function renderCobranzaPorTerrenoAccion() {
  ...
  setText('orange-cart-kpi-terreno-mes', '$'+Math.round(terreno).toLocaleString('es-BO'));
  setText('orange-cart-kpi-accion-mes', '$'+Math.round(accion).toLocaleString('es-BO'));
  ...
  if (canvas) {
    ...
    _chartCarteraTerrenoAccion = new Chart(canvas, {
      ...
      options: { responsive:true, maintainAspectRatio:false, cutout:'65%', plugins:{ legend:{display:false}, tooltip:{callbacks:{label:ctx=>ctx.label+': $'+ctx.parsed.toLocaleString('es-BO')}} } }
    });
  }
}
...
function renderCobranzaPorModalidad() {
  ...
  cont.innerHTML = entries.map(([k,v]) => `
    <div style="margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;font-size:var(--fs-sm);margin-bottom:3px;">
        <span style="font-weight:600;">${esc(k)}</span>
        <span style="color:var(--cartera);font-weight:700;">$${Math.round(v).toLocaleString('es-BO')}</span>
      </div>
...
function renderIngresosVentaNuevaVsCartera() {
  ...
  setText('orange-cart-kpi-venta-nueva', '$'+Math.round(ventaNueva).toLocaleString('es-BO'));
  setText('orange-cart-kpi-cartera-vieja', '$'+Math.round(cartera).toLocaleString('es-BO'));
...
function renderSemanalCartera() {
  ...
  if (canvas) {
    ...
    _chartSemanalCartera = new Chart(canvas, {
      ...
      options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{position:'top'} }, scales:{ y:{beginAtZero:true, ticks:{callback:v=>'$'+(v/1000)+'K'}} } }
    });
  }
}
