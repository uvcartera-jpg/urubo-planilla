...
function renderDashboard() {
  ...
  // Funnel de conversión (mes actual): Leads y Agendadas se cargan a mano,
  // Concretadas y Cierres se calculan solos con los registros del sistema.
  ...
  if (funnelEl) {
    window._fbListenKpiManualVentas(anioActual(), mes, kpiM => {
      kpiManualActual = kpiM;
      const leads = kpiM.leads || 0;
      const agendadas = kpiM.visitasAgendadas || 0;
      const convAgVsConc = agendadas ? Math.round(totalVisitas/agendadas*100) : undefined;
      const convConcVsCierre = totalVisitas ? Math.round(conCierre/totalVisitas*100) : undefined;
      const funnelData = [
        { label:'Leads', n:leads, color:'var(--accent)' },
        { label:'Visitas agendadas', n:agendadas, color:'var(--info)' },
