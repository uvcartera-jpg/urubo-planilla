...
async function guardarVisitaProgramada() {
  ...
  mostrarFichaVisita({ nombre, telefono, asesorNombre: asesorSel?.nombre||'', fecha, hora, origen, campaniaCodigo });
  ...
  cont.innerHTML = mias.map(v => `
    <div class="cobro-card">
      ...
      <div class="cobro-monto">
        <button class="btn-del" onclick="window._fbRemoveVisitaProgramada('${v._key}')">🗑</button>
      </div>
    </div>`).join('');
}
...
let _chartTendenciaMarketing = null;

function renderTendenciaMarketing() {
  const canvas = document.getElementById('mkt-chart-tendencia');
  if (!canvas) return;
  const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const hoy = new Date();
  ...
  if (_chartTendenciaMarketing) _chartTendenciaMarketing.destroy();
  _chartTendenciaMarketing = new Chart(canvas, {
    type: 'line',
    data: {
  ...
  if (cont) {
    cont.innerHTML = campaniasData.length ? campaniasData.map(c => `
      <div class="vendor-item">
        ...
        <button class="btn-remove-vendor" onclick="window._fbRemoveCampania('${c._key}')">×</button>
...
function renderLeadsSemanales() {
  ...
  tbody.innerHTML = leadsSemanalesData.map(l => `<tr>
    ...
    <td><button class="btn-del" onclick="window._fbRemoveLeadsSemanal('${l._key}')">🗑</button></td>
  </tr>`).join('');
}
