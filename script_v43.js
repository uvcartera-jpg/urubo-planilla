...
const TAB_TITLES = {
  ...
  'asesores':'Usuarios', 'config':'Config'
};
...
const NAV_GRUPOS = {
  ...
  eliminaciones:'sistema', asesores:'sistema', config:'sistema',
};
      ...
      datasets: [
        {
          ...
          pointHitRadius: 12,
          pointHoverRadius: 5,
          pointHoverBackgroundColor: '#7fb8e8',
          pointHoverBorderColor: '#0a0f0c',
    ...
    options: {
      ...
      plugins: {
        legend: {
          ...
          labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, font: { family: "'DM Sans',sans-serif", size: 12 }, color: '#c4d1c0' }
        },
        tooltip: {
          backgroundColor: '#1a251c', titleColor: '#eef2ec', bodyColor: '#eef2ec',
      ...
      scales: {
        x: { grid: { display: false }, ticks: { font: { family: "'DM Sans',sans-serif", size: 11 }, color: '#93a390' } },
        y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,.06)' }, ticks: { font: { family: "'DM Sans',sans-serif", size: 11 }, color: '#93a390', precision: 0 } }
      }
    }
  });
}
...
function renderTendenciaMarketing(canvasId) {
  ...
  _chartsTendenciaMarketing[canvasId] = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        { label:'Leads', data:datosPorBucket.map(b=>b.leads), borderColor:'#c99ee8', backgroundColor:'transparent', tension:.35, pointRadius:3 },
        { label:'Visitas agendadas', data:datosPorBucket.map(b=>b.agendadas), borderColor:'#7fb8e8', backgroundColor:'transparent', tension:.35, pointRadius:3 },
        { label:'Visitas concretadas', data:datosPorBucket.map(b=>b.concretadas), borderColor:'#f0b862', backgroundColor:'transparent', tension:.35, pointRadius:3 },
        { label:'Cierres', data:datosPorBucket.map(b=>b.cierres), borderColor:'#6ee7b7', backgroundColor:'rgba(110,231,183,.1)', fill:true, tension:.35, pointRadius:3 },
      ]
    },
    options: {
      ...
      plugins:{
        legend:{ display:true, position:'bottom', labels:{usePointStyle:true, color:'#c4d1c0'} },
        datalabels: {
      ...
      scales:{
        x:{ ticks:{color:'#93a390'}, grid:{color:'rgba(255,255,255,.06)'} },
        y:{ beginAtZero:true, ticks:{ precision:0, color:'#93a390' }, grid:{color:'rgba(255,255,255,.06)'} }
      }
    }
  });
}
...
function renderVentasConcretadasAdmin() {
  ...
  // KPIs compactos (arriba, junto a pendientes)
  setText('vc-kpi-total', aprobadas.length);
  const totalVendido = aprobadas.reduce((s,v)=>s+(v.precioVenta||0),0);
  setText('vc-kpi-vendido', totalVendido ? '$'+Math.round(totalVendido).toLocaleString('es-BO') : '—');
  const descPcts = aprobadas.filter(v=>v.descuentoPct).map(v=>v.descuentoPct);
  setText('vc-kpi-desc', descPcts.length ? (descPcts.reduce((s,v)=>s+v,0)/descPcts.length).toFixed(1)+'%' : '—');
  const m2Vals = aprobadas.filter(v=>v.m2Usd).map(v=>v.m2Usd);
  setText('vc-kpi-m2', m2Vals.length ? '$'+Math.round(m2Vals.reduce((s,v)=>s+v,0)/m2Vals.length) : '—');

  // Poblar filtros multi-selección
  ...
  renderVentasDashboard(aprobadas);
}
...
const VCD_DARK = { grid:'rgba(255,255,255,.06)', text:'#9db396', textStrong:'#e4ece1' };
const VCD_PALETTE = ['#6ee7b7','#7fb8e8','#f0b862','#c99ee8','#f08a7e','#8fd98a','#6b9dd6','#e0a458'];
...
function vcdAbrirSeccion(nombre) {
  ...
  setTimeout(() => {
    const aprobadas = (typeof ventasConcretadasData !== 'undefined') ? ventasConcretadasData.filter(v => v.estadoAutorizacion === 'aprobado') : [];
    if (aprobadas.length) renderVentasDashboard(aprobadas);
  }, 50);
}
...
function vcdCambiarPeriodo(periodo) {
  ...
  renderVentasDashboard(ventasConcretadasData.filter(v => v.estadoAutorizacion === 'aprobado'));
}
...
function renderVentasDashboard(aprobadas) {
  ...
  if (c3 && asesorPorVentas.length) {
    _chartsVCD['vcd-chart-asesor-ventas'] = new Chart(c3, {
      ...
      options: darkOpts({ indexAxis:'y', plugins:{ legend:{display:false}, datalabels:{anchor:'center',align:'center',color:'#0a0f0c',font:{weight:'700',size:11}} } })
    });
  }
  ...
  if (c4 && asesorPorExced.length) {
    _chartsVCD['vcd-chart-asesor-exced'] = new Chart(c4, {
      type:'bar',
      data:{ labels:asesorPorExced.map(([k])=>k), datasets:[{ data:asesorPorExced.map(([,d])=>Math.round(d.exced)), backgroundColor:asesorPorExced.map(([,d])=>d.exced>=0?VCD_PALETTE[0]:VCD_PALETTE[4]), borderRadius:5, maxBarThickness:22 }] },
  ...
  const donutOpts = { cutout:'62%', responsive:true, maintainAspectRatio:false,
    plugins:{ legend:{position:'bottom', labels:{color:VCD_DARK.text, font:{size:9.5}, boxWidth:8, padding:8}},
      datalabels:{color:'#0a0f0c', font:{weight:'700',size:10}, formatter:v=>v>0?v:''} } };
  function pintarDonut(id, counts) {
    ...
    _chartsVCD[id] = new Chart(el, { type:'doughnut',
      data:{ labels:entries.map(([k])=>k), datasets:[{ data:entries.map(([,n])=>n), backgroundColor:VCD_PALETTE, borderWidth:2, borderColor:'#0d1410' }] },
      options: donutOpts });
  }
  ...
  if (c7 && convAsesor.length) {
    _chartsVCD['vcd-chart-conversion-asesor'] = new Chart(c7, { type:'bar',
      data:{ labels:convAsesor.map(([k])=>k), datasets:[{ data:convAsesor.map(([,p])=>p), backgroundColor:convAsesor.map(([,p])=>p>=70?VCD_PALETTE[0]:p>=40?VCD_PALETTE[2]:VCD_PALETTE[4]), borderRadius:5, maxBarThickness:22 }] },
  ...
  if (c8 && puntos.length) {
    _chartsVCD['vcd-chart-scatter'] = new Chart(c8, { type:'scatter',
      data:{ datasets:[{ label:'Ventas', data:puntos, backgroundColor:'rgba(110,231,183,.65)', pointRadius:4, pointHoverRadius:6 }] },
...
function renderDetalleCierres() {
  ...
  if (!cierres.length) {
    tbody.innerHTML = '<tr><td colspan="12"><div class="empty-state">Sin cierres todavía.</div></td></tr>';
  ...
  tbody.innerHTML = filasCalc.map(({r, metraje, m2Lista, m2Cierre}) => `<tr${r.esExterno?' style="background:#fdf6e8;"':''}>
...
function renderHistoricoMensualVentas() {
  ...
  _chartHistPrecioDesc = new Chart(c1, {
    ...
    data: {
      labels,
      datasets: [
        { label:'% Descuento', data:descPromedios, borderColor:'#7fb8e8', backgroundColor:'transparent', yAxisID:'y', tension:.3, pointRadius:3 },
        { label:'Precio de Venta', data:precioPromedios, borderColor:'#6ee7b7', backgroundColor:'transparent', yAxisID:'y1', tension:.3, pointRadius:3 },
      ]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      interaction:{ intersect:false, mode:'index' },
      plugins:{
        legend:{ position:'top', labels:{usePointStyle:true, color:'#c4d1c0'} },
        datalabels: {
      ...
      scales:{
        y:  { position:'left', beginAtZero:true, grid:{color:'rgba(255,255,255,.06)'}, ticks:{ maxTicksLimit:6, color:'#93a390', callback:v=>Math.round(v*10)/10+'%' }, title:{display:true,text:'% Descuento',color:'#93a390'} },
        y1: { position:'right', beginAtZero:true, grid:{drawOnChartArea:false}, ticks:{ maxTicksLimit:6, color:'#93a390', callback:v=>'$'+Math.round(v).toLocaleString('es-BO') }, title:{display:true,text:'Precio de Venta',color:'#93a390'} },
        x:  { ticks:{color:'#93a390'}, grid:{color:'rgba(255,255,255,.06)'} },
      }
    }
  });
  ...
  _chartHistM2Cant = new Chart(c2, {
    ...
    data: {
      labels,
      datasets: [
        { label:'$/m²', data:m2Promedios, borderColor:'#7fb8e8', backgroundColor:'transparent', yAxisID:'y', tension:.3, pointRadius:3 },
        { label:'Cantidad de ventas', data:cantidades, borderColor:'#6ee7b7', backgroundColor:'transparent', yAxisID:'y1', tension:.3, pointRadius:3 },
      ]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      ...
      scales:{
        y:  { position:'left', beginAtZero:true, grid:{color:'rgba(255,255,255,.06)'}, ticks:{ maxTicksLimit:6, color:'#93a390', callback:v=>'$'+Math.round(v) }, title:{display:true,text:'$/m²',color:'#93a390'} },
        y1: { position:'right', beginAtZero:true, grid:{drawOnChartArea:false}, ticks:{ maxTicksLimit:6, color:'#93a390', precision:0, stepSize:1 }, title:{display:true,text:'Cantidad',color:'#93a390'} },
        x:  { ticks:{color:'#93a390'}, grid:{color:'rgba(255,255,255,.06)'} },
      }
    }
  });
  ...
  _chartsAuxiliares[canvasId] = new Chart(canvas, {
    ...
    options: {
      ...
      scales:{
        ...
        y:{ grid:{display:false}, ticks:{ font:{ family:"'DM Sans',sans-serif", size:11.5, weight:'600' }, color:'#c4d1c0' } }
      },
...
function _tortaEstados(canvasId, counts, colorMap) {
  ...
  _chartsAuxiliares[canvasId] = new Chart(canvas, {
    type: 'doughnut',
    data: { labels, datasets:[{ data, backgroundColor:colores, borderWidth:2, borderColor:'#141d16' }] },
    options: {
      cutout:'60%', responsive:true, maintainAspectRatio:false,
      plugins:{
        legend:{ position:'right', labels:{ usePointStyle:true, boxWidth:8, font:{size:11}, color:'#c4d1c0' } },
        datalabels:{ color:'#0a0f0c', font:{weight:'700', size:11}, formatter:(v)=> v>0?v:'' }
      }
    }
  });
}
...
function renderHomeChartVentasPie(vMes) {
  ...
  _chartsHome['h-chart-ventas-pie'] = new Chart(canvas, {
    ...
    options: {
      cutout:'62%',
      plugins:{
        legend:{ position:'bottom', labels:{ usePointStyle:true, boxWidth:8, font:{size:11}, color:'#c4d1c0' } },
        datalabels: { font:{ family:"'DM Sans',sans-serif", weight:'700', size:12 }, color:'#0a0f0c', formatter:(v)=> v>0?v:'' }
      },
      maintainAspectRatio:false
    }
  });
...
function renderHomeChartVentasTendencia() {
  ...
  _chartsHome['h-chart-ventas-linea'] = new Chart(canvas, {
    ...
    options: {
      ...
      maintainAspectRatio:false, scales:{ x:{ ticks:{color:'#93a390'}, grid:{color:'rgba(255,255,255,.06)'} }, y:{ beginAtZero:true, ticks:{precision:0, color:'#93a390'}, grid:{color:'rgba(255,255,255,.06)'} } }
    }
  });
}
...
async function renderTendenciaExpensas() {
  ...
  _chartTendenciaExpensas = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        { label:'Cobranza', data: ordenados.map(b=>b.monto), borderColor:'#6ee7b7', backgroundColor:'rgba(110,231,183,.12)',
          fill:true, tension:.35, pointRadius:3, pointBackgroundColor:'#6ee7b7' },
        { label:'Meta', data: metaPorBucket, borderColor:'#f0b862', borderDash:[6,4], backgroundColor:'transparent',
          fill:false, tension:.2, pointRadius:2, pointBackgroundColor:'#f0b862' }
      ]
    },
    options: {
      responsive: true,
      ...
      scales: {
        y: { ticks: { color:'#93a390', callback: v => 'Bs '+Number(v).toLocaleString('es-BO',{maximumFractionDigits:0}) }, grid:{color:'rgba(255,255,255,.06)'} },
        x: { ticks: { color:'#93a390', autoSkip:true, maxRotation:0 }, grid:{color:'rgba(255,255,255,.06)'} }
      }
    }
  });
}
