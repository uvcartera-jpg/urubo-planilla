...
function abrirDetalleAsesor(nombre) {
  ...
  setText('da-pct', pct+'%');
  setText('da-vendido', vendido ? '$'+vendido.toLocaleString('es-BO') : '—');
...
function renderAvancePorAsesor() {
  ...
  tbody.innerHTML = filas.length ? filas.map(([nombre,d]) => {
    ...
    return `<tr style="cursor:pointer;" onclick="abrirDetalleAsesor('${escJs(nombre)}')" title="Ver detalle">
      ...
      <td>${d.vendido ? '$'+d.vendido.toLocaleString('es-BO') : '—'}</td>
    </tr>`;
...
function renderDashboard() {
  ...
  setText('v-kpi-total-sub', asesores.length + ' asesor' + (asesores.length!==1?'es':''));
...
function renderHomeChartVentasPie(vMes) {
  ...
  _chartsHome['h-chart-ventas-pie'] = new Chart(canvas, {
    ...
    options: { cutout:'62%', plugins:{ legend:{ position:'bottom', labels:{ usePointStyle:true, boxWidth:8, font:{size:11} } } }, maintainAspectRatio:false }
  });
}
