...
/* ═══════════════════════════════════════════
   SELECTOR DE LOTE (inventario con estado de negociación)
═══════════════════════════════════════════ */
...
function renderListaLotesPicker() {
  ...
  cont.innerHTML = datos.map(l => {
    ...
    return `<div class="lote-picker-row ${bloqueado?'bloqueado':''}" onclick="${bloqueado?'':`seleccionarLotePicker('${l._key}')`}">
      ...
      <span style="font-size:var(--fs-sm);">${infoExtra}</span>
    </div>`;
  }).join('');
}
...
function pintarEstadoLoteSeleccionado(l) {
  ...
  if (l.estado === 'Reservado' || l.estado === 'Vendido') {
    info.innerHTML = `<span style="color:var(--danger);font-weight:600;">Este lote ya no está disponible (${l.estado}).</span>`;
    acciones.innerHTML = '';
    return;
  }
  if (esMiNegociacion) {
    info.innerHTML = `<span style="color:var(--info);">📌 Ya lo tenés marcado en negociación.</span>`;
    ...
    info.innerHTML = `<span style="color:var(--warn);font-weight:600;">⚠️ En negociación por ${esc(l.negociadoPorNombre||'otro asesor')} — igual podés cargar tu visita, pero avisale al cliente que puede haber otro interesado.</span>`;
    ...
    info.innerHTML = `<span style="color:var(--ok);">Disponible.</span>`;
...
function renderDashboard() {
  ...
  // Funnel de conversión (mes actual) — nuevo modelo: cada registro YA es una visita realizada
  const totalVisitas  = vMes.length;
  const interesados   = vMes.filter(r => r.interes === 'SI').length;
  const conCierre     = vMes.filter(r => r.conclusion === 'Reserva' || r.huboCierre === 'SI').length;
  const conv1 = totalVisitas ? Math.round(interesados/totalVisitas*100) : 0;
  const conv2 = interesados ? Math.round(conCierre/interesados*100) : 0;

  const funnelEl = document.getElementById('chart-funnel');
  if (funnelEl) {
    const funnelData = [
      { label:'Visitas totales', n:totalVisitas, color:'var(--info)' },
      { label:'Interesados', n:interesados, color:'var(--warn)', pct:conv1 },
      { label:'Reservas', n:conCierre, color:'var(--cartera)', pct:conv2 },
    ];
...
function switchUserTab(id, scroll = true) {
  ['ventas','expensas','marketing','fichero'].forEach(t => {
...
function entrarUsuario() {
  ...
  if (modulos.includes('ventas')) {
    ...
    window._fbListenLotes(arr => { todosLotes = arr; renderListaLotesPicker(); });
