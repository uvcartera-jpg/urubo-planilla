...
function renderCategorizacionExpensas() {
  ...
  if (!pendientes.length) { tbody.innerHTML = '<tr><td colspan="5"><div class="empty-state">✅ Todos los cobros cargados están categorizados.</div></td></tr>'; return; }
  const opciones = '<option value="">Elegir categoría...</option>' + EXPENSAS_CATEGORIAS.map(c=>`<option value="${c}">${c}</option>`).join('');
  tbody.innerHTML = pendientes.slice(0,200).map(r => `<tr>
    ...
    <td><select onchange="asignarCategoriaExpensa('${r.key}', this.value)" style="padding:6px 10px;border:1.5px solid var(--line);border-radius:var(--r-sm);font-size:var(--fs-sm);font-family:'DM Sans',sans-serif;">${opciones}</select></td>
  </tr>`).join('') + (pendientes.length>200 ? `<tr><td colspan="5" style="text-align:center;color:var(--gris);padding:10px;">Mostrando 200 de ${pendientes.length}.</td></tr>` : '');
}
