...
function renderDetalleCierres() {
  ...
  tbody.innerHTML = filasCalc.map(({r, metraje, m2Lista, m2Cierre}) => `<tr>
...
function renderDashboard() {
  ...
  // KPIs generales
  countUp(document.getElementById('v-kpi-total'), todosRegs.length);
...
function renderHomeChartVentasTendencia() {
  ...
  const datos = buckets.map(b => todosRegs.filter(r=>r.mes===b.mes).length);
