...
async function mostrarCotizacionPublica(key) {
  ...
  document.body.innerHTML = `<div id="cotiz-publica-root" style="min-height:100vh;background:#F7F5F1;display:flex;align-items:center;justify-content:center;font-family:'Inter',sans-serif;">
    <div style="text-align:center;color:#1F3A2E;">
      <div style="font-size:14px;letter-spacing:.1em;text-transform:uppercase;">Cargando tu cotización...</div>
    </div>
  </div>`;
  ...
  if (!c) {
    root.innerHTML = _cotizPublicaErrorHtml('Esta cotización no existe o fue eliminada.');
    return;
  }
  ...
  const highlights = [
    ['01','Desarrollo emblemático','Urubó Village es el proyecto urbanístico que ha impulsado el crecimiento y valorización de toda la zona del Urubó.'],
    ['02','Infraestructura de primer nivel','Calles pavimentadas, servicios subterráneos, amplias áreas verdes y paisajismo profesional.'],
    ['03','Conectividad estratégica','Acceso directo a Santa Cruz a través del Puente Arq. Cristóbal Roda, obra financiada con inversión privada.'],
    ['04','Vida de club','El Country Club Urubó Village ofrece piscinas, canchas de tenis y fútbol, gimnasio y espacios sociales para toda la familia.'],
    ['05','Alta plusvalía','El Urubó mantiene una valorización constante, siendo una de las zonas más demandadas para vivir e invertir.'],
  ];
  ...
  root.innerHTML = `
    <style>
      ...
      #cotiz-publica-root{ font-family:'Inter',sans-serif; background:#F7F5F1; color:#111; }
      .cp-serif{ font-family:'Playfair Display',serif; }
      .cp-wrap{ max-width:900px; margin:0 auto; padding:0 24px; }
      .cp-hero{ position:relative; height:70vh; min-height:420px; background:url('assets/entrada.jpg') center/cover; display:flex; align-items:flex-end; }
      .cp-hero::before{ content:''; position:absolute; inset:0; background:linear-gradient(180deg,rgba(0,0,0,.15),rgba(0,0,0,.65)); }
      .cp-hero-content{ position:relative; z-index:1; color:#fff; padding:48px 24px; width:100%; max-width:900px; margin:0 auto; }
      .cp-wordmark{ font-size:13px; letter-spacing:.25em; text-transform:uppercase; opacity:.9; margin-bottom:16px; }
      .cp-hero h1{ font-size:clamp(28px,5vw,46px); font-weight:600; margin:0 0 10px; line-height:1.15; }
      .cp-hero p{ font-size:16px; opacity:.9; max-width:480px; }
      .cp-section{ padding:56px 0; border-bottom:1px solid rgba(0,0,0,.08); }
      .cp-label{ font-size:11px; letter-spacing:.15em; text-transform:uppercase; color:#1F3A2E; font-weight:600; margin-bottom:10px; }
      .cp-card{ background:#fff; border-radius:4px; padding:28px 32px; }
      .cp-row{ display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid rgba(0,0,0,.06); font-size:15px; }
      .cp-row:last-child{ border-bottom:none; }
      .cp-precio-final{ font-size:38px; font-weight:600; }
      .cp-badge{ display:inline-block; background:#1F3A2E; color:#fff; font-size:12px; font-weight:600; padding:4px 12px; border-radius:99px; margin-left:10px; }
      .cp-tachado{ text-decoration:line-through; opacity:.5; font-size:18px; }
      .cp-photo{ width:100%; height:340px; object-fit:cover; border-radius:4px; }
      .cp-grid2{ display:grid; grid-template-columns:1fr 1fr; gap:20px; }
      .cp-hl{ display:flex; gap:18px; margin-bottom:26px; }
      .cp-hl-num{ font-family:'Playfair Display',serif; font-size:26px; color:#1F3A2E; font-weight:600; flex-shrink:0; width:40px; }
      .cp-btn{ background:#1F3A2E; color:#fff; border:none; padding:16px 36px; font-size:15px; font-weight:600; border-radius:4px; cursor:pointer; font-family:'Inter',sans-serif; }
      .cp-btn:hover{ background:#16291f; }
      @media(max-width:640px){ .cp-grid2{ grid-template-columns:1fr; } }
    </style>

    <div class="cp-hero">
      <div class="cp-hero-content">
        <div class="cp-wordmark">URUBÓ VILLAGE</div>
        <h1 class="cp-serif">Tu cotización</h1>
        <p>Preparada especialmente para ${esc(c.cliente)} — gracias por tu interés en ser parte de esta comunidad.</p>
      </div>
    </div>

    <div class="cp-wrap">
      <div class="cp-section">
        <div class="cp-label">Terreno cotizado</div>
        <div class="cp-card">
          <div class="cp-row"><span>Terreno</span><strong>N° ${esc(c.terreno)} — Módulo ${esc(c.modulo)}</strong></div>
          <div class="cp-row"><span>Superficie</span><strong>${(c.superficie||0).toLocaleString('es-BO')} m²</strong></div>
          <div class="cp-row"><span>Categoría</span><strong>${esc(c.categoria||'—')}</strong></div>
          <div class="cp-row"><span>Modalidad</span><strong>${esc(c.modalidad)}</strong></div>
          ${c.conAccion==='SI' ? `<div class="cp-row"><span>Acción del Country Club</span><strong>Incluida — ${fmt(c.accionPrecio)}</strong></div>` : ''}
        </div>
      </div>

      <div class="cp-section">
        <div class="cp-label">Precio</div>
        <div class="cp-card">
          ${hayDescuento ? `<div style="margin-bottom:6px;"><span class="cp-tachado">${fmt(c.precioLista)}</span><span class="cp-badge">-${c.descuentoPct}%</span></div>` : ''}
          <div class="cp-precio-final cp-serif">${fmt(c.precioFinal)}</div>
          <div style="font-size:13px;color:#666;margin-top:4px;">Precio total en dólares americanos</div>
        </div>
      </div>

      <div class="cp-section">
        <div class="cp-label">Plan de pago</div>
        <div class="cp-card">
          <div class="cp-row"><span>Reserva</span><strong>${fmt(c.reserva)}</strong></div>
          <div class="cp-row"><span>Aporte inicial (${c.pctAporte}%)</span><strong>${fmt(c.montoAporte)}</strong></div>
          <div class="cp-row"><span>Saldo a financiar</span><strong>${fmt(c.montoSaldo)}</strong></div>
          <div class="cp-row"><span>Plan de cuotas</span><strong>${c.nroCuotas} cuotas de ${fmt(c.montoCuota)}</strong></div>
        </div>
      </div>

      <div class="cp-section">
        <div class="cp-grid2">
          <img class="cp-photo" src="assets/pileta.jpg" alt=""/>
          <img class="cp-photo" src="assets/jardin.jpg" alt=""/>
        </div>
      </div>

      <div class="cp-section">
        <div class="cp-label">Lo más sobresaliente del proyecto</div>
        ${highlights.map(([n,t,d]) => `<div class="cp-hl"><div class="cp-hl-num">${n}</div><div><strong style="display:block;margin-bottom:4px;">${t}</strong><span style="font-size:14px;color:#555;">${d}</span></div></div>`).join('')}
      </div>

      <div class="cp-section" id="cp-confirmar-cont" style="text-align:center;">
        ${estadoReal === 'confirmado' ? `
          <div class="cp-serif" style="font-size:22px;color:#1F3A2E;">✓ Cotización confirmada</div>
          <p style="color:#555;margin-top:8px;">Gracias — tu asesor ya fue notificado y se va a comunicar para continuar con el proceso.</p>
        ` : `
          <p style="margin-bottom:18px;color:#555;">Válida por ${c.vigenciaDias} días desde que se generó.</p>
          <button class="cp-btn" onclick="confirmarCotizacionPublica('${key}')">Confirmar esta cotización</button>
        `}
      </div>

      <div style="padding:32px 0;text-align:center;font-size:12px;color:#999;">
...
async function confirmarCotizacionPublica(key) {
  await window._fbUpdateCotizacion(key, { estado: 'confirmado', fechaConfirmacion: Date.now() });
  mostrarCotizacionPublica(key); // re-renderiza ya con el estado confirmado
}
