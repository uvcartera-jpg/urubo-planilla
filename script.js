...
function renderPagosSimuladorContado(precioFinal, pctDescuento) {
  ...
  document.getElementById('sim-contado-resumen').innerHTML = `
    ...
    <div style="font-family:'Archivo Black',sans-serif;font-size:30px;color:var(--ok);margin:4px 0;">${f(precioFinal)}${pctDescuento?` <span style="font-size:16px;color:var(--gris);">(${pctDescuento}% dcto.)</span>`:''}</div>
...
async function renderDashboardExpensas() {
  ...
  if (deltaSemana !== null) {
    ...
    elDeltaSem.className = 'text-xs font-semibold mt-1 ' + (deltaSemana>=0?'text-emerald-400':'text-red-400');
  } else { elDeltaSem.textContent = 'sin semana anterior para comparar'; elDeltaSem.className='text-xs font-semibold mt-1 text-slate-500'; }
  ...
  if (deltaMes !== null) {
    ...
    elDeltaMes.className = 'text-xs font-semibold mt-1 ' + (deltaMes>=0?'text-emerald-400':'text-red-400');
  } else { elDeltaMes.textContent = 'sin mes anterior para comparar'; elDeltaMes.className='text-xs font-semibold mt-1 text-slate-500'; }
  ...
  if (metaMesKpi) {
    ...
    elMetaMes.className = 'text-xs font-semibold mt-0.5 ' + (pctVsMeta>=100?'text-emerald-400':pctVsMeta>=70?'text-amber-400':'text-red-400');
  } else {
    elMetaMes.textContent = 'sin meta cargada';
    elMetaMes.className = 'text-xs font-semibold mt-0.5 text-slate-500';
  }
  ...
  if (cv > 30) { elVol.textContent = '🔴 Riesgo Alto — muy irregular'; elVol.className = 'text-xs font-semibold mt-1 text-red-400'; }
  else if (cv > 15) { elVol.textContent = '🟡 Riesgo Moderado'; elVol.className = 'text-xs font-semibold mt-1 text-amber-400'; }
  else { elVol.textContent = '🟢 Cobranza estable'; elVol.className = 'text-xs font-semibold mt-1 text-emerald-400'; }
  ...
  dbeCharts.semanal = new ApexCharts(document.querySelector('#dbe-chart-semanal'), {
    chart: { type:'line', height:260, toolbar:{show:false}, background:'transparent', foreColor:'#B5C4E0' },
    series: [
    ...
    dataLabels: {
      ...
      background: { enabled:true, foreColor:'#1e293b', borderRadius:3, opacity:.85, borderWidth:0 },
      offsetY: -4,
    },
    tooltip: { theme:'dark', y:{formatter: v=>'Bs '+Math.round(v).toLocaleString('es-BO')} },
  });
  ...
  dbeCharts.mensual = new ApexCharts(document.querySelector('#dbe-chart-mensual'), {
    chart: { type:'area', height:260, toolbar:{show:false}, background:'transparent', foreColor:'#B5C4E0' },
    series: [
    ...
    dataLabels: {
      enabled: true, enabledOnSeries: [0],
      formatter: v => fmtBsK(v),
      style: { fontSize:'10px', colors:['#93c5fd'] },
      background: { enabled:true, foreColor:'#1e293b', borderRadius:3, opacity:.85, borderWidth:0 },
      offsetY: -6,
    },
    tooltip: { theme:'dark', y:{formatter: v=>'Bs '+Math.round(v).toLocaleString('es-BO')} },
  ...
  dbeCharts.modalidad = new ApexCharts(document.querySelector('#dbe-chart-modalidad'), {
    chart: { type:'donut', height:260, background:'transparent', foreColor:'#B5C4E0' },
    series: modalidadesOrdenadas.map(([,v])=>Math.round(v)),
    labels: modalidadesOrdenadas.map(([k])=>k),
    colors: ['#4A8EFF','#F59E0B','#22C55E','#A855F7'],
    legend: {
  ...
  dbeCharts.velas = new ApexCharts(document.querySelector('#dbe-chart-velas'), {
    chart: { height:340, toolbar:{show:false}, background:'transparent', foreColor:'#B5C4E0', parentHeightOffset:0 },
    series: [
    ...
    tooltip: { theme:'dark', shared:true, x:{format:'dd MMM'} },
  });
...
function renderHudVistaAmpliada() {
  ...
  const baseOpts = {
    chart: { fontFamily:'Inter, sans-serif', toolbar:{show:true}, zoom:{enabled:false}, background:'transparent', foreColor:'#B5C4E0', height:420 },
    grid: { borderColor:'rgba(255,255,255,.06)', strokeDashArray:3 },
    tooltip: { theme:'dark' },
  };
  ...
  if (id === 'semanal') {
    ...
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts,
      ...
      colors: ['#4A8EFF','#F59E0B'], stroke:{width:[0,3], curve:'smooth'},
    });
  } else if (id === 'mensual') {
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts,
      ...
      colors: ['#4A8EFF'], stroke:{width:2, curve:'smooth'}, fill:{type:'gradient', gradient:{opacityFrom:.45,opacityTo:0}},
    });
    ...
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts, chart:{...baseOpts.chart, type:'donut'},
      ...
      dataLabels:{ formatter:v=>v.toFixed(0)+'%' }, legend:{ position:'bottom', labels:{colors:'#B5C4E0'} },
    });
  } else if (id === 'velas') {
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts, chart:{...baseOpts.chart, height:460},
      ...
      fill:{ opacity:[1,.35] }, colors:['#22C55E','#A855F7'],
    });
  }
...
function renderHudEstadisticas() {
  ...
  if (!valores.length) { grid.innerHTML = '<div class="hud-stat-box">Sin datos suficientes.</div>'; return; }

  const box = (label, value, destacado) => `<div class="hud-stat-box${destacado?' hl':''}"><div class="hud-stat-label">${label}</div><div class="hud-stat-value">${value}</div></div>`;
  grid.innerHTML = [
...
function renderOrangeCarteraAgregados() {
  ...
  cont.innerHTML = `
    <div style="text-align:center;margin-bottom:18px;">
      <div style="font-family:'Archivo Black',sans-serif;font-size:36px;color:var(--cartera);">${totalLotesVendidos||0}</div>
