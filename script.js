...
async function cargarDetalleCobrosCartera() {
  const btn = document.getElementById('btn-cargar-detalle-cartera');
  ...
  try {
    ...
    toastOk('Detalle cargado: ' + orangeCarteraDetalleRaw.length + ' cobros.');
  } catch(e) {
    toastErr('No se pudo cargar el detalle: ' + e.message);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '📋 Cargar detalle de cobros'; }
  }
}
