...
async function mostrarCotizacionPublica(key) {
  // Sacamos TODO el CSS del sistema administrativo (styles.css + cualquier <style> del head) —
  // si no, sus reglas (tipografía, bordes, colores del panel interno) pelean con el diseño
  // de esta página y todo sale desprolijo. Esta vista tiene que arrancar de cero, sola.
  ...
  document.body.innerHTML = `<div id="cotiz-publica-root" style="min-height:100vh;background:#F7F5F0;display:flex;align-items:center;justify-content:center;font-family:sans-serif;">
    <div style="text-align:center;color:#1F3A2E;font-size:14px;">Cargando tu cotización...</div>
  </div>`;
  ...
  // Una sola foto por cotización, rotando entre las 3 — siempre la misma para esta cotización puntual
  // (así si el cliente recarga la página, ve la misma foto), pero distinta entre cotizaciones distintas.
  const FOTOS_HERO = ['pileta.jpg', 'entrada.jpg', 'jardin.jpg'];
  let _hash = 0; for (let i=0;i<key.length;i++) _hash = (_hash*31 + key.charCodeAt(i)) % FOTOS_HERO.length;
  const fotoHero = FOTOS_HERO[_hash];
  ...
  root.innerHTML = `
    <style>
      ...
      *{box-sizing:border-box;}
      ...
      #cotiz-publica-root{ font-family:'Inter',sans-serif; background:#F7F5F0; color:#0E0E0E; -webkit-font-smoothing:antialiased; }
      .cp-serif{ font-family:'Fraunces',serif; }
      .cp-wrap{ max-width:640px; margin:0 auto; padding:0 28px; }
      .cp-top{ padding:40px 0 28px; }
      .cp-top .cp-word{ font-size:13px; color:#8A8A82; }
      .cp-top h1{ font-size:clamp(24px,5vw,30px); font-weight:500; margin-top:14px; line-height:1.25; }
      .cp-top p{ font-size:15px; color:#5b5b54; margin-top:10px; max-width:440px; line-height:1.55; }
      .cp-photo-full{ width:100%; height:360px; object-fit:cover; display:block; }
      .cp-block{ padding:36px 0; border-bottom:1px solid #E5E1D8; }
      .cp-block:last-of-type{ border-bottom:none; }
      .cp-block h2{ font-size:19px; font-weight:500; margin-bottom:18px; }
      .cp-deflist div{ display:flex; justify-content:space-between; align-items:baseline; padding:11px 0; border-bottom:1px solid #E5E1D8; font-size:15px; gap:16px; }
      .cp-deflist div:last-child{ border-bottom:none; }
      .cp-deflist span:first-child{ color:#5b5b54; }
      .cp-deflist span:last-child{ font-weight:500; text-align:right; }
      .cp-price-old{ font-size:16px; color:#8A8A82; text-decoration:line-through; }
      .cp-price-pct{ font-size:13px; color:#1F3A2E; font-weight:600; margin-left:8px; }
      .cp-price-final{ font-size:clamp(34px,8vw,46px); font-weight:500; margin-top:6px; line-height:1; }
      .cp-price-sub{ font-size:13px; color:#8A8A82; margin-top:8px; }
      .cp-price-rule{ width:52px; height:2px; background:#1F3A2E; margin-top:18px; }
      .cp-highlights p{ font-size:15px; line-height:1.6; color:#3a3a35; margin-bottom:16px; }
      .cp-highlights p:last-child{ margin-bottom:0; }
      .cp-highlights b{ color:#0E0E0E; font-weight:600; }
      .cp-estado{ text-align:center; padding:40px 0; }
      .cp-estado .cp-tag{ display:inline-block; font-size:13px; padding:7px 16px; border-radius:2px; font-weight:500; }
      .cp-tag-pendiente{ background:#0E0E0E; color:#fff; }
      .cp-tag-confirmado{ background:#1F3A2E; color:#fff; }
      .cp-estado p{ font-size:14px; color:#5b5b54; margin-top:16px; max-width:380px; margin-left:auto; margin-right:auto; line-height:1.6; }
      .cp-foot{ padding:28px 0 40px; font-size:12px; color:#8A8A82; line-height:1.7; }
      .cp-socios{ background:#0E0E0E; color:#fff; padding:40px 28px; text-align:center; }
      .cp-socios-lbl{ font-size:12px; color:#9a9a92; margin-bottom:16px; }
      .cp-socios-marcas{ display:flex; justify-content:center; align-items:center; gap:24px; flex-wrap:wrap; }
      .cp-marca{ font-family:'Fraunces',serif; font-size:19px; }
      .cp-dot{ color:#666; font-size:14px; }
    </style>

    <div class="cp-top cp-wrap">
      <div class="cp-word">Urubó Village</div>
      <h1 class="cp-serif">Preparado para ${esc(c.cliente)}</h1>
      <p>Terreno N° ${esc(c.terreno)}, Módulo ${esc(c.modulo)} — la propuesta que conversaste con tu asesor, con el detalle del precio y el plan de pago.</p>
    </div>

    <img class="cp-photo-full" src="${fotoHero}" alt="">

    <div class="cp-wrap">
      <div class="cp-block">
        <h2 class="cp-serif">El terreno</h2>
        <div class="cp-deflist">
          <div><span>Terreno</span><span>N° ${esc(c.terreno)} — Módulo ${esc(c.modulo)}</span></div>
          <div><span>Superficie</span><span>${(c.superficie||0).toLocaleString('es-BO')} m²</span></div>
          <div><span>Categoría</span><span>${esc(c.categoria||'—')}</span></div>
          <div><span>Modalidad</span><span>${esc(c.modalidad)}</span></div>
          ${c.conAccion==='SI' ? `<div><span>Acción del Country Club</span><span>Incluida — ${fmt(c.accionPrecio)}</span></div>` : ''}
        </div>
      </div>

      <div class="cp-block">
        <h2 class="cp-serif">Precio</h2>
        ${hayDescuento ? `<div><span class="cp-price-old">${fmt(c.precioLista)}</span><span class="cp-price-pct">-${c.descuentoPct}%</span></div>` : ''}
        <div class="cp-price-final cp-serif">${fmt(c.precioFinal)}</div>
        <div class="cp-price-sub">Precio total en dólares americanos</div>
        <div class="cp-price-rule"></div>
      </div>

      <div class="cp-block">
        <h2 class="cp-serif">Plan de pago</h2>
        <div class="cp-deflist">
          <div><span>Reserva</span><span>${fmt(c.reserva)}</span></div>
          <div><span>Aporte inicial (${c.pctAporte}%)</span><span>${fmt(c.montoAporte)}</span></div>
          <div><span>Saldo a financiar</span><span>${fmt(c.montoSaldo)}</span></div>
          <div><span>Cuotas</span><span>${c.nroCuotas} de ${fmt(c.montoCuota)}</span></div>
        </div>
      </div>

      <div class="cp-block cp-highlights">
        <h2 class="cp-serif">Por qué Urubó Village</h2>
        <p><b>Desarrollo emblemático.</b> Es el proyecto urbanístico que ha impulsado el crecimiento y la valorización de toda la zona del Urubó.</p>
        <p><b>Infraestructura de primer nivel.</b> Calles pavimentadas, servicios subterráneos, amplias áreas verdes y paisajismo profesional.</p>
        <p><b>Conectividad estratégica.</b> Acceso directo a Santa Cruz por el Puente Arq. Cristóbal Roda, obra financiada con inversión privada.</p>
        <p><b>Vida de club.</b> El Country Club ofrece piscinas, canchas de tenis y fútbol, gimnasio y espacios sociales para toda la familia.</p>
        <p><b>Alta plusvalía.</b> El Urubó mantiene una valorización constante, siendo una de las zonas más demandadas para vivir e invertir.</p>
      </div>
    </div>

    <div class="cp-socios">
      <div class="cp-socios-lbl">Un desarrollo de</div>
      <div class="cp-socios-marcas">
        <span class="cp-marca">Grupo Roda</span><span class="cp-dot">•</span><span class="cp-marca">Grupo Kinn</span>
      </div>
    </div>

    <div class="cp-wrap">
      <div class="cp-estado">
        <span class="cp-tag ${estadoReal==='confirmado'?'cp-tag-confirmado':'cp-tag-pendiente'}">${estadoReal==='confirmado' ? '✓ Confirmada' : 'Pendiente de confirmación'}</span>
        <p>${estadoReal==='confirmado'
          ? 'Tu asesor ya confirmó esta cotización — se va a comunicar para continuar con el proceso.'
          : 'Tu asesor va a confirmar esta cotización una vez que hables con él. Cualquier consulta, escribile directo.'}</p>
      </div>

      <div class="cp-foot">
        Constructora e Inmobiliaria Urubó Village S.A. — NIT 293710026<br>
        Asesor: ${esc(c.asesorNombre||'—')}
      </div>
    </div>`;
}

function _cotizPublicaErrorHtml(mensaje) {
  return `<div style="text-align:center;padding:60px 24px;color:#1F3A2E;font-family:'Inter',sans-serif;max-width:420px;">
