...
function renderDetalleCierres() {
  ...
  if (!cierres.length) {
    tbody.innerHTML = '<tr><td colspan="10"><div class="empty-state">Sin cierres todavía.</div></td></tr>';
    if (tfoot) tfoot.innerHTML = '';
    return;
  }

  tbody.innerHTML = cierres.map(r => {
    ...
    const precioM2 = (metraje && r.precioVenta) ? Math.round((r.precioVenta/metraje)*100)/100 : (r.precioM2Final || null);
    return `<tr>
      ...
      <td>${precioM2 ? '$'+precioM2.toLocaleString('es-BO',{maximumFractionDigits:2}) : '—'}</td>
