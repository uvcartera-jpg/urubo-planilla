...
async function sincronizarInventarioOrange() {
  ...
  try {
    ...
    // Disponibilidad de Orange → nuestro Estado. "Bloqueado" (reservas internas de Orange
    // fuera de venta) lo mapeamos a "No disponible" porque no es lo mismo que nuestro "Reservado".
    const mapEstado = { 'Vendido':'Vendido', 'Disponible':'Disponible', 'Reservado':'Reservado', 'Bloqueado':'No disponible' };

    const lotesUV = json.data.filter(l => l.Pertenece === 'UV'); // solo Urubó Village — se salta LCK, GIR, etc.
    let creados = 0, actualizados = 0;

    for (let i = 0; i < lotesUV.length; i++) {
      const l = lotesUV[i];
      const terreno = String(l.Terreno || '').trim();
      if (!terreno) continue;

      setBtn(`🔄 Sincronizando ${i+1}/${lotesUV.length}...`, true);
