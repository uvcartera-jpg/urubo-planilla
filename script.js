...
async function renderDashboardExpensas() {
  ...
  dbeCharts.semanal = new ApexCharts(document.querySelector('#dbe-chart-semanal'), {
    chart: { type:'line', height:230, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8' },
    series: [
    ...
    yaxis: { labels: { formatter: v => fmtBsK(v) } },
  ...
  dbeCharts.mensual = new ApexCharts(document.querySelector('#dbe-chart-mensual'), {
    chart: { type:'area', height:230, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8' },
    series: [
  ...
  dbeCharts.modalidad = new ApexCharts(document.querySelector('#dbe-chart-modalidad'), {
    chart: { type:'donut', height:230, background:'transparent', foreColor:'#94a3b8' },
  ...
  /* ── Gráfico 4: Candlestick + Volumen (últimos 20 días con cobros) ── */
  ...
  dbeCharts.velas = new ApexCharts(document.querySelector('#dbe-chart-velas'), {
    chart: { type:'candlestick', height:190, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8' },
  ...
  dbeCharts.volumen = new ApexCharts(document.querySelector('#dbe-chart-volumen'), {
    chart: { type:'bar', height:90, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8' },
    series: [{ name:'Transacciones', data: velas20.map(d=>({x:d.x, y:d.v})) }],
...
function renderSemanalExpensas() {
  ...
  delMes.forEach(r => {
    ...
    (porSemana[key] ||= { anio, semana, total:0, meta:0 });
    porSemana[key].total += r.monto;
    const cat = expensasCategoriasData[r.key]?.categoria;
    if (cat === 'Vigente' || cat === 'Mora') porSemana[key].meta += r.monto;
  });
  const semanas = Object.values(porSemana).sort((a,b)=>a.semana-b.semana);
  if (!semanas.length) { tbody.innerHTML = '<tr><td colspan="5"><div class="empty-state">Sin cobros en ' + expFilMes + ' ' + expFilAnio + '.</div></td></tr>'; return; }
  tbody.innerHTML = semanas.map(s => `<tr>
    ...
    <td>${_fmtMontoBs(s.meta)}</td>
    <td style="color:var(--ok);">${_fmtMontoBs(s.meta)}</td>
    <td style="color:var(--gris);">${_fmtMontoBs(s.total-s.meta)}</td>
  </tr>`).join('') + `<tr style="font-weight:700;background:var(--bg-soft);">
    <td>Total</td><td>${_fmtMontoBs(semanas.reduce((s,x)=>s+x.total,0))}</td><td colspan="2"></td>
    <td>${_fmtMontoBs(semanas.reduce((s,x)=>s+(x.total-x.meta),0))}</td></tr>`;
  ...
  if (canvas) {
    ...
    _chartSemanalExpensas = new Chart(canvas, {
      ...
      options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{position:'top'}, datalabels:{display:false} }, scales:{ y:{beginAtZero:true, ticks:{callback:v=>_fmtMontoBs(v)}} } }
    });
  }
}
