...
function _abrirVentanaDocumento(titulo, htmlBody) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${titulo}</title><meta charset="utf-8">
    <style>
      body{ font-family:Arial,Helvetica,sans-serif; font-size:13px; color:#111; padding:30px; max-width:750px; margin:0 auto; }
      h1{ text-align:center; font-size:18px; text-decoration:underline; margin-bottom:24px; }
      table{ width:100%; border-collapse:collapse; margin:10px 0; }
      td,th{ border:1px solid #333; padding:6px 10px; font-size:12.5px; }
      .header-band{ background:#1e3a5f; color:#fff; font-weight:700; padding:6px 10px; }
      .no-border td{ border:none; padding:3px 0; }
      .right{ text-align:right; } .center{ text-align:center; }
      .sign-box{ height:70px; border:1px solid #333; }
      @media print { body{ padding:10px; } }
      button.print-btn{ margin-bottom:20px; padding:10px 18px; background:#2451FF; color:#fff; border:none; border-radius:6px; font-weight:700; cursor:pointer; }
    </style></head><body>
...
function generarMateriaReservada() {
  ...
  const filasCuotas = cuotas.map(c => `<tr><td>${_fmtFechaDoc(c.fecha)}</td><td colspan="2" class="center">${esc(c.concepto)}</td><td class="right">$${(c.monto||0).toLocaleString('es-BO')}</td></tr>`).join('');

  const html = `
    <h1>Materia Reservada de Venta</h1>
    <table class="no-border" style="margin-bottom:14px;">
      <tr><td><strong>Fecha:</strong></td><td>${_fmtFechaDoc(document.getElementById('mr-fecha').value)}</td><td class="right" style="font-size:22px;color:#c00;font-weight:700;">${esc(document.getElementById('mr-nro-reserva').value)}</td></tr>
      <tr><td><strong>Cliente:</strong></td><td colspan="2">${esc(r.nombre)}</td></tr>
      <tr><td><strong>Terreno:</strong></td><td>${esc(r.terrenoInteres||'—')} (Módulo ${esc(modulo)})</td><td></td></tr>
      <tr><td><strong>Precio:</strong></td><td>$${(r.precioVenta||0).toLocaleString('es-BO')}</td><td><strong>Acción:</strong> ${conAccion?'SÍ — $'+precioAccion.toLocaleString('es-BO'):'NO'}</td></tr>
      <tr><td><strong>M2:</strong></td><td>${(r.metraje||0).toLocaleString('es-BO')}</td><td><strong>$/M2:</strong> $${r.precioM2Final||0}</td></tr>
      <tr><td><strong>T/C:</strong></td><td>${esc(document.getElementById('mr-tc').value)}</td><td><strong>Canal:</strong> ${esc(r.origen||'—')}</td></tr>
      <tr><td><strong>Contrato:</strong></td><td>${esc(document.getElementById('mr-tipo-contrato').value)}</td><td></td></tr>
    </table>

    <table><tr class="header-band"><td colspan="2">Política Actual</td></tr></table>
    <table style="margin-top:-10px;">
      <tr><td><strong>${esc(r.formaPago||'Modalidad')}</strong></td><td class="right">Descuento máx.: ${esc(document.getElementById('mr-politica-pct').value||'—')}</td></tr>
    </table>

    <table><tr class="header-band"><td colspan="4">Propuesta</td></tr></table>
    <table style="margin-top:-10px;">
      <tr><td><strong>Modalidad</strong></td><td><strong>Descuento</strong></td><td><strong>% Dcto</strong></td><td><strong>Precio de Venta</strong></td></tr>
      <tr><td>${esc(r.formaPago||'—')}</td><td>$${(r.descuentoUsd||0).toLocaleString('es-BO')}</td><td>${r.descuentoPct||0}%</td><td>$${(r.precioVenta||0).toLocaleString('es-BO')}</td></tr>
    </table>

    <table>
      <tr><th class="center">Fecha</th><th colspan="2" class="center">Concepto</th><th class="center">Monto</th></tr>
      ${filasCuotas}
      <tr style="background:#cfe8f5;"><td colspan="3"><strong>Total</strong></td><td class="right"><strong>$${totalCuotas.toLocaleString('es-BO')}</strong></td></tr>
    </table>

    <table class="no-border" style="margin-top:14px;">
      <tr><td><strong>Comentario sobre la venta:</strong></td></tr>
      <tr><td style="border:1px solid #333;padding:10px;min-height:40px;">${esc(document.getElementById('mr-comentario').value)}</td></tr>
    </table>
    ...
    <p style="margin-top:16px;font-size:12px;"><strong>Correo:</strong> ${esc(document.getElementById('mr-correo').value)} &nbsp;&nbsp; <strong>Celular:</strong> ${esc(document.getElementById('mr-celular').value)}</p>`;
...
function generarSolicitudContrato() {
  ...
  const cuotas = docCuotasLineas.mr;
  const filasCuotas = cuotas.map(c => `<tr><td>${esc(c.concepto)}</td><td class="right">$${(c.monto||0).toLocaleString('es-BO')}</td><td class="center">${_fmtFechaDoc(c.fecha)}</td></tr>`).join('');

  const html = `
    <h1>Solicitud de Elaboración de Contrato</h1>
    <table class="no-border">
      <tr><td><strong>Nombre del Cliente:</strong></td><td>${esc(r.nombre)}</td></tr>
      ...
      <tr><td><strong>Fecha de la solicitud:</strong></td><td>${_fmtFechaDoc(document.getElementById('mr-fecha').value)}</td></tr>
    </table>

    <table class="no-border" style="margin-top:10px;">
      <tr><td><strong>Terreno:</strong></td><td colspan="3">${esc(r.terrenoInteres||'—')} — módulo ${esc(modulo)}</td></tr>
      <tr>
        <td><strong>Forma de pago:</strong></td>
        <td>Contado [${modalidadesCheck['Contado']}]</td>
        <td>Contado Diferido [${modalidadesCheck['Contado Diferido']}]</td>
        <td>Créd. Directo [${modalidadesCheck['Crédito Directo']}] / Créd. Bancario [${modalidadesCheck['Crédito Bancario']}]</td>
      </tr>
    </table>

    <p style="margin-top:14px;"><strong>Observaciones:</strong></p>
    <div style="border:1px solid #333;padding:10px;min-height:70px;">
    ...
    <table style="margin-top:16px;">
      <tr><td><strong>Precio de lista del terreno:</strong></td><td class="right">$${(r.precioLista||0).toLocaleString('es-BO')}</td></tr>
      <tr><td><strong>Descuento:</strong></td><td class="right">$${(r.descuentoUsd||0).toLocaleString('es-BO')}</td></tr>
      <tr><td><strong>Precio de venta:</strong></td><td class="right"><strong>$${(r.precioVenta||0).toLocaleString('es-BO')}</strong></td></tr>
    </table>
    ...
    <table class="no-border" style="margin-top:24px;">
      <tr><td><strong>Solicitado por:</strong></td></tr>
      <tr><td style="border:1px solid #333;height:60px;">${esc(r.asesorNombre||'')}</td></tr>
      <tr><td><strong>Autorizado por:</strong></td></tr>
      <tr><td style="border:1px solid #333;height:60px;">${esc(document.getElementById('mr-autorizado-por').value)}</td></tr>
    </table>
