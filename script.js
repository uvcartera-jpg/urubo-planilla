...
function renderCategorizacionCobros() {
  ...
  if (!tbody) return;
  const pendientes = orangeCarteraDetalleRaw.filter(r => !carteraCategoriasData[r.key]);
  if (contador) contador.textContent = pendientes.length + ' pendiente' + (pendientes.length!==1?'s':'') + ' por categorizar';

  if (!pendientes.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">✅ Todos los cobros cargados están categorizados.</div></td></tr>';
    return;
  }
  ...
  </tr>`).join('') + (pendientes.length>200 ? `<tr><td colspan="6" style="text-align:center;color:var(--gris);padding:10px;">Mostrando 200 de ${pendientes.length} — categorizá estos y se van cargando los siguientes.</td></tr>` : '');
}
