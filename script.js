...
async function guardarDocumentoReserva() {
  ...
  const data = {
    ...
    tipoContrato: document.getElementById('mr-tipo-contrato').value,
...
function _restaurarDatosDocGuardado(d) {
  ...
  document.getElementById('mr-tipo-contrato').value = d.tipoContrato||'TCO';
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${titulo}</title><meta charset="utf-8">
    <style>
      ...
      *{ box-sizing:border-box; }
      body{ font-family:Arial,Helvetica,sans-serif; font-size:13px; line-height:1.45; color:#111; padding:14px; max-width:720px; margin:0 auto; }
      h1{ text-align:center; font-size:17px; text-decoration:underline; margin:0 0 14px; font-weight:700; }
      ...
      .fila .etq{ font-weight:700; flex-shrink:0; }
      .fila.dos{ display:grid; grid-template-columns:1fr 1fr; gap:4px 20px; }
      .bloque-sombreado{ background:#f2f2f2; padding:6px 8px; border-radius:2px; }
      ...
      td,th{ border:1px solid #333; padding:5px 8px; font-size:12.5px; word-wrap:break-word; }
      .header-band{ background:#1e3a5f; color:#fff; font-weight:700; padding:4px 8px; font-size:12.5px; }
      .right{ text-align:right; } .center{ text-align:center; }
      .sign-box{ height:58px; border:1px solid #333; width:50%; }
      .dashed td{ border:1px dashed #999; }
...
function generarMateriaReservada() {
  ...
  const bloqueAccion = conAccion ? `
    ...
    <table style="margin-top:0;">
      ...
      <tr style="background:#cfe8f5;"><td colspan="3"><strong>Total Acción</strong></td><td class="right"><strong>$${cuotasAccion.reduce((s,c)=>s+(c.monto||0),0).toLocaleString('es-BO')}</strong></td></tr>
    </table>` : '';
  ...
  const html = `
    ...
    <div class="fila dos" style="grid-template-columns:1.4fr 1fr;">
      <span><span class="etq">T/C:</span> ${esc(document.getElementById('mr-tc').value)}</span>
      <span><span class="etq">Categoría:</span> ${esc(categoria||'—')}</span>
    </div>
    <div class="fila"><span class="etq">Contrato:</span><span>${esc(document.getElementById('mr-tipo-contrato').value)}</span></div>
    ...
    <table style="margin-top:0;">
      ...
      <tr><td><strong>Modalidad elegida</strong></td>${checklistModalidad.map(c=>`<td class="center">${c?'☑':'☐'}</td>`).join('')}</tr>
      ...
      <tr><td><strong>Precio Mínimo</strong></td>${MODALIDADES_4.map(()=>'<td class="center">—</td>').join('')}</tr>
      <tr><td><strong>M2$ Mínimo</strong></td>${MODALIDADES_4.map(()=>'<td class="center">—</td>').join('')}</tr>
    </table>
    ...
    <table class="dashed">
      ...
      ${filasCuotas}
      <tr style="background:#cfe8f5;"><td colspan="2"><strong>Total</strong></td><td colspan="2" class="right"><strong>$${totalCuotas.toLocaleString('es-BO')}</strong></td></tr>
    </table>
...
function generarSolicitudContrato() {
  ...
  const html = `
    ...
    <div style="margin-top:14px;">
      ...
      <div class="fila"><span class="etq" style="width:220px;">Precio de venta:</span><span><strong>${(r.precioVenta||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</strong></span></div>
    </div>
