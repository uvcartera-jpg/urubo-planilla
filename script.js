...
function procesarExcelExpensas(input) {
  ...
  reader.onload = e => {
    ...
    if (headerRow >= 0) {
      ...
      for (let i = headerRow+1; i < aoa.length; i++) {
        ...
        parsed.push({
          ...
          mes: MESES_ES_NOMBRE[per.mesNum],
          anio: per.anio,
          semana: 'S' + (() => {
            // semana relativa al mes (1-5), más intuitiva que la ISO del año para el usuario
      ...
      // Calcular estado real por lote, combinando lo que trae este archivo
      // + lo que ya existe cargado en el sistema para ese mismo lote (semanas previas)
      const periodosPorLote = {};
      parsed.forEach(r => {
        (periodosPorLote[r.terreno] ||= []).push({mesNum:r.mesPeriodoNum, anio:r.anioPeriodo});
      });
      expAdminData.forEach(r => {
        if (r.terreno && r.mesPeriodoNum && r.anioPeriodo) {
          (periodosPorLote[r.terreno] ||= []).push({mesNum:r.mesPeriodoNum, anio:r.anioPeriodo});
        }
      });
      parsed.forEach(r => { r.estado = calcularEstadoExpensa(periodosPorLote[r.terreno]); });
    ...
    // Selector de categoría (elegida por el usuario antes de confirmar)
    const catSel = document.getElementById('exp-import-categoria');
    if (catSel) {
      const cats = (typeof categoriasData !== 'undefined' ? categoriasData : []).filter(c => c.tipo === 'expensas');
      catSel.innerHTML = cats.length
        ? cats.map(c => `<option value="${esc(c._key||c.categoriaKey||'')}">${esc(c.nombre||c.categoriaNombre)}</option>`).join('')
        : `<option value="">Expensa común</option>`;
    }
    ...
    if (tbody) tbody.innerHTML = expensasExcelPend.map(r=>`<tr>
      <td>${esc(r.cliente)}</td>
      <td>${esc(r.terreno)}</td>
      <td>${esc(r.mes)} ${esc(String(r.anio))}${r.tipoPago && r.tipoPago!=='completo' ? ' <span style="color:var(--gris);font-size:var(--fs-xs);">('+esc(r.tipoPago)+')</span>' : ''}</td>
      <td>${esc(r.semana)}</td>
...
async function confirmarExcelExpensas() {
  ...
  const catNombre = catSel?.options[catSel.selectedIndex]?.text || 'Expensa común';
  ...
  for (const r of expensasExcelPend) {
    try {
      const data = { ...r, categoriaKey: catKey, categoriaNombre: catNombre, cargaId, cargaArchivo: cargaNombreArchivo };
      if (r.terreno && r.mesPeriodoNum && r.anioPeriodo) {
...
function renderExpensasAdmin() {
  ...
  const ESTADOS_KPI = ['Vigente','Mora 1-30','Mora 31-60','Mora +60'];
  const porEstado = { 'Vigente':{monto:0,n:0}, 'Mora 1-30':{monto:0,n:0}, 'Mora 31-60':{monto:0,n:0}, 'Mora +60':{monto:0,n:0}, 'Sin gestión':{monto:0,n:0} };
  datos.forEach(e => {
    const key = porEstado[e.estado] ? e.estado : 'Sin gestión';
    porEstado[key].monto += (e.monto||0);
    porEstado[key].n += 1;
  });
  const idsEstado = { 'Vigente':'vigente', 'Mora 1-30':'mora1', 'Mora 31-60':'mora2', 'Mora +60':'mora3' };
...
function renderDetalleExpensasPorCliente() {
  const cont = document.getElementById('exp-detalle-lista');
  if (!cont) return;
  ...
  const grupos = Object.values(porLote).sort((a,b)=> (a.cliente||'').localeCompare(b.cliente||''));
    ...
    return `
      <div class="admin-section" style="margin-bottom:10px;padding:16px 18px;">
        <div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;" onclick="const el=document.getElementById('exp-det-${idx}'); el.style.display = el.style.display==='none'?'block':'none';">
          <div>
            <div style="font-weight:600;">${esc(g.cliente)} ${_pintarEstadoExp(estado)}</div>
        ...
        <div id="exp-det-${idx}" style="display:none;margin-top:12px;">
          <div class="tabla-wrap">
            <table><thead><tr><th>Período</th><th>Semana</th><th>Monto</th><th>Tipo</th><th>Cargado</th></tr></thead>
            <tbody>
              ${filas.map(r=>`<tr>
                <td>${esc(r.mes||'')} ${esc(String(r.anio||''))}</td>
                <td>${esc(r.semana||'—')}</td>
                <td style="font-weight:600;">Bs ${(r.monto||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</td>
                <td>${esc(r.tipoPago||'—')}</td>
                <td style="font-size:var(--fs-xs);color:var(--gris);">${esc(r.fechaCarga||'')}</td>
              </tr>`).join('')}
            </tbody></table>
          </div>
        </div>
