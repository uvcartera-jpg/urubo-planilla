...
async function cargarDetalleCobrosCartera(silencioso) {
  ...
  try {
    ...
    orangeCarteraDetalleRaw = json.data.map(r => {
      ...
      return {
        fecha: r.FechaDePago || '', fechaOrden: _parseFechaOrange(r.FechaDePago)?.getTime() || 0,
        cliente: r.NombreCliente||'', terreno: r.Terreno||'', asesor: r.Asesor||'',
        modalidad: r.Modalidad||'Sin modalidad', modulo: r.Modulo||'Sin módulo',
        tipoPago: esTerreno ? 'Terreno' : 'Acción', monto, tipoIngreso,
      };
...
function renderIngresosVentaNuevaVsCartera() {
  ...
  const barra = document.getElementById('orange-cart-barra-nueva-vs-cartera');
  if (barra) barra.style.width = pctNueva + '%';
  setText('orange-cart-pct-nueva', pctNueva + '%');
}
