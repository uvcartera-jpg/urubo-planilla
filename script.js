...
function renderDocumentosGuardados() {
  ...
  tbody.innerHTML = datos.map(d => {
    ...
    <td>
      <button class="btn-clear" style="padding:5px 10px;font-size:var(--fs-sm);" onclick="abrirDocumentoGuardado('${d._key}')">✏️ Editar</button>
      <button class="btn-clear" style="padding:5px 10px;font-size:var(--fs-sm);color:var(--danger);" onclick="eliminarDocumentoGuardado('${d._key}')">🗑</button>
    </td>
  </tr>`;
  }).join('');
}
...
function abrirDocumentoGuardado(key) {
  ...
  if (!r) { toastErr('El registro original ya no existe.'); return; }

  // Nos vamos al formulario "Nuevo" para poder EDITAR antes de generar, no directo a imprimir.
  ...
  // Rellenamos los campos editables con lo guardado.
  ...
  toggleAccionDoc('mr'); // muestra/oculta según con-accion, y ya no pisa mrAccion porque ya tiene datos
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${titulo}</title><meta charset="utf-8">
    <style>
      ...
      *{ box-sizing:border-box; }
      body{ font-family:Arial,Helvetica,sans-serif; font-size:13px; line-height:1.4; color:#111; padding:14px; max-width:760px; margin:0 auto; }
      h1{ text-align:center; font-size:17px; text-decoration:underline; margin:0 0 14px; font-weight:700; }
      table{ width:100%; border-collapse:collapse; margin:5px 0; }
      td,th{ border:1px solid #333; padding:4px 8px; font-size:12.5px; }
      ...
      .sign-box{ height:56px; border:1px solid #333; }
...
function generarMateriaReservada() {
  ...
  const html = `
    <table class="no-border"><tr>
      <td style="width:80%;"><h1 style="text-align:left;margin:0;">Materia Reservada de Venta</h1></td>
      <td style="width:20%;text-align:right;">${_logoHtmlDoc()}</td>
    </tr></table>

    <table class="no-border" style="margin-bottom:8px;">
      <tr><td style="width:16%;"><strong>Fecha:</strong></td><td style="width:44%;">${_fmtFechaDoc(document.getElementById('mr-fecha').value)}</td>
          <td style="width:20%;"></td><td class="right" style="width:20%;font-size:18px;color:#c00;font-weight:700;">${esc(document.getElementById('mr-nro-reserva').value)}</td></tr>
      <tr><td><strong>Cliente:</strong></td><td colspan="3">${esc(r.nombre)}</td></tr>
      <tr><td><strong>Terreno:</strong></td><td>${esc(_fmtTerrenoDescriptivo(r.terrenoInteres))}</td><td><strong>Acción:</strong></td><td>${conAccion?'Sí':'No'}</td></tr>
      <tr><td><strong>Precio:</strong></td><td>${(r.precioVenta||0).toLocaleString('es-BO')}</td><td><strong>$/M2:</strong></td><td>${r.precioM2Final||0}</td></tr>
      <tr><td><strong>M2:</strong></td><td>${(r.metraje||0).toLocaleString('es-BO')}</td><td><strong>Canal:</strong></td><td>${esc(r.origen||'—')}</td></tr>
      <tr><td><strong>T/C:</strong></td><td>${esc(document.getElementById('mr-tc').value)}</td><td><strong>Categoría:</strong></td><td>${esc(categoria||'—')}</td></tr>
      <tr><td><strong>Contrato:</strong></td><td colspan="3">${esc(document.getElementById('mr-tipo-contrato').value)}</td></tr>
    </table>

    <table><tr class="header-band"><td colspan="5">1 &nbsp; Política Actual</td></tr></table>
    <table style="margin-top:0;">
    ...
    <table class="no-border" style="margin-top:8px;">
      <tr><td><strong>Comentario sobre la venta:</strong></td></tr>
      <tr><td style="border:1px solid #333;padding:6px;min-height:26px;">${esc(document.getElementById('mr-comentario').value)}</td></tr>
    </table>
    ...
    <table style="margin-top:14px;">
      ...
      <tr><th class="center">Gerencia Comercial</th><th class="center">Gerencia Administrativa y Financiera</th></tr>
      <tr><td class="sign-box"></td><td class="sign-box"></td></tr>
    </table>

    <p style="margin-top:10px;font-size:10px;"><strong>Correo:</strong> ${esc(document.getElementById('mr-correo').value)} &nbsp;&nbsp; <strong>Celular:</strong> ${esc(document.getElementById('mr-celular').value)}
...
function generarSolicitudContrato() {
  ...
  const filasPie = filasContacto.map((c,i) => `<tr><td>${c}</td><td>${filasCelular[i]||''}</td></tr>`).join('');

  const html = `
    <h1 style="text-align:center;">SOLICITUD DE ELABORACION DE CONTRATO</h1>

    <table class="no-border" style="background:#f2f2f2;">
      <tr><td style="width:26%;"><strong>Nombre del Cliente:</strong></td><td>${esc(r.nombre)}</td></tr>
      ${tieneCoprop ? `<tr><td><strong>Nombre Copropietario:</strong></td><td>${esc(document.getElementById('mr-copropietario-nombre').value)}</td></tr>` : ''}
      <tr><td><strong>Fecha de la solicitud:</strong></td><td>${_fmtFechaLargaDoc(document.getElementById('mr-fecha').value)}</td></tr>
    </table>

    <table class="no-border" style="margin-top:10px;">
      <tr><td style="width:26%;"><strong>Terreno:</strong></td><td colspan="3">${esc(_fmtTerrenoDescriptivo(r.terrenoInteres))}</td></tr>
    </table>
    <table class="no-border">
      <tr>
        <td style="width:26%;"><strong>Forma de pago:</strong></td>
        <td style="width:37%;">Contado <span style="border:1px solid #333;display:inline-block;width:45px;height:24px;text-align:center;font-size:13px;line-height:22px;">${modalidadesCheck['Contado']}</span></td>
        <td style="width:37%;">Crédito Directo <span style="border:1px solid #333;display:inline-block;width:45px;height:24px;text-align:center;font-size:13px;line-height:22px;">${modalidadesCheck['Crédito Directo']}</span></td>
      </tr>
      <tr>
        <td></td>
        <td>Contado Diferido <span style="border:1px solid #333;display:inline-block;width:45px;height:24px;text-align:center;font-size:13px;line-height:22px;">${modalidadesCheck['Contado Diferido']}</span></td>
        <td>Crédito Bancario <span style="border:1px solid #333;display:inline-block;width:45px;height:24px;text-align:center;font-size:13px;line-height:22px;">${modalidadesCheck['Crédito Bancario']}</span></td>
      </tr>
    </table>

    <p style="margin:10px 0 3px;"><strong>Observaciones:</strong></p>
    ...
    <table class="no-border" style="margin-top:12px;">
      <tr><td style="width:35%;"><strong>Precio de lista del terreno:</strong></td><td class="right" style="background:#f2f2f2;">${(r.precioLista||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</td></tr>
      <tr><td><strong>Descuento:</strong></td><td class="right" style="background:#f2f2f2;">${(r.descuentoUsd||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</td></tr>
      <tr><td><strong>Precio de venta:</strong></td><td class="right"><strong>${(r.precioVenta||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</strong></td></tr>
    </table>
