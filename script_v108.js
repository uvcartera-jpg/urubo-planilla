...
function renderHudEstadisticas() {
  ...
  const id = hudModalChartActual;
  let valores = [];
  if (id === 'semanal') valores = d.semanas10.map(s=>s.total);
  else if (id === 'mensual') valores = d.meses12.map(m=>m.total);
  else if (id === 'modalidad') valores = d.modalidadesOrdenadas.map(([,v])=>v);
  else if (id === 'velas') valores = d.velas20.map(x=>x.c);

  if (!valores.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos suficientes.</div>'; return; }

  const box = (label, value, destacado) => `<div class="bi-stat-box${destacado?' hl':''}"><div class="bi-stat-label">${label}</div><div class="bi-stat-value">${value}</div></div>`;
  grid.innerHTML = [
    box('Media', d.fmtBsK(HudStats.mean(valores)), true),
    box('Mediana', d.fmtBsK(HudStats.median(valores))),
    box('Mínimo', d.fmtBsK(HudStats.min(valores))),
    box('Máximo', d.fmtBsK(HudStats.max(valores))),
    box('Desv. estándar', d.fmtBsK(HudStats.stddev(valores))),
    box('Coef. Variación', HudStats.cv(valores).toFixed(1)+'%', HudStats.cv(valores)>30),
    box('Q1 (25%)', d.fmtBsK(HudStats.q1(valores))),
    box('Q3 (75%)', d.fmtBsK(HudStats.q3(valores))),
    box('Rango Intercuartil', d.fmtBsK(HudStats.iqr(valores))),
...
function abrirModalGraficoJS(canvasId, titulo, sub, tipoAnalisis) {
  ...
  if (!original) { toastErr('Ese gráfico todavía no cargó — esperá un segundo y probá de nuevo.'); return; }
  ...
  document.getElementById('cjs-modal').classList.add('open');
  cambiarCjsTab('vista');
}
...
function renderCjsEstadisticas() {
  ...
  const original = Chart.getChart(_cjsModalOrigenId);
  if (!original) { grid.innerHTML = '<div class="bi-stat-box">Sin datos.</div>'; return; }

  const labels = original.data.labels || [];
  const ds = original.data.datasets.find(d => Array.isArray(d.data));
  if (!ds || !ds.data.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos para analizar.</div>'; return; }
  ...
  /* ── DISPERSIÓN: puntos {x,y} — correlación entre 2 variables ── */
  if (tipo === 'dispersion') {
    const puntos = ds.data.filter(p => p && typeof p.x === 'number' && typeof p.y === 'number');
    if (!puntos.length) { grid.innerHTML = '<div class="bi-stat-box">Sin puntos numéricos.</div>'; return; }
    ...
    const direccion = correlacion > .05 ? 'positiva (suben juntas)' : correlacion < -.05 ? 'negativa (una sube, la otra baja)' : 'prácticamente nula';
    grid.innerHTML = [
      box('Correlación', correlacion.toFixed(2), true, Math.abs(correlacion)<.3?'warn':null),
      box('Relación', `${fuerza} y ${direccion}`),
  ...
  /* ── COMPOSICIÓN: partes de un todo (donut) — quién domina, qué tan concentrado está ── */
  if (tipo === 'composicion') {
    ...
    const concentracion = pctTop2Juntas > 80 ? 'Alta — pocas categorías dominan casi todo' : pctTop2Juntas > 55 ? 'Media' : 'Baja — está bien repartido';
    grid.innerHTML = [
      box('Total', f(total), true),
      box('Líder', `${top?.l||'—'} (${pctTop.toFixed(0)}%)`),
      box('Valor del líder', f(top?.v||0)),
      box('Concentración', concentracion, false, pctTop2Juntas>80?'warn':null),
      box('N° de categorías', pares.length),
    ].join('');
    return;
  }

  /* ── RANKING: comparación entre personas/categorías — quién gana, quién pierde, la brecha ── */
  if (tipo === 'ranking') {
    const valores = ds.data.map(v=>Number(v)||0);
    const pares = labels.map((l,i)=>({l, v:valores[i]})).sort((a,b)=>b.v-a.v);
    ...
    const brecha = (lider?.v||0) - (ultimo?.v||0);
    grid.innerHTML = [
      box('Líder', `${lider?.l||'—'}`, true),
      box('Valor del líder', f(lider?.v||0), true),
      box('Último lugar', `${ultimo?.l||'—'} — ${f(ultimo?.v||0)}`),
      box('Brecha (líder vs. último)', f(brecha), false, brecha > media ? 'warn':null),
  ...
  /* ── TENDENCIA (por defecto): algo que evoluciona en el tiempo ── */
  const valores = ds.data.map(v => (v && typeof v==='object') ? Number(v.y) : Number(v)).filter(v=>!isNaN(v));
  if (!valores.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos numéricos.</div>'; return; }
  ...
  // Tendencia: pendiente de una regresión lineal simple sobre los puntos
  const n = valores.length;
  ...
  const direccionTendencia = pendiente > my*0.02 ? '📈 Creciendo' : pendiente < -my*0.02 ? '📉 Bajando' : '➡️ Estable';
  const proyeccionSiguiente = ultimo + pendiente;

  grid.innerHTML = [
    box('Valor actual', f(ultimo), true),
    box('Crecimiento total del período', (crecimientoTotal>=0?'+':'')+crecimientoTotal.toFixed(1)+'%', false, Math.abs(crecimientoTotal)>25?'warn':null),
    box('Tendencia', direccionTendencia),
    box('Mejor momento', `${labels[idxMax]||'—'} (${f(valores[idxMax])})`),
    box('Peor momento', `${labels[idxMin]||'—'} (${f(valores[idxMin])})`),
    box('Promedio del período', f(my)),
