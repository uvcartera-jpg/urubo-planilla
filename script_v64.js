...
// ── Filtro global del panel de Ventas — Año + Mes, o Gestión completa (abril-marzo) ──
...
let ventasModoVista = 'mes'; // 'mes' | 'gestion'
let ventasFilGestion = _gestionActual();

function cambiarModoVentas(modo) {
  ventasModoVista = modo;
  const btnMes = document.getElementById('btn-vmodo-mes');
  const btnGestion = document.getElementById('btn-vmodo-gestion');
  const selMes = document.getElementById('fil-ventas-mes');
  const selGestion = document.getElementById('fil-ventas-gestion');
  if (modo === 'gestion') {
    btnMes.style.background = 'transparent'; btnMes.style.boxShadow = 'none';
    btnGestion.style.background = '#fff'; btnGestion.style.boxShadow = '0 1px 3px rgba(0,0,0,.1)';
    if (selMes) selMes.style.display = 'none';
    if (selGestion) selGestion.style.display = '';
  } else {
    btnGestion.style.background = 'transparent'; btnGestion.style.boxShadow = 'none';
    btnMes.style.background = '#fff'; btnMes.style.boxShadow = '0 1px 3px rgba(0,0,0,.1)';
    if (selMes) selMes.style.display = '';
    if (selGestion) selGestion.style.display = 'none';
  }
  renderDashboard();
}
...
// ¿Este registro cae dentro del período elegido? Sirve para "por mes" (año+mes exacto)
// y para "gestión completa" (1° abril del año elegido al 31 marzo del siguiente).
...
function _labelPeriodoVentas() {
  return ventasModoVista === 'gestion' ? ('Gestión ' + _labelGestion(ventasFilGestion)) : (ventasFilMes + ' ' + ventasFilAnio);
}
