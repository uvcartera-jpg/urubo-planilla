...
let ventasFilMes  = new Date().toLocaleString('es-BO',{month:'long'});
ventasFilMes = ventasFilMes.charAt(0).toUpperCase() + ventasFilMes.slice(1);
...
let expFilMes  = new Date().toLocaleString('es-BO',{month:'long'});
expFilMes = expFilMes.charAt(0).toUpperCase() + expFilMes.slice(1);
...
async function renderDashboardExpensas() {
  ...
  /* ── Gráfico 4: Candlestick + Volumen (últimos 20 días con cobros) — un solo panel,
     estilo TradingView: las velas arriba sin fechas (para no repetirlas), el volumen
     abajo pegado, con las fechas ahí — así se leen como una sola pieza, no dos gráficos sueltos. ── */
  ...
  dbeCharts.velas = new ApexCharts(document.querySelector('#dbe-chart-velas'), {
    chart: { type:'candlestick', height:200, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8', parentHeightOffset:0 },
    series: [{ data: velas20.map(d=>({ x:d.x, y:[d.o,d.h,d.l,d.c] })) }],
    xaxis: { type:'datetime', labels:{show:false}, axisTicks:{show:false}, axisBorder:{show:false} },
    yaxis: { labels: { formatter: v => fmtBsK(Math.round(v)) } },
    plotOptions: { candlestick: { colors: { upward:'#10b981', downward:'#ef4444' }, wick:{useFillColor:true} } },
    grid: { borderColor:'#334155', padding:{bottom:0} },
    tooltip: { theme:'dark', x:{format:'dd MMM'} },
  });
  dbeCharts.velas.render();

  if (dbeCharts.volumen) dbeCharts.volumen.destroy();
  dbeCharts.volumen = new ApexCharts(document.querySelector('#dbe-chart-volumen'), {
    chart: { type:'bar', height:100, toolbar:{show:false}, background:'transparent', foreColor:'#94a3b8', parentHeightOffset:0 },
    series: [{ name:'Cobros del día', data: velas20.map(d=>({x:d.x, y:d.v})) }],
    xaxis: { type:'datetime', labels:{style:{fontSize:'9px'}}, tooltip:{enabled:false} },
    yaxis: { labels: { formatter: v => Math.round(v) }, title:{ text:'N° cobros', style:{fontSize:'9px'} } },
    colors: ['#475569'],
    plotOptions: { bar: { columnWidth:'60%' } },
    grid: { borderColor:'#334155', padding:{top:0} },
    dataLabels: { enabled:false },
    tooltip: { theme:'dark', x:{format:'dd MMM'} },
  });
  dbeCharts.volumen.render();
...
async function guardarMetaExpensasCategorias() {
  ...
  renderCobranzaExpensasPorModalidad();
}
...
let carteraFilMes  = new Date().toLocaleString('es-BO',{month:'long'});
carteraFilMes = carteraFilMes.charAt(0).toUpperCase() + carteraFilMes.slice(1);
