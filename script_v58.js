...
function renderDetalleCierres() {
  ...
  // Poblar el filtro de año dinámicamente según lo que haya en los datos
  const selAnio = document.getElementById('fil-cierres-anio');
  if (selAnio && !selAnio.dataset.filled) {
    const fechaDe = r => r.ts ? new Date(r.ts).getFullYear() : null;
    const anios = [...new Set(cierres.map(fechaDe).filter(Boolean))].sort((a,b)=>b-a);
    if (anios.length) {
  ...
  const filAnio = selAnio?.value || '';
  const filMes  = document.getElementById('fil-cierres-mes')?.value || '';
  if (filAnio) cierres = cierres.filter(r => r.ts && new Date(r.ts).getFullYear() === parseInt(filAnio));
  if (filMes)  cierres = cierres.filter(r => r.mes === filMes);

  cierres = cierres.sort((a,b)=>(b.ts||0)-(a.ts||0));

  if (!cierres.length) {
    tbody.innerHTML = '<tr><td colspan="13"><div class="empty-state">Sin cierres para este período.</div></td></tr>';
...
async function renderCobranzaCarteraKPIs() {
  const hoy = new Date();
  const anioActualN = hoy.getFullYear();
  const mesActualN = hoy.getMonth();
  ...
  orangeCarteraDetalleRaw.forEach(r => {
    ...
    if (f.getFullYear() === anioActualN) {
      cobradoGestion += r.monto;
      if (f.getMonth() === mesActualN) cobradoMes += r.monto;
    }
    if (r.fechaOrden >= inicioSemana.getTime() && r.fechaOrden < finSemana.getTime()) cobradoSemana += r.monto;
  });

  setText('orange-cart-anio-actual', anioActualN);
  ...
  const metaC = await window._fbGetMetaOnce(anioActualN, mesActual(), 'cartera');
...
/* ── Cobrado en Terreno vs Cobrado en Acción — este mes ── */
function renderCobranzaPorTerrenoAccion() {
  ...
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (f.getFullYear()===hoy.getFullYear() && f.getMonth()===hoy.getMonth()) {
...
/* ── Cobrado por Modalidad — este mes ── */
function renderCobranzaPorModalidad() {
  ...
  if (!entries.length) { cont.innerHTML = '<div class="empty-state">Sin cobros este mes todavía.</div>'; return; }
...
function renderIngresosVentaNuevaVsCartera() {
  ...
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (_gestionDeFecha(f) !== gestionActualN) return; // solo cobros DENTRO de la gestión en curso
    if (r.tipoIngreso === 'Venta Nueva') ventaNueva += r.monto; else cartera += r.monto;
  });
  const total = ventaNueva + cartera;
  const pctNueva = total ? Math.round(ventaNueva/total*100) : 0;

  setText('orange-cart-gestion-label', _labelGestion(gestionActualN));
