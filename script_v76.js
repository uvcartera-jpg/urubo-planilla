...
async function cargarDetalleCobrosCartera(silencioso) {
  ...
  try {
    const [respCart, respVentas] = await Promise.all([
...
function renderDetalleCobrosCartera() {
  ...
  tbody.innerHTML = datos.slice(0,500).map(r => `<tr>
    ...
    <td>${esc(r.cliente)}</td>
    <td style="font-family:monospace;font-size:var(--fs-sm);">${esc(r.terreno)}</td>
    <td><span style="padding:2px 9px;border-radius:999px;font-size:var(--fs-xs);font-weight:600;${r.tipoPago==='Terreno'?'background:var(--ok-bg);color:var(--ok-ink);':'background:var(--info-bg,#eef4fb);color:var(--info);'}">${r.tipoPago}</span></td>
