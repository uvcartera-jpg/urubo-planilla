...
function renderListaLotesPicker() {
  ...
  cont.innerHTML = datos.map(l => {
    ...
    const bloqueado = l.estado === 'Reservado' || l.estado === 'Vendido' || l.estado === 'Bloqueado' || l.estado === 'No disponible';
    ...
    else if (l.estado === 'Bloqueado') infoExtra = `<span style="color:var(--accent);font-weight:600;">🚫 Bloqueado</span>`;
  ...
  try {
    ...
    orangeCarteraDetalleRaw = json.data.map(r => {
      ...
      return {
        ...
        tipoPago: esTerreno ? 'Terreno' : 'Acción', monto,
      };
