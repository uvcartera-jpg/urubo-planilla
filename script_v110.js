...
async function categorizarAutomaticamenteExpensas() {
  ...
  const cantidad = Object.keys(paraGuardar).length;
  if (!cantidad) return;
  try {
    await window._fbSetCategoriasExpensaEnLote(paraGuardar);
    toastOk(`${cantidad} cobro${cantidad!==1?'s':''} categorizado${cantidad!==1?'s':''} automáticamente (mes de cuota vs. mes de pago).`);
  } catch (e) {
    console.error('[CATEGORIZACION-AUTO] Error al guardar en lote:', e);
  }
}
...
async function cargarDetalleExpensas(silencioso) {
  ...
  try {
    ...
    await categorizarAutomaticamenteExpensas();
