...
const TAB_TITLES = {
  ...
  'tabla':'Registros', 'inventario':'Inventario', 'eliminaciones':'Eliminaciones',
...
function renderVisitasProgramadasCallCenter() {
  ...
  if (!mias.length) { cont.innerHTML = '<div class="empty-state">Todavía no agendaste ninguna visita.</div>'; return; }
  cont.innerHTML = mias.map(v => `
    <div class="cobro-card">
      <div class="cobro-card-left">
        <div class="cobro-nombre">${esc(v.nombre)} ${v.estado==='confirmada'?'<span class="badge badge-contrato" style="font-size:var(--fs-xs);">✓ Confirmada</span>':'<span class="badge badge-seguim" style="font-size:var(--fs-xs);">⏳ Pendiente</span>'}</div>
...
function renderAnalisisOrigenModalidad() {
  ...
  function pintarTabla(id, filas) {
    ...
    if (!tbody) return;
    if (!filas.length) { tbody.innerHTML = '<tr><td colspan="4"><div class="empty-state">Sin datos.</div></td></tr>'; return; }
    ...
    tbody.innerHTML = filas.map(([k,d], i) => {
      ...
      return `<tr>
        <td style="font-weight:600;">${esc(k)} ${esTop?'<span style="color:var(--ok);font-size:var(--fs-xs);">🏆 top ingresos</span>':''}</td>
        <td>${d.n}</td>
...
function renderDetalleCierres() {
  ...
  tbody.innerHTML = filasCalc.map(({r, metraje, m2Lista, m2Cierre}) => `<tr${r.esExterno?' style="background:#fdf6e8;"':''}>
      <td style="font-weight:600;">${esc(r.nombre||'—')} ${r.esExterno?'<span class="badge badge-def" style="font-size:var(--fs-xs);background:#fef3c7;color:#92400e;">🤝 Externo</span>':''}</td>
      <td style="font-family:monospace;font-weight:600;">${esc(r.terrenoInteres||'—')}</td>
      <td>${r.precioLista ? '$'+Math.round(r.precioLista).toLocaleString('es-BO') : '—'}</td>
      <td>${metraje ? Number(metraje).toLocaleString('es-BO')+' m²' : '<span style="color:var(--gris);">falta cargar</span>'}</td>
      <td>${m2Lista ? '$'+m2Lista.toLocaleString('es-BO',{maximumFractionDigits:2}) : '—'}</td>
      <td style="color:${r.descuentoUsd>0?'var(--danger)':'var(--ok)'};font-weight:600;">
        ${r.descuentoUsd ? '$'+Math.round(r.descuentoUsd).toLocaleString('es-BO')+' ('+r.descuentoPct+'%)' : '—'}
      </td>
      <td style="font-weight:700;color:var(--cartera);">$${Math.round(r.precioVenta).toLocaleString('es-BO')}</td>
      <td>${m2Cierre ? '$'+m2Cierre.toLocaleString('es-BO',{maximumFractionDigits:2}) : '—'}</td>
      <td><span class="badge badge-def" style="font-size:var(--fs-xs);">${esc(r.formaPago||'—')}</span></td>
  ...
  if (tfoot) {
    ...
    const fmt  = (v) => '$'+Math.round(v).toLocaleString('es-BO');
    const fmt2 = (v) => '$'+v.toLocaleString('es-BO',{maximumFractionDigits:2});

    tfoot.innerHTML = `
      <tr style="background:var(--bg-soft);font-weight:700;">
        <td colspan="2">TOTAL (${n} cierres)</td>
        <td>${fmt(sumLista)}</td>
        <td>${metrajes.length ? sumMetraje.toLocaleString('es-BO')+' m²' : '—'}</td>
        <td>—</td>
        <td style="color:var(--danger);">${fmt(sumDesc)} (${promPct.toFixed(1)}% prom.)</td>
        <td style="color:var(--cartera);">${fmt(sumVenta)}</td>
        <td colspan="4"></td>
      </tr>
      <tr style="background:var(--bg-soft);font-weight:600;color:var(--ink-700);">
        <td colspan="2">PROMEDIO por cierre</td>
        <td>${fmt(sumLista/n)}</td>
        <td>${metrajes.length ? promMetraje.toLocaleString('es-BO',{maximumFractionDigits:0})+' m²' : '—'}</td>
        <td>${m2ListaVals.length ? fmt2(promM2Lista) : '—'}</td>
        <td style="color:var(--danger);">${fmt(sumDesc/n)}</td>
        <td style="color:var(--cartera);">${fmt(sumVenta/n)}</td>
        <td>${m2CierreVals.length ? fmt2(promM2Cierre) : '—'}</td>
        <td colspan="3"></td>
      </tr>`;
  }
}
...
function renderDashboard() {
  ...
  if (visitasEl) {
    const maxV = Math.max(...Object.values(origenCounts),1);
    const colV = {'Pauta Virtual':'var(--info)','Referido Cliente':'var(--ok)','Referido Empresa':'var(--cartera)','Redes Sociales':'var(--accent)','Feria':'var(--warn)','Puerta Fría':'var(--danger)'};
    visitasEl.innerHTML = Object.entries(origenCounts).sort((a,b)=>b[1]-a[1]).map(([k,n])=>`
      <div class="bar-item">
  ...
  document.getElementById('chart-asesores').innerHTML =
    Object.entries(asesorCounts).sort((a,b)=>b[1]-a[1]).map(([k,n],i)=>`
      <div class="bar-item" style="cursor:pointer;" onclick="abrirDetalleAsesor('${escJs(k)}')" title="Ver detalle de ${esc(k)}">
        <div class="bar-label"><span>${k} 🔍</span><span>${n}</span></div>
        <div class="bar-track">
...
function renderHomeChartCarteraMora(cartaAdmin) {
  const cont = document.getElementById('h-chart-cartera-mora');
  if (!cont) return;
  const conteo = {};
  ...
  cont.innerHTML = entries.length ? entries.map(([k,n]) => `
    <div class="bar-item">
      <div class="bar-label"><span>${esc(k)}</span><span><strong>${n}</strong></span></div>
      <div class="bar-track"><div class="bar-fill" style="width:${n/max*100}%;background:${ESTADOS_COLOR[k]||'#6b7280'};"></div></div>
    </div>`).join('') : '<div class="empty-state">Sin datos este mes.</div>';
}

function renderHomeChartExpensasEstado(expMes) {
  const cont = document.getElementById('h-chart-exp-estado');
  if (!cont) return;
  const conteo = {};
