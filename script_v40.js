...
async function guardarContratoFirmado() {
  ...
  try {
    await window._fbPushVentaConcretada({
      ...
      dsctoAdicional, precioVenta: r.precioVenta||0,
...
function renderVentasConcretadasAdmin() {
  ...
  // Filtros dinámicos
  const selAnio = document.getElementById('vc-fil-anio');
  if (selAnio && !selAnio.dataset.filled) {
    const anios = [...new Set(aprobadas.map(v=>v.año).filter(Boolean))].sort((a,b)=>b-a);
    selAnio.innerHTML = '<option value="">Todos los años</option>' + anios.map(a=>`<option value="${a}">${a}</option>`).join('');
    if (anios.length) selAnio.dataset.filled = '1';
  }
  const selCanal = document.getElementById('vc-fil-canal');
  if (selCanal && !selCanal.dataset.filled) {
    const canales = [...new Set(aprobadas.map(v=>v.canal||v.gestion).filter(Boolean))].sort();
    selCanal.innerHTML = '<option value="">Todos los canales</option>' + canales.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');
    if (canales.length) selCanal.dataset.filled = '1';
  }

  // Tabla
  const filAnio = selAnio?.value || '';
  const filCanal = selCanal?.value || '';
  const term = (document.getElementById('vc-buscar')?.value||'').trim().toLowerCase();
  let filtradas = aprobadas;
  if (filAnio) filtradas = filtradas.filter(v => String(v.año)===filAnio);
  if (filCanal) filtradas = filtradas.filter(v => (v.canal||v.gestion)===filCanal);
  ...
  if (tbody) {
    ...
    </tr>`).join('') : '<tr><td colspan="15"><div class="empty-state">Sin resultados.</div></td></tr>';
  }
}
