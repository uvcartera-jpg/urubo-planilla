...
function procesarExcelExpensas(input) {
  ...
  reader.onload = e => {
    ...
    if (headerRow >= 0) {
      ...
      parsed.forEach(r => {
        r.estado = calcularEstadoExpensa(periodosPorLote[r.terreno]);
        // Categoría automática según el estado (vos ya creaste "Mora Expensas" / "Vigente Expensas" / "Pagos Adelantados")
...
async function renderTendenciaExpensas() {
  ...
  expAdminData.forEach(e => {
    ...
    (buckets[key] ||= {label, orden, monto:0, anio:e.anio, mesNum}).monto += (e.monto||0);
  });
  ...
  for (const b of ordenados) {
    if (gran === 'anio') {
      ...
      if (!(ck in metaCache)) metaCache[ck] = await window._fbGetMetaOnce(b.anio, MESES_ES_NOMBRE[b.mesNum], 'expensas');
      metaPorBucket.push(metaCache[ck]);
    }
  }
      ...
      <div class="admin-section" style="margin-bottom:10px;padding:16px 18px;">
        ...
        <div id="exp-det-${idx}" style="display:none;margin-top:12px;">
          <div class="tabla-wrap">
            <table><thead><tr><th>Cuota pagada</th><th>Fecha de cobro</th><th>Monto</th><th>Tipo</th><th>Categoría</th></tr></thead>
            <tbody>
              ${filas.map(r=>`<tr>
