...
function renderDashboard() {
  ...
  const totalVisitas  = vMes.length; // "concretadas" = ya vinieron al proyecto
  const conCierre     = vMes.filter(r => r.conclusion === 'Reserva' || r.huboCierre === 'SI').length;
  const leadsReal      = (typeof leadsData !== 'undefined' ? leadsData : []).filter(l => l.mes===mes && l.anio===anioC).length;
...
function renderCampanias() {
  ...
  ['mkt-lead-campania','cc-campania'].forEach(id => {
