...
function alternarTema() {
  ...
  const nuevoOscuro = !esOscuroAhora;
  if (nuevoOscuro) document.documentElement.setAttribute('data-theme', 'dark');
  else document.documentElement.removeAttribute('data-theme');
  try { localStorage.setItem('uv-tema', nuevoOscuro ? 'dark' : 'light'); } catch(e) {}
...
function _actualizarBotonTema() {
  ...
  const icon2 = document.getElementById('theme-toggle-icon-2');
  if (icon)  icon.textContent  = oscuro ? '☀️' : '🌙';
  if (label) label.textContent = oscuro ? 'Modo claro' : 'Modo oscuro';
  if (icon2) icon2.textContent = oscuro ? '☀️' : '🌙';
}
...
let pinAdminLocal = null; // se carga desde Firebase apenas arranca la app (ver fb-ready) — ya no depende del dispositivo
let pinBuf = '';
...
document.addEventListener('fb-ready', () => {
  // Si la URL trae ?cotizacion=KEY, es un cliente viendo su cotización por link — mostramos
  // esa vista pública sola, sin login, y cortamos acá (no seguimos con el flujo normal).
  const cotizKey = new URLSearchParams(window.location.search).get('cotizacion');
  if (cotizKey) { mostrarCotizacionPublica(cotizKey); return; }
  ...
  // PIN de admin: se carga UNA vez de Firebase, así es el mismo en cualquier computadora.
  // Si Firebase todavía no tiene ninguno cargado, migramos el que esta máquina tenía
  // guardado localmente (si tenía) — para no invalidar de golpe el PIN que ya usaban.
  window._fbGetAdminPinOnce().then(pinFirebase => {
    if (pinFirebase) {
      pinAdminLocal = pinFirebase;
    } else {
      const pinLocalViejo = localStorage.getItem('uv_planilla_pin') || '0000';
      pinAdminLocal = pinLocalViejo;
      window._fbSetAdminPin(pinLocalViejo);
    }
  });
});
...
function verificarPin() {
  ...
  setTimeout(() => {
    let ok = false;
    if (modoAdmin) {
      if (pinAdminLocal === null) {
        toastErr('Todavía está cargando — esperá un segundo y probá de nuevo.');
...
function entrarAsesor() {
  ...
  // Gerencia → admin panel completo, sin restricciones
  if (rol === 'gerencia') {
    entrarAdmin();
    return;
  }

  // Estos 4 módulos solo existen en el panel admin (no hay una versión "simple" de
  // Cartera/Inventario/Dashboard/Reportes para el panel de vendedores). Si la persona
  // tiene AUNQUE SEA UNO de estos, va al admin — pero restringido a TODOS los módulos
  // que tenga, no solo al primero que matchee (ese era el bug: Cartera + Expensas
  // juntos, por ejemplo, antes solo dejaba ver Cartera y la Expensas se perdía).
  const modulosSoloAdmin = ['cobranza', 'inventario', 'dashboard', 'reportes'];
  const necesitaAdmin = modulos.some(m => modulosSoloAdmin.includes(m));
  if (necesitaAdmin) {
    entrarAdmin();
    setTimeout(() => {
      _restrictAdminPanelForRole(modulos);
      // Arranca en el primer módulo relevante que tenga, con esta prioridad
      if (modulos.includes('cobranza'))        switchAdminTab('cartera');
      else if (modulos.includes('expensas'))   switchAdminTab('expensas');
      else if (modulos.includes('inventario')) switchAdminTab('inventario');
      else if (modulos.includes('dashboard'))  switchAdminTab('dashboard');
    }, 200);
    return;
  }

  // Ventas / Call Center / Marketing / Expensas (sin ningún módulo "solo admin") →
  // panel simple de usuario, que ya soporta cualquier combinación de estos a la vez.
  entrarUsuario();
}

// Restricción de sidebar según rol del usuario
function _restrictAdminPanelForRole(modulos) {
  const a = asesorActual;
  if (!a) return;
  modulos = Array.isArray(modulos) ? modulos : (a.modulos || []);
  ...
  if (a.rol === 'gerencia') return; // gerencia ve todo el panel, sin restricción

  // Qué ítems del menú habilita CADA módulo — antes esto solo miraba UN rol y
  // ocultaba todo lo demás; ahora se suman los ítems de TODOS los módulos que
  // tenga la persona, así "Cartera + Expensas" muestra las dos, no solo una.
  const mapaModuloNav = {
    dashboard:  ['nav-dashboard'],
    marketing:  ['nav-marketing-admin'],
    cobranza:   ['nav-cartera','nav-materia-reserva','nav-cotizaciones'],
    expensas:   ['nav-expensas'],
    inventario: ['nav-inventario'],
    ventas:     ['nav-tabla','nav-contratos'],
  };
  const idsPermitidos = new Set(['nav-home']);
  modulos.forEach(m => (mapaModuloNav[m]||[]).forEach(id => idsPermitidos.add(id)));

  document.querySelectorAll('.nav-item').forEach(el => {
    el.style.display = idsPermitidos.has(el.id) ? '' : 'none';
  });
  ...
  setTimeout(() => {
    document.querySelectorAll('.nav-acc-body').forEach(body => {
      ...
      if (head?.classList.contains('nav-acc-head')) head.style.display = visibles ? '' : 'none';
      body.style.display = visibles ? '' : 'none';
    });
  }, 50);
}
...
function entrarAdmin() {
  ...
  window._fbListenOrangeCarteraSyncInfo(info => renderOrangeCarteraSyncInfo(info));
  window._fbListenOrangeCarteraAgregados(agr => { orangeCarteraAgregados = agr; renderOrangeCarteraAgregados(); });
  window._fbListenCategoriasCobros(cats => { carteraCategoriasData = cats; renderCategorizacionCobros(); renderResumenMensualCartera(); renderSemanalCartera(); });
  window._fbListenDocumentosReserva(arr => { documentosReservaData = arr; renderDocumentosGuardados(); });
  window._fbListenCotizaciones(arr => { cotizacionesData = arr; renderCotizacionesGuardadas(); });
  ...
  window._fbListenPlazosModalidad(data => { plazosModalidadData = data; if (document.getElementById('pc-plazos-tbody')) renderPlazosModalidad(); });
...
function switchAdminTab(id) {
  ...
  if (id === 'panelcontrol')    { renderPanelControl(); renderPlazosModalidad(); }
  if (id === 'materia-reserva') cambiarModoMateriaReserva('nuevo');
  if (id === 'cotizaciones') cambiarModoCotizaciones('nueva');
  if (id === 'cartera') {
    ...
    if (!orangeCarteraDetalleRaw.length) { cargarDetalleCobrosCartera(true); sincronizarOrangeCartera(); } // primera carga, silenciosa
    iniciarAutoSyncCarteraOrange();
  }
  if (id === 'expensas') {
    if (!orangeExpensasDetalleRaw.length) { cargarDetalleExpensas(true); sincronizarOrangeExpensas(); }
    iniciarAutoSyncExpensasOrange();
  }
}
...
// La clave ahora incluye el período de vigencia (año-mes) — así "Agosto" puede tener
// una política distinta de "Septiembre", y las ventas viejas usan la que regía en su momento.
function _politicaKey(modalidad, categoria, anio, mes) {
  const periodo = (anio && mes) ? `${anio}-${String(PC_MESES.indexOf(mes)+1).padStart(2,'0')}` : '';
  return String(modalidad||'—').replace(/[^a-zA-Z0-9]+/g,'-') + '_' + String(categoria||'—').replace(/[^a-zA-Z0-9]+/g,'-') + (periodo?'_'+periodo:'');
}
// "Contado" se mide en DÍAS (completar el pago en ese plazo, en los pagos que hagan falta).
// El resto se mide en CUOTAS (mensuales). Si no hay nada cargado, avisamos en vez de inventar un número.
function _unidadPlazoModalidad(modalidad) {
  return modalidad === 'Contado' ? 'días' : 'cuotas';
}
function obtenerPlazoModalidad(modalidad) {
  const key = String(modalidad||'').replace(/[^a-zA-Z0-9]+/g,'-');
  const d = plazosModalidadData[key];
  return d ? { plazoMax: d.plazoMax, unidad: _unidadPlazoModalidad(modalidad) } : null;
}

function renderPlazosModalidad() {
  const tbody = document.getElementById('pc-plazos-tbody');
  if (!tbody) return;
  tbody.innerHTML = PC_MODALIDADES.map(modalidad => {
    const key = modalidad.replace(/[^a-zA-Z0-9]+/g,'-');
    const d = plazosModalidadData[key];
    const unidad = _unidadPlazoModalidad(modalidad);
    return `<tr>
      <td style="font-weight:600;">${esc(modalidad)}</td>
      <td><input type="number" min="1" data-plazo-modalidad="${esc(modalidad)}" value="${d?d.plazoMax:''}" placeholder="—" style="width:100px;text-align:center;"/></td>
      <td style="color:var(--gris);">${unidad}</td>
    </tr>`;
  }).join('');
}

async function guardarPlazosModalidad() {
  const inputs = document.querySelectorAll('#pc-plazos-tbody input[data-plazo-modalidad]');
  let n = 0;
  for (const inp of inputs) {
    const val = parseFloat(inp.value);
    if (isNaN(val) || val <= 0) continue;
    const modalidad = inp.dataset.plazoModalidad;
    await window._fbSetPlazoModalidad(modalidad, { modalidad, plazoMax: val, unidad: _unidadPlazoModalidad(modalidad) });
    n++;
  }
  toastOk('Plazos guardados (' + n + ' modalidades).');
}

function obtenerPoliticaDescuento(modalidad, categoria, anio, mes) {
  const key = _politicaKey(modalidad, categoria, anio, mes);
  let p = politicaDescuentoData.find(p => p._key === key);
  if (p) return p;
  // Si no hay política exacta para ese mes, usamos la vigente más reciente ANTES de esa fecha
  // (una política "queda puesta" hasta que se cargue una nueva, no hace falta repetirla cada mes).
  if (anio && mes) {
    const objetivoOrden = anio*100 + (PC_MESES.indexOf(mes)+1);
    const candidatas = politicaDescuentoData
      .filter(p => p.modalidad===modalidad && p.categoria===categoria && p.anio && p.mes)
      .map(p => ({ ...p, orden: p.anio*100 + (PC_MESES.indexOf(p.mes)+1) }))
      .filter(p => p.orden <= objetivoOrden)
...
// mes/anio: el período de la VENTA (no el de hoy) — así cada venta usa la política que regía en su momento.
function calcularExcedentePolitica(modalidad, categoria, descuentoPctOtorgado, precioLista, mes, anio) {
  const politica = obtenerPoliticaDescuento(modalidad, categoria, anio, mes);
...
let pcPeriodoActivo = { anio: new Date().getFullYear(), mes: PC_MESES[new Date().getMonth()] };

function pcCambiarPeriodo() {
  pcPeriodoActivo.anio = parseInt(document.getElementById('pc-fil-anio').value);
  pcPeriodoActivo.mes  = document.getElementById('pc-fil-mes').value;
  renderPanelControl();
}
...
function renderPanelControl() {
  ...
  const selAnio = document.getElementById('pc-fil-anio');
  const selMes  = document.getElementById('pc-fil-mes');
  if (selAnio && !selAnio.dataset.filled) {
    const anioActualN = new Date().getFullYear();
    selAnio.innerHTML = [anioActualN+1, anioActualN, anioActualN-1].map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.value = pcPeriodoActivo.anio;
    selAnio.dataset.filled = '1';
  }
  if (selMes && !selMes.dataset.filled) {
    selMes.innerHTML = PC_MESES.map(m=>`<option value="${esc(m)}">${esc(m)}</option>`).join('');
    selMes.value = pcPeriodoActivo.mes;
    selMes.dataset.filled = '1';
  }

  const { anio, mes } = pcPeriodoActivo;
  tbody.innerHTML = PC_MODALIDADES.map(modalidad => {
    const celdas = PC_CATEGORIAS.map(cat => {
      const key = _politicaKey(modalidad, cat, anio, mes);
      const exacta = politicaDescuentoData.find(p => p._key === key);
      const heredada = !exacta ? obtenerPoliticaDescuento(modalidad, cat, anio, mes) : null;
      const valor = exacta ? exacta.pctMax : (heredada ? heredada.pctMax : '');
      const nota = heredada ? `<div style="font-size:9px;color:var(--gris);margin-top:2px;">heredada de ${esc(heredada.mes)} ${heredada.anio}</div>` : '';
      return `<td><input type="number" min="0" max="100" step="0.5" data-pc-key="${key}" data-modalidad="${esc(modalidad)}" data-categoria="${cat}"
        value="${valor}" placeholder="0" style="width:80px;text-align:center;"/> %${nota}</td>`;
    }).join('');
...
async function guardarPoliticaDescuento() {
  ...
  for (const inp of inputs) {
    ...
    await window._fbSetPoliticaDescuento(inp.dataset.pcKey, {
      modalidad: inp.dataset.modalidad, categoria: inp.dataset.categoria, pctMax: val, anio, mes,
    });
    n++;
  }
  toastOk('Política de ' + mes + ' ' + anio + ' guardada (' + n + ' combinaciones).');
}
...
function abrirModalContratoFirmado(registroKey) {
  ...
  const buscadoCf = String(r.terrenoInteres||'').trim().toLowerCase();
  const loteCf = todosLotes.find(l => String(_terrenoDeLote(l)).trim().toLowerCase() === buscadoCf);
  document.getElementById('cf-categoria').value = ['A','B','C'].includes(loteCf?.categoria) ? loteCf.categoria : (['A','B','C'].includes(r.calificacion) ? r.calificacion : 'B');
...
async function guardarContratoFirmado() {
  ...
  try {
    await window._fbPushVentaConcretada({
      ...
      dsctoAdicional, excedente: calcularExcedentePolitica(r.formaPago, categoria, r.descuentoPct||0, r.precioLista||0, meses[fechaRef.getMonth()], fechaRef.getFullYear()) - dsctoAdicional,
...
function renderVisitasProgramadasCallCenter() {
  ...
  if (cont) {
    ...
    if (!mias.length) { cont.innerHTML = '<div class="empty-state">Todavía no agendaste ninguna visita.</div>'; }
      ...
      const badgeEstado = v => {
        if (v.estado === 'completada') return '<span class="badge badge-contrato" style="font-size:var(--fs-xs);">✅ Completada — cargó el registro</span>';
        if (v.estado === 'en_proceso') return '<span class="badge" style="font-size:var(--fs-xs);background:#fde68a;color:#92400e;">⚠️ Abrió el formulario, no guardó todavía</span>';
        return '<span class="badge badge-seguim" style="font-size:var(--fs-xs);">⏳ Pendiente — todavía no la tocó</span>';
      };
      cont.innerHTML = mias.map(v => `
        <div class="cobro-card">
          <div class="cobro-card-left">
            <div class="cobro-nombre">${esc(v.nombre)} ${badgeEstado(v)} ${v.esDevolucion?'<span class="badge" style="font-size:var(--fs-xs);background:#fef3c7;color:#92400e;">↩️ Devuelta</span>':''}</div>
...
function renderCallCenterKPIs() {
  ...
  const confirmadas = visitasProgramadasData.filter(v => v.estado === 'completada').length;
  const devueltas = visitasProgramadasData.filter(v => v.esDevolucion);
  const devueltasConfirmadas = devueltas.filter(v => v.estado === 'completada').length;
...
function renderVisitasProgramadasAsesor() {
  ...
  cont.innerHTML = misVisitasProgramadas.map(v => {
    const enProceso = v.estado === 'en_proceso';
    return `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#fff;border-radius:var(--r-sm);padding:10px 12px;margin-bottom:6px;${enProceso?'border:1.5px solid var(--warn);':''}">
      <div>
        <div style="font-weight:600;">${esc(v.nombre)} ${enProceso?'<span style="font-size:var(--fs-xs);font-weight:700;color:var(--warn);background:var(--warn-bg);padding:2px 8px;border-radius:999px;margin-left:6px;">⚠️ Te quedó a medias — falta guardar</span>':''}</div>
      ...
      <button onclick="confirmarVisitaProgramada('${v._key}')" style="padding:8px 14px;background:${enProceso?'var(--warn)':'var(--info)'};color:#fff;border:none;border-radius:var(--r-sm);font-size:var(--fs-sm);font-weight:600;cursor:pointer;white-space:nowrap;">${enProceso?'↻ Retomar y guardar':'✓ Confirmar visita'}</button>
    </div>`;
  }).join('');
}
...
function confirmarVisitaProgramada(key) {
  ...
  // OJO: acá NO se marca "completada" — recién se marca así cuando el registro se GUARDA
  // de verdad (ver guardarVisita). Si se marcara acá, y el asesor no llega a guardar,
  // la asignación de Call Center desaparece sin que nadie se entere — era justo el bug.
  window._fbUpdateVisitaProgramada(key, { estado: 'en_proceso' });
  toastOk('Visita cargada en el formulario — completá el detalle y guardá para terminar.');
...
function renderListaLotesPicker() {
  ...
  const estColor = { 'Disponible':'var(--ok)', 'Reservado':'var(--warn)', 'Vendido':'var(--danger)', 'Bloqueado':'var(--accent)', 'No disponible':'#9ca3af' };

  cont.innerHTML = datos.map(l => {
    const terreno = _terrenoDeLote(l);
    // Solo "Vendido" bloquea de verdad — es lo único irreversible. Reservado/Bloqueado/No
    // disponible se pueden seleccionar igual (con advertencia): puede ser una reserva que
    // el asesor ya hizo directo en Orange y necesita registrar acá también.
    const bloqueado = l.estado === 'Vendido';
    ...
    let infoExtra = '';
    if (l.estado === 'Vendido') infoExtra = `<span style="color:var(--danger);font-weight:600;">✕ Vendido</span>`;
    else if (l.estado === 'Reservado') infoExtra = `<span style="color:var(--warn);font-weight:600;">⚠️ Figura Reservado — si ya lo reservaste vos en Orange, podés seleccionarlo igual</span>`;
    else if (l.estado === 'No disponible') infoExtra = `<span style="color:#9ca3af;">⚠️ No disponible — verificá antes de ofrecerlo</span>`;
    else if (l.estado === 'Bloqueado') infoExtra = `<span style="color:var(--accent);font-weight:600;">⚠️ Marcado "Bloqueado" en Orange — verificá antes de ofrecerlo</span>`;
    ...
    // Precio de lista: se oculta solo cuando está Vendido (única situación realmente cerrada)
...
function pintarEstadoLoteSeleccionado(l) {
  ...
  if (l.estado === 'Vendido') {
    info.innerHTML = `<span style="color:var(--danger);font-weight:600;">Este lote ya está Vendido — no se puede volver a ofrecer.</span>`;
    acciones.innerHTML = '';
    return;
  }
  if (l.estado === 'Reservado') {
    info.innerHTML = `<span style="color:var(--warn);font-weight:600;">⚠️ Este lote figura "Reservado" en el sistema. Si ya lo reservaste vos directo en Orange, seguí igual para registrar la visita/reserva acá.</span><br>${precioTxt}`;
...
async function guardarVisita() {
  ...
  try {
    if (editKey) {
      ...
      const nuevoRef = await window._fbPushRegistro(asesorActual._key, data);
      // Si esta visita venía de una asignación de Call Center, recién ACÁ se cierra el
      // círculo de verdad — se guardó el registro, así que Call Center ya puede ver que
      // esa asignación se completó (y no se pierde silenciosa si el asesor no llega a guardar).
      const claveVisitaProgramada = fNombreEl.dataset.visitaProgramadaKey;
      if (claveVisitaProgramada) {
        await window._fbUpdateVisitaProgramada(claveVisitaProgramada, {
          estado: 'completada', registroKey: nuevoRef.key, completadaTs: Date.now(),
        });
        delete fNombreEl.dataset.visitaProgramadaKey;
      }
    }
...
function limpiarFormVisita() {
  delete document.getElementById('f-nombre')?.dataset.visitaProgramadaKey;
  ...
  cont.innerHTML = misRegs.map(r => {
    ...
    <div class="reg-card">
      ...
      <div style="margin-top:10px;display:flex;justify-content:flex-end;align-items:center;gap:8px;flex-wrap:wrap;">
        ${accionContrato}
        <button onclick="abrirCotizacionDesdeRegistro('${r._key}')" style="padding:6px 14px;background:var(--ok);color:#fff;border:none;border-radius:var(--r-sm);font-size:var(--fs-sm);font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;">💰 Generar cotización</button>
...
function calcularFunnelDatos(mes, anioC) {
  const vMes = soloInternos(todosRegs.filter(r => r.mes === mes && (!anioC || !r.ts || new Date(r.ts).getFullYear() === anioC)));
...
function abrirDetalleAsesor(nombre) {
  ...
  setText('da-titulo', '🧑‍💼 ' + nombre + ' — historial completo');
...
function renderAnalisisOrigenModalidad() {
  const cierres = todosRegs.filter(r => r.conclusion === 'Reserva' && r.precioVenta > 0 && _regEnPeriodoVentas(r));
...
/* ═══════════════════════════════════════════
   SINCRONIZACIÓN DE VENTAS CON ORANGE (GetVentas)
   Cada fila es una venta real (no un cronograma) — se cargan directo como
   Ventas Concretadas ya aprobadas, sin pasar por la cola de autorización
   (esa cola es para lo que cargan los asesores a mano).
═══════════════════════════════════════════ */
function _parseFechaOrange(str) {
  if (!str) return null;
...
async function sincronizarVentasOrange(soloEsteAnio) {
  const btnAnio = document.getElementById('btn-sync-ventas-anio');
  const btnTodo = document.getElementById('btn-sync-ventas-todo');
  const status = document.getElementById('vc-sync-status');
  const setEstado = (txt, disabled) => {
    if (status) status.textContent = txt;
    if (btnAnio) btnAnio.disabled = disabled;
    if (btnTodo) btnTodo.disabled = disabled;
  };
  setEstado('🔄 Conectando con Orange...', true);

  try {
    const resp = await fetch(ORANGE_API_URL + '?op=GetVentas');
    ...
    const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const anioActualN = new Date().getFullYear();
    let ventas = json.data.filter(r => r.NroVenta && r.Cliente);
    if (soloEsteAnio) {
      ventas = ventas.filter(r => {
        const f = _parseFechaOrange(r.FechaFirma) || _parseFechaOrange(r.Fechacreacion);
        return f && f.getFullYear() === anioActualN;
      });
    }
    ...
    for (let i = 0; i < ventas.length; i++) {
      const r = ventas[i];
      setEstado(`🔄 Sincronizando ${i+1}/${ventas.length}...`, true);

      const fecha = _parseFechaOrange(r.FechaFirma) || _parseFechaOrange(r.Fechacreacion) || new Date();
      const mesNombre = meses[fecha.getMonth()];
      const anio = fecha.getFullYear();
      const trimestre = ['1er','1er','1er','2do','2do','2do','3er','3er','3er','4to','4to','4to'][fecha.getMonth()] + ' Trimestre';
      const categoria = ['A','B','C'].includes(r.CategoriaLote) ? r.CategoriaLote : '';
      const descuentoPct = Math.round((r.PorcDescuento||0)*10)/10;

      await window._fbSetVentaConcretadaOrange(r.NroVenta, {
        registroKey: '',
        año: anio, anio,
        mes: mesNombre, trimestre,
        cliente: r.Cliente, terreno: r.Terreno, modulo: r.Modulo || '',
        gestion: r.MedioDifusion || '', canal: r.MedioDifusion || '',
        asesorNombre: r.AsesorComercial || '',
        estadoVenta: r.EstadoContrato || '', estatus: r.Pertenece || '', contrato: r.EstadoContrato || '',
        modalidad: r.Modalidad || '',
        precioLista: r.PrecioLista||0, descuentoUsd: r.Descuento||0, descuentoPct,
        dsctoAdicional: 0,
        excedente: categoria ? calcularExcedentePolitica(r.Modalidad, categoria, descuentoPct, r.PrecioLista||0, mesNombre, anio) : 0,
        precioVenta: r.PrecioVenta||0,
        metraje: r.Metraje||0, m2Usd: r.M2||0,
        fechaReserva: fecha.toISOString().split('T')[0],
        accion: r.PrecioAccion||0, categoria, ciudadOrigen: '',
        esHistorico: true,
        estadoAutorizacion: 'aprobado', // vienen del sistema real de Orange, no de un asesor cargando a mano
        solicitadoPorKey: 'orange-sync', solicitadoPorNombre: 'Sincronizado desde Orange',
      });
      n++;
    }

    setEstado('', false);
    toastOk(`Ventas sincronizadas: ${n} registros desde Orange.`);
  } catch (e) {
    setEstado('', false);
    toastErr('No se pudo sincronizar: ' + e.message);
  }
}

/* ═══════════════════════════════════════════
   DOCUMENTOS DE VENTA — Materia Reservada de Venta (RRHH) + Solicitud de Elaboración de Contrato (Legal)
   Dos flujos independientes. Cada uno trae lo que el asesor ya cargó al marcar Reserva;
   lo que falta se completa a mano en su propia pantalla. tipo: 'mr' (materia reserva) | 'sc' (solicitud contrato)
═══════════════════════════════════════════ */
let docReservaSeleccionada = { mr: null, sc: null };
let docCuotasLineas = { mr: [], sc: [], mrAccion: [] };
let documentosReservaData = [];
let docActualKey = null; // null = estamos armando uno nuevo. Si tiene valor, "Guardar" ACTUALIZA ese documento (no crea otro).

function cambiarModoMateriaReserva(modo) {
  document.getElementById('mr-modo-nuevo').style.display = (modo==='nuevo') ? '' : 'none';
  document.getElementById('mr-modo-guardados').style.display = (modo==='guardados') ? '' : 'none';
  const btnNuevo = document.getElementById('btn-mr-modo-nuevo');
  const btnGuardados = document.getElementById('btn-mr-modo-guardados');
  if (modo === 'guardados') {
    btnNuevo.style.background = 'transparent'; btnNuevo.style.boxShadow = 'none';
    btnGuardados.style.background = '#fff'; btnGuardados.style.boxShadow = '0 1px 3px rgba(0,0,0,.1)';
    renderDocumentosGuardados();
  } else {
    btnGuardados.style.background = 'transparent'; btnGuardados.style.boxShadow = 'none';
    btnNuevo.style.background = '#fff'; btnNuevo.style.boxShadow = '0 1px 3px rgba(0,0,0,.1)';
  }
}

async function guardarDocumentoReserva() {
  const r = docReservaSeleccionada.mr;
  if (!r) { toastErr('Elegí una reserva primero.'); return; }

  const nombreQuienGuarda = asesorActual?.nombre || 'Admin';
  const data = {
    registroKey: r._key, cliente: r.nombre, terreno: r.terrenoInteres||'',
    nroReserva: document.getElementById('mr-nro-reserva').value,
    fecha: document.getElementById('mr-fecha').value,
    celular: document.getElementById('mr-celular').value,
    correo: document.getElementById('mr-correo').value,
    tieneCoprop: document.getElementById('mr-tiene-copropietario').value,
    copropietarioNombre: document.getElementById('mr-copropietario-nombre').value,
    copropietarioCorreo: document.getElementById('mr-copropietario-correo').value,
    copropietarioCelular: document.getElementById('mr-copropietario-celular').value,
    conAccion: document.getElementById('mr-con-accion').value,
    accionPrecio: document.getElementById('mr-accion-precio').value,
    tc: document.getElementById('mr-tc').value,
    politicaPct: document.getElementById('mr-politica-pct').value,
    autorizadoPor: document.getElementById('mr-autorizado-por').value,
    comentario: document.getElementById('mr-comentario').value,
    cuotas: docCuotasLineas.mr,
    cuotasAccion: docCuotasLineas.mrAccion,
    precioVenta: r.precioVenta||0,
  };

  // Si ya estamos editando un documento existente, ACTUALIZAMOS ese mismo — nunca se duplica.
  if (docActualKey) {
    await window._fbUpdateDocumentoReserva(docActualKey, {
      ...data,
      modificadoPorNombre: nombreQuienGuarda,
      modificadoTs: Date.now(),
      version: (documentosReservaData.find(x=>x._key===docActualKey)?.version || 1) + 1,
    });
    toastOk('Documento actualizado (versión ' + ((documentosReservaData.find(x=>x._key===docActualKey)?.version || 1) + 1) + ').');
    return;
  }

  // Es nuevo — pero antes chequeamos que no exista YA un documento para esta misma reserva,
  // para no crear un duplicado por accidente (control documental: un solo maestro por reserva).
  const existente = documentosReservaData.find(x => x.registroKey === r._key);
  if (existente) {
    const confirmar = confirm('Ya existe un documento guardado para esta reserva (N° ' + (existente.nroReserva||'—') + ', guardado por ' + (existente.guardadoPorNombre||'—') + ').\n\n¿Querés ACTUALIZAR ese documento con estos datos en vez de crear uno duplicado?\n\n(Aceptar = actualizar el existente / Cancelar = crear uno nuevo de todas formas)');
    if (confirmar) {
      docActualKey = existente._key;
      await window._fbUpdateDocumentoReserva(docActualKey, {
        ...data, modificadoPorNombre: nombreQuienGuarda, modificadoTs: Date.now(),
        version: (existente.version || 1) + 1,
      });
      toastOk('Documento actualizado (versión ' + ((existente.version || 1) + 1) + ').');
      return;
    }
  }

  const nuevaKey = await window._fbPushDocumentoReserva({
    ...data, version: 1, creadoPorNombre: nombreQuienGuarda, creadoTs: Date.now(),
    guardadoPorNombre: nombreQuienGuarda, // compat con la columna vieja de la tabla
  });
  docActualKey = nuevaKey.key;
  toastOk('Documento guardado.');
}

function renderDocumentosGuardados() {
  const tbody = document.getElementById('mr-guardados-tbody');
  if (!tbody) return;
  const term = (document.getElementById('mr-buscar-guardados')?.value||'').trim().toLowerCase();
  let datos = documentosReservaData;
  if (term) datos = datos.filter(d => (d.cliente||'').toLowerCase().includes(term) || (d.terreno||'').toLowerCase().includes(term));

  if (!datos.length) { tbody.innerHTML = '<tr><td colspan="8"><div class="empty-state">Sin documentos guardados todavía.</div></td></tr>'; return; }

  tbody.innerHTML = datos.map(d => {
    const ultimoToque = d.modificadoTs ? ('Modificado ' + new Date(d.modificadoTs).toLocaleDateString('es-BO') + ' por ' + esc(d.modificadoPorNombre||'—'))
      : ('Creado por ' + esc(d.creadoPorNombre||d.guardadoPorNombre||'—'));
    return `<tr>
    <td>${esc(d.nroReserva||'—')}</td>
    <td>${esc(d.fecha||'—')}</td>
    <td>${esc(d.cliente)}</td>
    <td style="font-family:monospace;">${esc(d.terreno)}</td>
    <td style="font-weight:600;color:var(--cartera);">$${(d.precioVenta||0).toLocaleString('es-BO')}</td>
    <td style="font-size:var(--fs-xs);color:var(--gris);">v${d.version||1} · ${ultimoToque}</td>
    <td>
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);" title="Ver Materia Reservada" onclick="verDocumentoGuardado('${d._key}','materia')">👁 Materia</button>
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);" title="Ver Solicitud de Contrato" onclick="verDocumentoGuardado('${d._key}','contrato')">👁 Contrato</button>
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);" onclick="abrirDocumentoGuardado('${d._key}')">✏️ Editar</button>
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);color:var(--danger);" onclick="eliminarDocumentoGuardado('${d._key}')">🗑</button>
    </td>
  </tr>`;
  }).join('');
}

async function eliminarDocumentoGuardado(key) {
  const d = documentosReservaData.find(x => x._key === key);
  if (!d) return;
  if (!confirm('¿Eliminar el documento de "' + d.cliente + '" (terreno ' + d.terreno + ')? Esto no se puede deshacer.')) return;
  await window._fbRemoveDocumentoReserva(key);
  if (docActualKey === key) docActualKey = null;
  toastOk('Documento eliminado.');
}

// Restaura en memoria y en los campos del formulario (aunque esté oculto) todo lo de un
// documento guardado — lo usan tanto "Editar" como "Ver" (la diferencia es qué hacen después).
function _restaurarDatosDocGuardado(d) {
  const r = todosRegs.find(x => x._key === d.registroKey);
  if (!r) { toastErr('El registro original ya no existe.'); return null; }

  docReservaSeleccionada.mr = r;
  docCuotasLineas.mr = d.cuotas || [];
  docCuotasLineas.mrAccion = d.cuotasAccion || [];

  const lote = todosLotes.find(l => String(_terrenoDeLote(l)).trim().toLowerCase() === String(r.terrenoInteres||'').trim().toLowerCase());
  document.getElementById('mr-datos-auto').style.display = '';
  document.getElementById('mr-form').style.display = '';
  document.getElementById('mr-buscar').value = r.nombre;
  document.getElementById('mr-datos-auto-content').innerHTML = `
    <div><strong>Cliente</strong><br>${esc(r.nombre)}</div>
    <div><strong>Terreno</strong><br>${esc(r.terrenoInteres||'—')}</div>
    <div><strong>Módulo</strong><br>${esc(lote?.modulo||'—')}</div>
    <div><strong>Categoría</strong><br>${esc(lote?.categoria||'—')}</div>
    <div><strong>Modalidad</strong><br>${esc(r.formaPago||'—')}</div>
    <div><strong>Canal</strong><br>${esc(r.origen||'—')}</div>
    <div><strong>Precio lista</strong><br>$${(r.precioLista||0).toLocaleString('es-BO')}</div>
    <div><strong>Precio venta</strong><br>$${(r.precioVenta||0).toLocaleString('es-BO')}</div>
    <div><strong>Descuento</strong><br>$${(r.descuentoUsd||0).toLocaleString('es-BO')} (${r.descuentoPct||0}%)</div>
    <div><strong>Metraje</strong><br>${(r.metraje||0).toLocaleString('es-BO')} m²</div>
    <div><strong>$/m²</strong><br>$${r.precioM2Final||0}</div>
    <div><strong>Asesor</strong><br>${esc(r.asesorNombre||'—')}</div>`;

  document.getElementById('mr-nro-reserva').value = d.nroReserva||'';
  document.getElementById('mr-fecha').value = d.fecha||'';
  document.getElementById('mr-celular').value = d.celular||'';
  document.getElementById('mr-correo').value = d.correo||'';
  document.getElementById('mr-tiene-copropietario').value = d.tieneCoprop||'NO';
  document.getElementById('mr-copropietario-nombre').value = d.copropietarioNombre||'';
  document.getElementById('mr-copropietario-correo').value = d.copropietarioCorreo||'';
  document.getElementById('mr-copropietario-celular').value = d.copropietarioCelular||'';
  document.getElementById('mr-con-accion').value = d.conAccion||'NO';
  document.getElementById('mr-accion-precio').value = d.accionPrecio||'';
  document.getElementById('mr-tc').value = d.tc||'6.96';
  document.getElementById('mr-politica-pct').value = d.politicaPct||'';
  document.getElementById('mr-autorizado-por').value = d.autorizadoPor||'';
  document.getElementById('mr-comentario').value = d.comentario||'';

  toggleCopropietarioDoc('mr');
  toggleAccionDoc('mr');
  renderCuotasLineasDoc('mr');
  renderCuotasLineasDoc('mrAccion');
  return r;
}

// 👁 VER — solo abre el documento para reimprimir/consultar. No toca el modo de edición
// ni pide confirmación: mirar un documento oficial no es una acción riesgosa.
function verDocumentoGuardado(key, cual) {
  const d = documentosReservaData.find(x => x._key === key);
  if (!d) return;
  if (!_restaurarDatosDocGuardado(d)) return;
  if (cual === 'materia') generarMateriaReservada(); else generarSolicitudContrato();
}

// ✏️ EDITAR — modifica un documento oficial ya guardado, así que pide confirmación antes.
function abrirDocumentoGuardado(key) {
  const d = documentosReservaData.find(x => x._key === key);
  if (!d) return;
  const confirmar = confirm('Vas a EDITAR el documento de "' + d.cliente + '" (terreno ' + d.terreno + ', N° ' + (d.nroReserva||'—') + ').\n\nLos cambios que guardes van a reemplazar la versión actual de este documento.\n\n¿Confirmás que querés editarlo?');
  if (!confirmar) return;

  docActualKey = key; // de acá en más, "Guardar" ACTUALIZA este documento, no crea uno nuevo.
  const r = _restaurarDatosDocGuardado(d);
  if (!r) return;

  cambiarModoMateriaReserva('nuevo');
  toastOk('Cargado en el formulario — editá lo que necesites y tocá "Ver Materia" o "Ver Contrato" cuando quieras.');
}

function buscarReservasDoc(tipo) {
  const term = document.getElementById(tipo+'-buscar').value.trim().toLowerCase();
  const cont = document.getElementById(tipo+'-resultados');
  if (!term) { cont.innerHTML = ''; return; }
  const reservas = todosRegs.filter(r => r.conclusion === 'Reserva' &&
    ((r.nombre||'').toLowerCase().includes(term) || (r.terrenoInteres||'').toLowerCase().includes(term)))
    .sort((a,b)=>(b.ts||0)-(a.ts||0)).slice(0,15);

  if (!reservas.length) { cont.innerHTML = '<div class="empty-state">Sin resultados.</div>'; return; }
  cont.innerHTML = reservas.map(r => `
    <div onclick="seleccionarReservaDoc('${tipo}','${r._key}')" style="padding:10px 14px;border:1.5px solid var(--line);border-radius:var(--r-sm);margin-bottom:6px;cursor:pointer;display:flex;justify-content:space-between;" onmouseover="this.style.background='var(--fill)'" onmouseout="this.style.background=''">
      <span><strong>${esc(r.nombre)}</strong> — ${esc(r.terrenoInteres||'sin terreno')}</span>
      <span style="color:var(--gris);font-size:var(--fs-sm);">${esc(r.asesorNombre||'')} · ${esc(r.mes||'')}</span>
    </div>`).join('');

  window['_docReservasVisibles_'+tipo] = reservas;
}

function seleccionarReservaDoc(tipo, key) {
  const r = (window['_docReservasVisibles_'+tipo]||[]).find(x => x._key === key);
  if (!r) return;
  docReservaSeleccionada[tipo] = r;
  if (tipo === 'mr') docActualKey = null; // arrancamos un documento nuevo, no seguimos editando otro

  const lote = todosLotes.find(l => String(_terrenoDeLote(l)).trim().toLowerCase() === String(r.terrenoInteres||'').trim().toLowerCase());
  const modulo = lote?.modulo || '';
  const categoria = lote?.categoria || '';

  document.getElementById(tipo+'-datos-auto').style.display = '';
  document.getElementById(tipo+'-form').style.display = '';
  document.getElementById(tipo+'-resultados').innerHTML = '';
  document.getElementById(tipo+'-buscar').value = r.nombre;

  document.getElementById(tipo+'-datos-auto-content').innerHTML = `
    <div><strong>Cliente</strong><br>${esc(r.nombre)}</div>
    <div><strong>Terreno</strong><br>${esc(r.terrenoInteres||'—')}</div>
    <div><strong>Módulo</strong><br>${esc(modulo||'—')}</div>
    <div><strong>Categoría</strong><br>${esc(categoria||'—')}</div>
    <div><strong>Modalidad</strong><br>${esc(r.formaPago||'—')}</div>
    <div><strong>Canal</strong><br>${esc(r.origen||'—')}</div>
    <div><strong>Precio lista</strong><br>$${(r.precioLista||0).toLocaleString('es-BO')}</div>
    <div><strong>Precio venta</strong><br>$${(r.precioVenta||0).toLocaleString('es-BO')}</div>
  ...
  if (tipo === 'mr') {
    document.getElementById('mr-nro-reserva').value = todosRegs.filter(x=>x.conclusion==='Reserva').length;
    document.getElementById('mr-fecha').value = hoyIso;
    document.getElementById('mr-celular').value = '';
    document.getElementById('mr-correo').value = '';

    const fecha = r.ts ? new Date(r.ts) : new Date();
    const nombresMes = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const politica = categoria ? obtenerPoliticaDescuento(r.formaPago, categoria, fecha.getFullYear(), nombresMes[fecha.getMonth()]) : null;
    document.getElementById('mr-politica-pct').value = politica ? politica.pctMax + '%' : '';
    document.getElementById('mr-politica-nota').textContent = politica ? '(de Panel de Control)' : '(sin política cargada — completá a mano)';
  } else {
    document.getElementById('sc-fecha').value = hoyIso;
    document.getElementById('sc-celular').value = '';
    document.getElementById('sc-correo').value = '';
    document.getElementById('sc-autorizado-por').value = '';
  }

  docCuotasLineas[tipo] = [
    { concepto: 'Reserva', fecha: hoyIso, monto: 1000 },
    { concepto: 'Pago Final', fecha: '', monto: Math.max((r.precioVenta||0)-1000, 0) },
  ];
  renderCuotasLineasDoc(tipo);

  if (tipo === 'mr') {
    docCuotasLineas.mrAccion = [];
    document.getElementById('mr-con-accion').value = 'NO';
    document.getElementById('mr-accion-precio-cont').style.display = 'none';
    document.getElementById('mr-cuotas-accion-cont').style.display = 'none';
    renderCuotasLineasDoc('mrAccion');
  }
}

function toggleCopropietarioDoc(tipo) {
  const si = document.getElementById(tipo+'-tiene-copropietario').value === 'SI';
  document.getElementById(tipo+'-copropietario-nombre-cont').style.display = si ? '' : 'none';
  document.getElementById(tipo+'-copropietario-correo-cont').style.display = si ? '' : 'none';
  const contCel = document.getElementById(tipo+'-copropietario-celular-cont');
  if (contCel) contCel.style.display = si ? '' : 'none';
}
function toggleAccionDoc(tipo) {
  const si = document.getElementById(tipo+'-con-accion').value === 'SI';
  const cont = document.getElementById(tipo+'-accion-precio-cont');
  if (cont) cont.style.display = si ? '' : 'none';
  if (tipo === 'mr') {
    const contCuotas = document.getElementById('mr-cuotas-accion-cont');
    if (contCuotas) contCuotas.style.display = si ? '' : 'none';
    if (si && !docCuotasLineas.mrAccion.length) { docCuotasLineas.mrAccion = [{concepto:'Cuota 1', fecha:'', monto:0}]; renderCuotasLineasDoc('mrAccion'); }
  }
}

function renderCuotasLineasDoc(tipo) {
  const cont = document.getElementById(tipo+'-cuotas-lista');
  const lineas = docCuotasLineas[tipo];
  cont.innerHTML = lineas.map((c,i) => `
    <div style="display:grid;grid-template-columns:1fr 140px 140px 34px;gap:8px;margin-bottom:6px;align-items:center;">
      <input type="text" value="${esc(c.concepto)}" placeholder="Concepto (ej: Cuota 1)" onchange="docCuotasLineas['${tipo}'][${i}].concepto=this.value" />
      <input type="date" value="${c.fecha||''}" onchange="docCuotasLineas['${tipo}'][${i}].fecha=this.value" />
      <input type="number" value="${c.monto||''}" step="0.01" onchange="docCuotasLineas['${tipo}'][${i}].monto=parseFloat(this.value)||0;recalcularTotalCuotasDoc('${tipo}');" />
      <button type="button" onclick="quitarLineaPagoDoc('${tipo}',${i})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:18px;">×</button>
    </div>`).join('');
  recalcularTotalCuotasDoc(tipo);
}
function agregarLineaPagoDoc(tipo) {
  docCuotasLineas[tipo].push({ concepto: 'Cuota ' + docCuotasLineas[tipo].length, fecha:'', monto:0 });
  renderCuotasLineasDoc(tipo);
}
function quitarLineaPagoDoc(tipo, i) {
  docCuotasLineas[tipo].splice(i,1);
  renderCuotasLineasDoc(tipo);
}
function recalcularTotalCuotasDoc(tipo) {
  const total = docCuotasLineas[tipo].reduce((s,c)=>s+(c.monto||0),0);
  setText(tipo+'-cuotas-total', '$'+total.toLocaleString('es-BO'));
}

// Fecha larga en español: "jueves, 3 de septiembre de 2026"
function _fmtFechaLargaDoc(iso) {
  if (!iso) return '';
  const [y,m,d] = iso.split('-').map(Number);
  const fecha = new Date(y, m-1, d);
  const txt = fecha.toLocaleDateString('es-BO', {weekday:'long', day:'numeric', month:'long', year:'numeric'});
  return txt;
}

// "1-12-166" (Módulo-Manzana-Lote, formato interno) → "terreno 166 - manzano 12 - modulo 01"
function _fmtTerrenoDescriptivo(terrenoCodigo) {
  if (!terrenoCodigo) return '—';
  const partes = String(terrenoCodigo).split('-');
  if (partes.length < 3) return terrenoCodigo;
  const [modulo, manzana, lote] = partes;
  return `terreno ${lote} - manzano ${manzana} - modulo ${String(modulo).padStart(2,'0')}`;
}

function _fmtFechaDoc(iso) {
  if (!iso) return '';
  const [y,m,d] = iso.split('-');
  return d+'/'+m+'/'+y;
}

// Logo del Village en base64 — se completa cuando el usuario suba la imagen (ver VILLAGE_LOGO_B64 más abajo).
let VILLAGE_LOGO_B64 = '';

function _logoHtmlDoc() {
  return VILLAGE_LOGO_B64
    ? `<img src="${VILLAGE_LOGO_B64}" style="width:70px;height:70px;object-fit:contain;"/>`
    : `<div style="width:70px;height:70px;background:#111;color:#fff;font-size:8px;font-weight:700;display:flex;align-items:center;justify-content:center;text-align:center;border-radius:6px;">URUBÓ<br>VILLAGE</div>`;
}

function _abrirVentanaDocumento(titulo, htmlBody) {
  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><title>${titulo}</title><meta charset="utf-8">
    <style>
      @page{ size:letter; margin:12mm; }
      *{ box-sizing:border-box; }
      body{ font-family:Arial,Helvetica,sans-serif; font-size:13px; line-height:1.45; color:#1a1a1a; padding:14px; max-width:720px; margin:0 auto; }
      h1{ text-align:center; font-size:18px; margin:0 0 4px; font-weight:700; color:#1F3A2E; letter-spacing:.02em; }
      .regla-marca{ height:3px; background:#1F3A2E; width:64px; margin:0 auto 16px; }
      .regla-marca.izq{ margin:0 0 14px; }

      /* Filas compactas etiqueta+valor — SIN tabla, SIN cuadrícula. La etiqueta ocupa
         solo lo que necesita, el valor va pegado al lado (así se ve tu Excel, no estirado). */
      .fila{ display:flex; gap:8px; margin:2px 0; align-items:baseline; }
      .fila .etq{ font-weight:700; flex-shrink:0; color:#1F3A2E; }
      .fila.dos{ display:grid; grid-template-columns:1fr 1fr; gap:4px 20px; }
      .bloque-sombreado{ background:#F7F5F0; padding:6px 8px; border-radius:2px; border-left:3px solid #1F3A2E; }

      /* Tablas REALES (con cuadrícula) — solo para lo que en el Excel es tabla de verdad:
         Política/Propuesta, Detalle de pagos, y el bloque de firmas. */
      table{ width:100%; border-collapse:collapse; margin:5px 0; table-layout:fixed; }
      td,th{ border:1px solid #c9c4b8; padding:5px 8px; font-size:12.5px; word-wrap:break-word; }
      .header-band{ background:#1F3A2E; color:#fff; font-weight:700; padding:4px 8px; font-size:12.5px; }
      .right{ text-align:right; } .center{ text-align:center; }
      .sign-box{ height:58px; border:1px solid #c9c4b8; width:50%; }
      .dashed td{ border:1px dashed #b8b3a6; }
      .fila-total{ background:#EDF1E8; font-weight:700; }
      .fila-total td{ border-color:#1F3A2E; }
      .cifra-verde{ color:#1F3A2E; font-weight:700; }

      /* Casillero para forma de pago — bien ancho y alto, para que entre un sello de goma */
      .check-sello{ border:1.5px solid #333; display:inline-block; width:70px; height:34px; text-align:center;
        line-height:32px; font-size:15px; font-weight:700; vertical-align:middle; }

      @media print { body{ padding:0; } .print-btn{ display:none; } }
      button.print-btn{ margin-bottom:16px; padding:9px 16px; background:#2451FF; color:#fff; border:none; border-radius:6px; font-weight:700; cursor:pointer; }
    </style></head><body>
...
function generarMateriaReservada() {
  const r = docReservaSeleccionada.mr;
  if (!r) { toastErr('Elegí una reserva primero.'); return; }
  const lote = todosLotes.find(l => String(_terrenoDeLote(l)).trim().toLowerCase() === String(r.terrenoInteres||'').trim().toLowerCase());
  const modulo = lote?.modulo || '';
  const categoria = lote?.categoria || '';
  const conAccion = document.getElementById('mr-con-accion').value === 'SI';
  const precioAccion = parseFloat(document.getElementById('mr-accion-precio').value)||0;
  const cuotas = docCuotasLineas.mr;
  const totalCuotas = cuotas.reduce((s,c)=>s+(c.monto||0),0);

  // Las 4 modalidades se muestran siempre, igual que en Solicitud de Contrato —
  // se completan solas desde Panel de Control cuando hay política cargada para esa categoría.
  const MODALIDADES_4 = ['Contado', 'Contado Diferido', 'Crédito Directo', 'Crédito Bancario'];
  const fecha = r.ts ? new Date(r.ts) : new Date();
  const nombresMes = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const politicas4 = MODALIDADES_4.map(m => categoria ? obtenerPoliticaDescuento(m, categoria, fecha.getFullYear(), nombresMes[fecha.getMonth()]) : null);
  const _normalizarTexto = s => (s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
  const esModalidadActual = m => _normalizarTexto(r.formaPago) === _normalizarTexto(m);
  const checklistModalidad = MODALIDADES_4.map(m => esModalidadActual(m) ? 'X' : '');

  const filaPropuesta = MODALIDADES_4.map((m,i) => {
    if (esModalidadActual(m)) return { desc:'$'+(r.descuentoUsd||0).toLocaleString('es-BO'), pct:(r.descuentoPct||0)+'%', pv:'$'+(r.precioVenta||0).toLocaleString('es-BO'), m2:'$'+(r.precioM2Final||0) };
    return { desc:'$0', pct:'0%', pv:'$0', m2:'$0' };
  });

  const filasCuotas = cuotas.map(c => `<tr><td>${_fmtFechaDoc(c.fecha)}</td><td colspan="2">${esc(c.concepto)}</td><td class="right">$${(c.monto||0).toLocaleString('es-BO')}</td></tr>`).join('');

  const cuotasAccion = docCuotasLineas.mrAccion || [];
  const bloqueAccion = conAccion ? `
    <table><tr class="header-band"><td colspan="4">Venta Acción</td></tr></table>
    <table style="margin-top:0;">
      ${cuotasAccion.map(c => `<tr><td>${esc(c.concepto)}</td><td colspan="2">${_fmtFechaDoc(c.fecha)}</td><td class="right">$${(c.monto||0).toLocaleString('es-BO')}</td></tr>`).join('')}
      <tr class="fila-total"><td colspan="3"><strong>Total Acción</strong></td><td class="right"><strong>$${cuotasAccion.reduce((s,c)=>s+(c.monto||0),0).toLocaleString('es-BO')}</strong></td></tr>
    </table>` : '';

  const html = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:2px;">
      <h1 style="text-align:left;margin:0;">Materia Reservada de Venta</h1>
      ${_logoHtmlDoc()}
    </div>
    <div class="regla-marca izq"></div>

    <div class="fila"><span class="etq" style="width:60px;">Fecha:</span><span>${_fmtFechaDoc(document.getElementById('mr-fecha').value)}</span>
      <span style="margin-left:auto;font-size:20px;color:#c00;font-weight:700;">${esc(document.getElementById('mr-nro-reserva').value)}</span></div>
    <div class="fila"><span class="etq" style="width:60px;">Cliente:</span><span>${esc(r.nombre)}</span></div>
    <div class="fila dos" style="grid-template-columns:1.4fr 1fr;">
      <span><span class="etq">Terreno:</span> ${esc(_fmtTerrenoDescriptivo(r.terrenoInteres))}</span>
      <span><span class="etq">Acción:</span> ${conAccion?'Sí':'No'}</span>
    </div>
    <div class="fila dos" style="grid-template-columns:1.4fr 1fr;">
      <span><span class="etq">Precio:</span> ${(r.precioVenta||0).toLocaleString('es-BO')}</span>
      <span><span class="etq">$/M2:</span> ${r.precioM2Final||0}</span>
    </div>
    <div class="fila dos" style="grid-template-columns:1.4fr 1fr;">
      <span><span class="etq">M2:</span> ${(r.metraje||0).toLocaleString('es-BO')}</span>
      <span><span class="etq">Canal:</span> ${esc(r.origen||'—')}</span>
    </div>
    <div class="fila dos" style="grid-template-columns:1.4fr 1fr;">
      <span><span class="etq">TCO:</span> ${esc(document.getElementById('mr-tc').value)}</span>
      <span><span class="etq">Categoría:</span> ${esc(categoria||'—')}</span>
    </div>

    <table style="margin-top:10px;"><tr class="header-band"><td colspan="5">1 &nbsp; Política Actual</td></tr></table>
    <table style="margin-top:0;">
      <tr><td></td>${MODALIDADES_4.map(m=>`<td class="center"><strong>${m}</strong></td>`).join('')}</tr>
      <tr><td><strong>Modalidad elegida</strong></td>${checklistModalidad.map(c=>`<td class="center" style="${c?'color:#1F3A2E;font-weight:700;font-size:15px;':''}">${c?'☑':'☐'}</td>`).join('')}</tr>
      <tr><td><strong>Descuento</strong></td>${politicas4.map(p=>`<td class="center">${p?p.pctMax+'%':'—'}</td>`).join('')}</tr>
      <tr><td><strong>Plazo</strong></td>${MODALIDADES_4.map(m => { const p = obtenerPlazoModalidad(m); return `<td class="center">${p?p.plazoMax+' '+p.unidad:'—'}</td>`; }).join('')}</tr>
      <tr><td><strong>Precio Mínimo</strong></td>${politicas4.map(p => { if (!p) return '<td class="center">—</td>'; const min = (r.precioLista||0) * (1 - p.pctMax/100); return `<td class="center">$${Math.round(min).toLocaleString('es-BO')}</td>`; }).join('')}</tr>
      <tr><td><strong>M2$ Mínimo</strong></td>${politicas4.map(p => { if (!p || !r.metraje) return '<td class="center">—</td>'; const min = (r.precioLista||0) * (1 - p.pctMax/100) / r.metraje; return `<td class="center">$${min.toFixed(1)}</td>`; }).join('')}</tr>
    </table>

    <table><tr class="header-band"><td colspan="5">2 &nbsp; Propuesta</td></tr></table>
    <table style="margin-top:0;">
      <tr><td></td>${MODALIDADES_4.map(m=>`<td class="center"><strong>${m}</strong></td>`).join('')}</tr>
      <tr><td><strong>Descuento</strong></td>${filaPropuesta.map(f=>`<td class="center">${f.desc}</td>`).join('')}</tr>
      <tr><td><strong>% Dcto</strong></td>${filaPropuesta.map(f=>`<td class="center">${f.pct}</td>`).join('')}</tr>
      <tr><td><strong>Precio de Venta</strong></td>${filaPropuesta.map(f=>`<td class="center">${f.pv}</td>`).join('')}</tr>
      <tr><td><strong>M2$ Venta</strong></td>${filaPropuesta.map(f=>`<td class="center">${f.m2}</td>`).join('')}</tr>
    </table>

    <table class="dashed">
      <tr><td colspan="2"><strong>Detalle</strong></td><td colspan="2"><strong>Fecha</strong></td></tr>
      ${filasCuotas}
      <tr class="fila-total"><td colspan="2"><strong>Total</strong></td><td colspan="2" class="right"><strong>$${totalCuotas.toLocaleString('es-BO')}</strong></td></tr>
    </table>

    ${bloqueAccion}

    <p style="margin:10px 0 3px;"><strong>Comentario sobre la venta:</strong></p>
    <div style="border:1px solid #333;background:#f2f2f2;padding:8px;min-height:30px;">${esc(document.getElementById('mr-comentario').value)}</div>

    <table style="margin-top:16px;">
      <colgroup><col style="width:50%;"><col style="width:50%;"></colgroup>
      <tr><th class="center">Asesor Comercial</th><th class="center">Controller</th></tr>
      <tr><td class="sign-box"></td><td class="sign-box"></td></tr>
      <tr><th class="center">Gerencia Comercial</th><th class="center">Gerencia Administrativa<br>y Financiera</th></tr>
      <tr><td class="sign-box"></td><td class="sign-box"></td></tr>
    </table>

    <p style="margin-top:10px;font-size:11px;"><strong>Correo:</strong> ${esc(document.getElementById('mr-correo').value)} &nbsp;&nbsp; <strong>Celular:</strong> ${esc(document.getElementById('mr-celular').value)}
      ${document.getElementById('mr-tiene-copropietario').value==='SI' ? `<br><strong>Correo copropietario:</strong> ${esc(document.getElementById('mr-copropietario-correo').value)} &nbsp;&nbsp; <strong>Celular copropietario:</strong> ${esc(document.getElementById('mr-copropietario-celular').value)}`:''}</p>`;

  _abrirVentanaDocumento('Materia Reservada de Venta - ' + r.nombre, html);
}

function generarSolicitudContrato() {
  const r = docReservaSeleccionada.mr;
  if (!r) { toastErr('Elegí una reserva primero.'); return; }
  const tieneCoprop = document.getElementById('mr-tiene-copropietario').value === 'SI';
  const conAccion = document.getElementById('mr-con-accion').value === 'SI';

  // Grilla 2x2 tal cual la planilla: Contado | Crédito Directo // Contado Diferido | Crédito Bancario
  const modalidadesCheck = { 'Contado':'', 'Contado Diferido':'', 'Crédito Directo':'', 'Crédito Bancario':'' };
  const _normalizarTexto2 = s => (s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
  const modKey = Object.keys(modalidadesCheck).find(k => _normalizarTexto2(r.formaPago) === _normalizarTexto2(k));
  if (modKey) modalidadesCheck[modKey] = 'X';

  const cuotas = docCuotasLineas.mr;
  const filasCuotas = cuotas.map(c => `<tr><td>${esc(c.concepto)}</td><td class="right">${(c.monto||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</td><td class="center">${_fmtFechaDoc(c.fecha)}</td></tr>`).join('');

  const filasContacto = [];
  if (document.getElementById('mr-correo').value) filasContacto.push(`Correo: ${esc(document.getElementById('mr-correo').value)}`);
  if (tieneCoprop && document.getElementById('mr-copropietario-correo').value) filasContacto.push(`Correo: ${esc(document.getElementById('mr-copropietario-correo').value)}`);
  const filasCelular = [];
  if (document.getElementById('mr-celular').value) filasCelular.push(`Celular: ${esc(document.getElementById('mr-celular').value)}`);
  if (tieneCoprop && document.getElementById('mr-copropietario-celular').value) filasCelular.push(`Celular: ${esc(document.getElementById('mr-copropietario-celular').value)}`);
  const pieContacto = filasContacto.map((c,i) => `<div class="fila dos"><span>${c}</span><span>${filasCelular[i]||''}</span></div>`).join('');

  const html = `
    <h1 style="text-align:center;">SOLICITUD DE ELABORACION DE CONTRATO</h1>
    <div class="regla-marca"></div>

    <div class="bloque-sombreado" style="margin-bottom:10px;">
      <div class="fila"><span class="etq">Nombre del Cliente:</span><span>${esc(r.nombre)}</span></div>
      ${tieneCoprop ? `<div class="fila"><span class="etq">Nombre Copropietario:</span><span>${esc(document.getElementById('mr-copropietario-nombre').value)}</span></div>` : ''}
      <div class="fila"><span class="etq">Fecha de la solicitud:</span><span>${_fmtFechaLargaDoc(document.getElementById('mr-fecha').value)}</span></div>
    </div>

    <div class="fila"><span class="etq">Terreno:</span><span>${esc(_fmtTerrenoDescriptivo(r.terrenoInteres))}</span></div>

    <div class="fila" style="margin-top:6px;">
      <span class="etq" style="width:90px;">Forma de pago:</span>
      <span style="margin-right:40px;">Contado <span class="check-sello">${modalidadesCheck['Contado']}</span></span>
      <span>Crédito Directo <span class="check-sello">${modalidadesCheck['Crédito Directo']}</span></span>
    </div>
    <div class="fila" style="margin-top:6px;">
      <span class="etq" style="width:90px;"></span>
      <span style="margin-right:14px;">Contado Diferido <span class="check-sello">${modalidadesCheck['Contado Diferido']}</span></span>
      <span>Crédito Bancario <span class="check-sello">${modalidadesCheck['Crédito Bancario']}</span></span>
    </div>

    <p style="margin:14px 0 3px;"><strong>Observaciones:</strong></p>
    <div style="border:1px solid #333;background:#f2f2f2;padding:8px;min-height:50px;">
      Venta de terreno a ${esc(r.formaPago||'')}${conAccion?' con acción del country club':' sin acción del country club'}.<br>
      ${esc(document.getElementById('mr-comentario').value)}
    </div>

    <div style="margin-top:14px;">
      <div class="fila"><span class="etq" style="width:220px;">Precio de lista del terreno:</span><span>${(r.precioLista||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</span></div>
      <div class="fila"><span class="etq" style="width:220px;">Descuento:</span><span>${(r.descuentoUsd||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</span></div>
      <div class="fila"><span class="etq" style="width:220px;">Precio de venta:</span><span class="cifra-verde">${(r.precioVenta||0).toLocaleString('es-BO',{minimumFractionDigits:2})}</span></div>
    </div>

    <table style="margin-top:10px;">
      <tr><th>Detalle</th><th>Monto</th><th>Fecha de pago</th></tr>
      ${filasCuotas}
    </table>

    <p style="margin:12px 0 3px;"><strong>Solicitado por:</strong></p>
    <div style="border:1px solid #333;background:#f2f2f2;min-height:44px;"></div>

    <p style="margin:12px 0 3px;"><strong>Autorizado por:</strong></p>
    <div style="border:1px solid #333;background:#f2f2f2;min-height:44px;padding:6px;">${esc(document.getElementById('mr-autorizado-por').value)}</div>

    <div style="margin-top:14px;">
      ${pieContacto}
    </div>`;

  _abrirVentanaDocumento('Solicitud de Elaboracion de Contrato - ' + r.nombre, html);
}

/* ═══════════════════════════════════════════
   COTIZACIONES — el cliente la ve por link público, sin login, y puede confirmarla.
═══════════════════════════════════════════ */
let cotizacionesData = [];
let cotizLoteElegido = null;

function cambiarModoCotizaciones(modo) {
  document.getElementById('cotiz-modo-nueva').style.display = (modo==='nueva') ? '' : 'none';
  document.getElementById('cotiz-modo-guardadas').style.display = (modo==='guardadas') ? '' : 'none';
  const btnN = document.getElementById('btn-cotiz-modo-nueva');
  const btnG = document.getElementById('btn-cotiz-modo-guardadas');
  if (modo === 'guardadas') {
    btnN.style.background='transparent'; btnN.style.boxShadow='none';
    btnG.style.background='#fff'; btnG.style.boxShadow='0 1px 3px rgba(0,0,0,.1)';
    renderCotizacionesGuardadas();
  } else {
    btnG.style.background='transparent'; btnG.style.boxShadow='none';
    btnN.style.background='#fff'; btnN.style.boxShadow='0 1px 3px rgba(0,0,0,.1)';
  }
}

function buscarTerrenoCotizacion() {
  const term = document.getElementById('cotiz-buscar-terreno').value.trim().toLowerCase();
  const cont = document.getElementById('cotiz-resultados-terreno');
  if (!term) { cont.innerHTML = ''; return; }
  const resultados = todosLotes.filter(l => String(_terrenoDeLote(l)).toLowerCase().includes(term)).slice(0,10);
  if (!resultados.length) { cont.innerHTML = '<div class="empty-state">Sin resultados.</div>'; return; }
  window._cotizLotesVisibles = resultados;
  cont.innerHTML = resultados.map(l => `
    <div onclick="seleccionarTerrenoCotizacion('${l._key}')" style="padding:10px 14px;border:1.5px solid var(--line);border-radius:var(--r-sm);margin-bottom:6px;cursor:pointer;display:flex;justify-content:space-between;">
      <span><strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')}</span>
      <span style="color:var(--gris);font-size:var(--fs-sm);">${l.metraje||0} m² · ${l.estado||'—'}</span>
    </div>`).join('');
}

function seleccionarTerrenoCotizacion(key) {
  const l = (window._cotizLotesVisibles||[]).find(x => x._key === key);
  if (!l) return;
  cotizLoteElegido = l;
  document.getElementById('cotiz-resultados-terreno').innerHTML = '';
  document.getElementById('cotiz-buscar-terreno').value = _terrenoDeLote(l);
  document.getElementById('cotiz-terreno-elegido').style.display = '';
  document.getElementById('cotiz-terreno-elegido').innerHTML = `
    <strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')} · Categoría ${esc(l.categoria||'—')} · ${(l.metraje||0).toLocaleString('es-BO')} m²
    ${l.estado && l.estado!=='Disponible' ? `<br><span style="color:var(--warn);font-weight:600;">⚠️ Figura como "${esc(l.estado)}" en Inventario — verificá antes de cotizar.</span>` : ''}`;
  document.getElementById('cotiz-precio-lista').value = l.precio || '';
  document.getElementById('cotiz-form-resto').style.display = '';
  recalcularCotizacion();
}

function toggleAccionCotizacion() {
  const si = document.getElementById('cotiz-con-accion').value === 'SI';
  document.getElementById('cotiz-accion-precio-cont').style.display = si ? '' : 'none';
}

function recalcularCotizacion() {
  const precioLista = parseFloat(document.getElementById('cotiz-precio-lista').value) || 0;
  const descPct = parseFloat(document.getElementById('cotiz-descuento-pct').value) || 0;
  const precioFinal = precioLista * (1 - descPct/100);
  document.getElementById('cotiz-precio-final').value = precioFinal ? precioFinal.toFixed(2) : '';

  const reserva = parseFloat(document.getElementById('cotiz-reserva').value) || 0;
  const pctAporte = parseFloat(document.getElementById('cotiz-pct-aporte').value) || 0;
  const montoAporte = precioFinal * (pctAporte/100);
  const montoSaldo = precioFinal - montoAporte;
  const nCuotas = parseInt(document.getElementById('cotiz-nro-cuotas').value) || 1;
  const montoCuota = nCuotas ? montoSaldo/nCuotas : 0;

  document.getElementById('cotiz-monto-aporte').value = montoAporte ? montoAporte.toFixed(2) : '';
  document.getElementById('cotiz-monto-saldo').value = montoSaldo ? montoSaldo.toFixed(2) : '';
  document.getElementById('cotiz-monto-cuota').value = montoCuota ? montoCuota.toFixed(2) : '';
}

async function guardarYGenerarCotizacion() {
  if (!cotizLoteElegido) { toastErr('Elegí un terreno primero.'); return; }
  const cliente = document.getElementById('cotiz-cliente').value.trim();
  if (!cliente) { toastErr('Cargá el nombre del cliente.'); return; }

  const l = cotizLoteElegido;
  const vigenciaDias = parseInt(document.getElementById('cotiz-vigencia-dias').value) || 5;
  const fechaVencimiento = Date.now() + vigenciaDias*24*60*60*1000;

  const data = {
    cliente, celular: document.getElementById('cotiz-celular').value, correo: document.getElementById('cotiz-correo').value,
    terreno: _terrenoDeLote(l), modulo: l.modulo||'', categoria: l.categoria||'', superficie: l.metraje||0,
    precioLista: parseFloat(document.getElementById('cotiz-precio-lista').value)||0,
    descuentoPct: Math.round((parseFloat(document.getElementById('cotiz-descuento-pct').value)||0)*100)/100,
    precioFinal: parseFloat(document.getElementById('cotiz-precio-final').value)||0,
    conAccion: document.getElementById('cotiz-con-accion').value,
    accionPrecio: parseFloat(document.getElementById('cotiz-accion-precio').value)||0,
    modalidad: document.getElementById('cotiz-modalidad').value,
    reserva: parseFloat(document.getElementById('cotiz-reserva').value)||0,
    pctAporte: parseFloat(document.getElementById('cotiz-pct-aporte').value)||0,
    montoAporte: parseFloat(document.getElementById('cotiz-monto-aporte').value)||0,
    montoSaldo: parseFloat(document.getElementById('cotiz-monto-saldo').value)||0,
    nroCuotas: parseInt(document.getElementById('cotiz-nro-cuotas').value)||0,
    montoCuota: parseFloat(document.getElementById('cotiz-monto-cuota').value)||0,
    vigenciaDias, fechaVencimiento,
    asesorCelular: document.getElementById('cotiz-asesor-celular').value,
    estado: 'pendiente',
    asesorNombre: asesorActual?.nombre || 'Admin',
  };

  const res = await window._fbPushCotizacion(data);
  const link = window.location.origin + window.location.pathname + '?cotizacion=' + res.key;
  document.getElementById('cotiz-link-generado').style.display = '';
  document.getElementById('cotiz-link-texto').value = link;
  toastOk('Cotización guardada.');
}

function copiarLinkCotizacion() {
  const input = document.getElementById('cotiz-link-texto');
  input.select();
  navigator.clipboard?.writeText(input.value);
  toastOk('Link copiado.');
}

function _estadoRealCotizacion(c) {
  if (c.estado === 'confirmado') return 'confirmado';
  if (c.fechaVencimiento && Date.now() > c.fechaVencimiento) return 'vencido';
  return 'pendiente';
}

function renderCotizacionesGuardadas() {
  const tbody = document.getElementById('cotiz-guardadas-tbody');
  if (!tbody) return;
  const term = (document.getElementById('cotiz-buscar-guardadas')?.value||'').trim().toLowerCase();
  let datos = cotizacionesData;
  if (term) datos = datos.filter(c => (c.cliente||'').toLowerCase().includes(term) || (c.terreno||'').toLowerCase().includes(term));

  const pend = cotizacionesData.filter(c=>_estadoRealCotizacion(c)==='pendiente').length;
  const conf = cotizacionesData.filter(c=>_estadoRealCotizacion(c)==='confirmado').length;
  const venc = cotizacionesData.filter(c=>_estadoRealCotizacion(c)==='vencido').length;
  setText('cotiz-kpi-pendientes', pend);
  setText('cotiz-kpi-confirmadas', conf);
  setText('cotiz-kpi-vencidas', venc);

  if (!datos.length) { tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Sin cotizaciones guardadas todavía.</div></td></tr>'; return; }

  const badges = { pendiente:'background:var(--warn-bg);color:var(--warn-ink);', confirmado:'background:var(--ok-bg);color:var(--ok-ink);', vencido:'background:var(--danger-bg);color:var(--danger-ink);' };
  tbody.innerHTML = datos.map(c => {
    const estadoReal = _estadoRealCotizacion(c);
    const link = window.location.origin + window.location.pathname + '?cotizacion=' + c._key;
    return `<tr>
    <td style="font-size:var(--fs-sm);">${new Date(c.ts).toLocaleDateString('es-BO')}</td>
    <td>${esc(c.cliente)}</td>
    <td style="font-family:monospace;font-size:var(--fs-sm);">${esc(c.terreno)}</td>
    <td style="font-weight:600;color:var(--cartera);">$${(c.precioFinal||0).toLocaleString('es-BO')}</td>
    <td><span style="padding:3px 10px;border-radius:999px;font-size:var(--fs-xs);font-weight:600;${badges[estadoReal]}">${estadoReal}</span></td>
    <td>
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);" onclick="navigator.clipboard.writeText('${link}');toastOk('Link copiado.');">📋 Copiar link</button>
      ${estadoReal==='pendiente' ? `<button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);color:var(--ok);" onclick="confirmarCotizacionDesdeAdmin('${c._key}')">✅ Confirmar</button>` : ''}
      <button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);color:var(--danger);" onclick="eliminarCotizacion('${c._key}')">🗑</button>
    </td>
  </tr>`;
  }).join('');
}
...
/* ═══════════════════════════════════════════
   VISTA PÚBLICA DE COTIZACIÓN — sin login, la ve el cliente por el link.
   Estilo: negro/crema/verde bosque, serif editorial — igual al tríptico de la marca.
═══════════════════════════════════════════ */
async function mostrarCotizacionPublica(key) {
  // Sacamos TODO el CSS del sistema administrativo — si no, sus reglas pelean con el diseño de esta página.
  document.querySelectorAll('head link[rel="stylesheet"], head style').forEach(el => el.remove());
  document.documentElement.style.margin = '0';
  document.body.style.margin = '0';

  document.title = 'Tu cotización — Urubó Village';
  document.body.innerHTML = `<div id="cotiz-publica-root" style="min-height:100vh;background:#ecebed;display:flex;align-items:center;justify-content:center;font-family:sans-serif;">
    <div style="text-align:center;color:#494C4A;font-size:14px;">Cargando tu cotización...</div>
  </div>`;

  const c = await window._fbGetCotizacionOnce(key);
  const root = document.getElementById('cotiz-publica-root');
  if (!c) { root.innerHTML = _cotizPublicaErrorHtml('Esta cotización no existe o fue eliminada.'); return; }

  const estadoReal = _estadoRealCotizacion(c);
  if (estadoReal === 'vencido') {
    root.innerHTML = _cotizPublicaErrorHtml('Esta cotización ya venció (era válida por ' + c.vigenciaDias + ' días). Pedile a tu asesor que te genere una nueva.');
    return;
  }

  const fmt = n => '$' + Math.round(n||0).toLocaleString('en-US');
  const fmtBs = n => 'Bs. ' + Math.round(n||0).toLocaleString('en-US');
  const fmtPct = n => { const r = Math.round((n||0)*100)/100; return (r % 1 === 0) ? String(r) : r.toFixed(2); };
  const hayDescuento = (c.descuentoPct||0) > 0;
  const waNumero = (c.asesorCelular||'').replace(/[^0-9]/g,'');
  const waLink = waNumero ? `https://wa.me/${waNumero}?text=${encodeURIComponent('Hola '+(c.asesorNombre||'')+', te escribo por la cotización del terreno '+(c.terreno||'')+' en Urubó Village.')}` : '#';
  const iconoCheck = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.5l4.5 4.5L19 7.5"/></svg>';

  root.innerHTML = `
    <style>
      @import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700&family=Roboto:wght@400;500;700&display=swap');
      *{box-sizing:border-box;}
      html,body{margin:0;}
      body{ font-family:Roboto,ui-sans-serif,system-ui,sans-serif; color:#494C4A; background:#ecebed; -webkit-font-smoothing:antialiased; }
      .num{font-variant-numeric:tabular-nums;}
      img{display:block;}
      .pagina{ min-height:100dvh; display:flex; justify-content:center; align-items:center; padding:2rem 1rem; }
      .shell{ display:flex; width:100%; max-width:1080px; height:calc(100dvh - 4rem); max-height:860px; overflow:hidden; border-radius:24px; box-shadow:0 25px 50px -12px rgba(0,0,0,.25); background:#fff; }
      .marca{ position:relative; flex:1 1 0; display:flex; flex-direction:column; overflow:hidden; color:#fff; min-width:0; }
      .marca .slide{ position:absolute; left:0; right:0; top:-70%; width:100%; height:170%; object-fit:cover; object-position:center bottom; opacity:0; transition:opacity 1.5s ease; }
      .marca .slide.arriba{top:0; object-position:center top;}
      .marca .slide.activa{opacity:1;}
      .marca .velo{ position:absolute; inset:0; background:linear-gradient(180deg, rgba(20,20,22,.35) 0%, rgba(12,12,14,.55) 60%, rgba(8,8,10,.72) 100%); }
      .marca .contenido{ position:relative; flex:1; display:flex; flex-direction:column; justify-content:space-between; padding:44px; }
      .etiqueta{ display:inline-flex; align-items:center; gap:8px; padding:6px 12px; border-radius:999px; background:rgba(255,255,255,.14); backdrop-filter:blur(6px); border:1px solid rgba(255,255,255,.28); font-family:Sora,sans-serif; font-size:11px; font-weight:600; letter-spacing:.08em; text-transform:uppercase; align-self:flex-start; margin-bottom:20px; }
      .etiqueta .dot{width:6px;height:6px;border-radius:50%;background:#4ade80;}
      .marca h2{ font-family:Sora,sans-serif; font-size:38px; font-weight:700; line-height:1.08; margin:0; max-width:440px; }
      .marca .sub{ margin:20px 0 0; max-width:440px; font-size:16px; line-height:1.6; color:rgba(255,255,255,.85); }
      .stats{display:flex; gap:36px;}
      .stats p{margin:0;}
      .stats .n{ font-family:Sora,sans-serif; font-size:28px; font-weight:700; line-height:1; }
      .stats .l{margin-top:4px; font-size:12.5px; color:rgba(255,255,255,.8);}
      .puntos{ position:absolute; right:44px; top:50px; display:flex; gap:6px; z-index:2; }
      .puntos span{ width:7px; height:7px; border-radius:999px; background:rgba(255,255,255,.45); transition:background .3s, width .3s; }
      .puntos span.activa{width:20px; background:#fff;}
      .marquee{ position:relative; background:#fff; padding:20px 0; border-top:1px solid rgba(0,0,0,.05); }
      .marquee .tit{ margin:0 0 12px; padding:0 44px; font-family:Sora,sans-serif; font-size:10px; font-weight:600; letter-spacing:.12em; text-transform:uppercase; color:#797979; }
      .marquee .pista{ overflow:hidden; -webkit-mask-image:linear-gradient(to right, transparent, #000 6%, #000 94%, transparent); mask-image:linear-gradient(to right, transparent, #000 6%, #000 94%, transparent); }
      .marquee ul{ display:flex; align-items:center; gap:60px; margin:0; padding:0 60px 0 0; list-style:none; width:max-content; animation:marquee 30s linear infinite; }
      .marquee li{flex-shrink:0; display:flex; align-items:center; gap:12px; color:#494C4A;}
      .marquee li .nombre{ font-family:Sora,sans-serif; font-size:14px; font-weight:600; letter-spacing:.04em; color:#494C4A; }
      @keyframes marquee{to{transform:translateX(-50%);}}
      .app{ width:440px; flex-shrink:0; height:100%; overflow-y:auto; background:#fff; }
      .res{ display:flex; flex-direction:column; padding:24px 24px 32px; min-height:100%; }
      .cab{display:flex; align-items:center; gap:12px;}
      .cab .uv-mark{ width:26px; height:26px; border-radius:6px; background:linear-gradient(135deg,#0A0A0A,#2d5a27); display:grid; place-items:center; color:#fff; font-family:Sora,sans-serif; font-weight:700; font-size:11px; }
      .cab span{ font-family:Sora,sans-serif; font-size:11px; font-weight:600; letter-spacing:.12em; text-transform:uppercase; color:#AB2020; }
      .chip-uv{ display:flex; align-items:center; gap:10px; margin-top:14px; padding:9px 12px; border-radius:10px; background:#F6F5F6; border:1px solid #e6e3e6; }
      .chip-uv .uv-mark{ width:30px; height:30px; border-radius:6px; flex-shrink:0; background:linear-gradient(135deg,#0A0A0A,#2d5a27); display:grid; place-items:center; color:#fff; font-family:Sora,sans-serif; font-weight:700; font-size:12px; }
      .chip-uv p{margin:0; font-size:12px; line-height:1.35; color:#797979;}
      .chip-uv p b{display:block; font-family:Sora,sans-serif; font-size:12.5px; color:#494C4A;}
      h1.tit{ margin:16px 0 0; font-family:Sora,sans-serif; font-size:26px; font-weight:700; color:#494C4A; }
      .kit{ margin-top:16px; padding:16px; border:1px solid #e6e3e6; border-left:4px solid #D62828; border-radius:10px; }
      .kit .n{margin:0; font-family:Sora,sans-serif; font-size:17px; font-weight:600;}
      .kit .k{ margin:4px 0 0; font-family:Sora,sans-serif; font-size:22px; font-weight:700; color:#D62828; }
      .kit .k small{font-size:14px; font-weight:600; color:#797979;}
      .grid2{ display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-top:16px; }
      .caja{background:#F6F5F6; border-radius:10px; padding:14px;}
      .caja p{margin:0;}
      .caja .l{font-size:11.5px; color:#797979; line-height:1.3;}
      .caja .l small{font-size:9.5px; display:block; margin-top:2px;}
      .caja .v{ margin-top:6px; font-family:Sora,sans-serif; font-size:22px; font-weight:700; color:#5A7602; line-height:1; }
      .sec{ margin:20px 0 0; font-family:Sora,sans-serif; font-size:13px; font-weight:600; color:#494C4A; }
      .fact{overflow:hidden; border:1px solid #e6e3e6; border-radius:8px;}
      .fact .h{ padding:6px 12px; text-align:center; color:#fff; font-family:Sora,sans-serif; font-size:10px; font-weight:600; letter-spacing:.08em; text-transform:uppercase; }
      .fact .b{padding:10px 12px; font-size:11px;}
      .fact .b > div{display:flex; justify-content:space-between; padding:3px 0;}
      .fact .sep{margin:6px 0; border-top:1px dashed #d9d6d9;}
      .fact .tot{align-items:flex-end;}
      .fact .tot b{font-family:Sora,sans-serif; font-size:17px; font-weight:700;}
      .fact .pie{ padding:6px 12px; text-align:center; font-size:10px; color:#797979; background:#F6F5F6; }
      .fact .pie.rojo{background:#FBEAEA; color:#801818; font-weight:600;}
      .nota{margin:8px 0 0; text-align:center; font-size:11px; color:#797979;}
      .precio{margin-top:20px; padding-top:16px; border-top:1px solid #e6e3e6;}
      .precio .tach{ margin:0; font-family:Sora,sans-serif; font-size:15px; font-weight:600; color:#a9a6a9; text-decoration:line-through; text-decoration-thickness:2px; }
      .precio .fila{display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-top:2px;}
      .precio .usd{ margin:0; font-family:Sora,sans-serif; font-size:26px; font-weight:700; color:#D62828; }
      .badge-desc{ display:inline-flex; align-items:center; gap:6px; padding:5px 10px; border-radius:999px; background:#E2E9CC; color:#5A7602; font-family:Sora,sans-serif; font-size:10.5px; font-weight:700; letter-spacing:.02em; }
      .precio .cuota{margin:8px 0 0; font-size:12.5px; color:#797979;}
      .precio .cuota b{color:#494C4A;}
      .precio .cuota .llave{ font-family:Sora,sans-serif; font-weight:600; text-transform:uppercase; letter-spacing:.1em; font-size:10px; }
      .equipos{display:flex; flex-direction:column; gap:12px; margin-top:8px;}
      .eq{ display:flex; gap:12px; padding:12px; border:1px solid #e6e3e6; border-radius:10px; background:#fff; }
      .eq .img{ flex-shrink:0; width:74px; height:92px; display:flex; align-items:center; justify-content:center; padding:6px; border-radius:8px; background:linear-gradient(135deg,#E8EDE4,#D6E0CE); color:#2d5a27; }
      .eq .img svg{width:38px; height:38px;}
      .eq .t{margin:0; font-family:Sora,sans-serif; font-size:13.5px; font-weight:600; line-height:1.25;}
      .eq .badge{ display:inline-block; margin-top:4px; padding:2px 8px; border-radius:999px; background:#FBEAEA; color:#AB2020; font-family:Sora,sans-serif; font-size:10px; font-weight:600; }
      .eq .d{margin:4px 0 0; font-size:11.5px; color:#797979;}
      .eq ul{ margin:8px 0 0; padding:0; list-style:none; display:flex; flex-direction:column; gap:5px; }
      .eq li{ display:flex; gap:7px; font-size:11.5px; line-height:1.4; color:#494C4A; }
      .eq li svg{flex-shrink:0; margin-top:2px; color:#AB2020;}
      .eq li b{color:#494C4A; font-weight:600;}
      .aviso{ margin-top:16px; padding:12px 16px; border-radius:10px; font-size:12.5px; line-height:1.5; background:#FAF1E2; border:1px solid #EAD5AC; color:#7A4B00; }
      .aviso strong{font-weight:700;}
      .cierre{ margin:12px 0 0; text-align:center; font-size:13px; line-height:1.6; color:#494C4A; }
      .btn{margin-top:auto; padding-top:24px;}
      .btn a{ display:flex; align-items:center; justify-content:center; gap:10px; height:54px; border-radius:10px; border:2px solid #494C4A; color:#494C4A; font-family:Sora,sans-serif; font-size:16px; font-weight:600; text-decoration:none; }
      .btn.wa a{ background:#25D366; border-color:#25D366; color:#fff; }
      .btn.desactivado a{ opacity:.4; pointer-events:none; }
      .foot{ margin-top:16px; text-align:center; font-size:10.5px; color:#797979; line-height:1.5; }
      .estado-tag{ display:inline-block; margin-top:10px; padding:5px 12px; border-radius:999px; font-family:Sora,sans-serif; font-size:11px; font-weight:700; }
      .estado-tag.pend{ background:#F6F5F6; color:#797979; }
      .estado-tag.conf{ background:#E2E9CC; color:#5A7602; }
      @media (max-width:1023px){
        .pagina{padding:0; align-items:stretch;}
        .shell{ flex-direction:column; height:auto; max-height:none; border-radius:0; box-shadow:none; }
        .marca{flex:none; min-height:440px;}
        .marca .contenido{padding:28px 24px;}
        .marca h2{font-size:28px;}
        .marca .sub{font-size:14.5px; margin-top:14px;}
        .stats{gap:22px;}
        .stats .n{font-size:22px; white-space:nowrap;}
        .stats .l{white-space:nowrap; font-size:11.5px;}
        .puntos{right:24px; top:24px;}
        .marquee .tit{padding:0 24px;}
        .app{width:100%; height:auto; overflow:visible;}
      }
    </style>
    ...
    <div class="pagina">
      <div class="shell">
        ...
        <aside class="marca">
          <img class="slide activa" src="pileta.jpg" alt="">
          <img class="slide" src="entrada.jpg" alt="">
          <img class="slide arriba" src="jardin.jpg" alt="">
          <div class="velo"></div>
          <div class="puntos"><span class="activa"></span><span></span><span></span></div>

          <div class="contenido">
            <div>
              <span class="etiqueta"><span class="dot"></span>Propuesta personalizada</span>
              <h2>Preparado para ${esc(c.cliente)}</h2>
              <p class="sub">Cotizá tu lote en minutos, descubrí todas las facilidades de pago y recibí una propuesta a tu medida, con condiciones especiales para copropietarios.</p>
            </div>
            <div class="stats">
              <div><p class="n">+10</p><p class="l">años de desarrollo</p></div>
              <div><p class="n">+500</p><p class="l">familias viviendo</p></div>
              <div><p class="n">+200</p><p class="l">hectáreas</p></div>
            </div>
          </div>

          <div class="marquee">
            <p class="tit">Un desarrollo de</p>
            <div class="pista">
              <ul>
                <li><span class="nombre">Grupo Roda</span></li><li><span class="nombre">·</span></li>
                <li><span class="nombre">Grupo Kinn</span></li><li><span class="nombre">·</span></li>
                <li><span class="nombre">Urubó Village</span></li><li><span class="nombre">·</span></li>
                <li><span class="nombre">Grupo Roda</span></li><li><span class="nombre">·</span></li>
                <li><span class="nombre">Grupo Kinn</span></li><li><span class="nombre">·</span></li>
                <li><span class="nombre">Urubó Village</span></li><li><span class="nombre">·</span></li>
              </ul>
        ...
        <div class="app">
          <div class="res">
            <div class="cab"><div class="uv-mark">UV</div><span>Tu cotización</span></div>

            <div class="chip-uv">
              <div class="uv-mark">UV</div>
              <p><b>Cotización especial · Urubó Village</b>Beneficio exclusivo para copropietarios.</p>
            </div>

            <h1 class="tit">Terreno N° ${esc(c.terreno)}</h1>
            <span class="estado-tag ${estadoReal==='confirmado'?'conf':'pend'}">${estadoReal==='confirmado' ? '✓ Confirmada por tu asesor' : 'Pendiente de confirmación'}</span>

            <div class="kit">
              <p class="n">${esc(c.modalidad)} · Categoría ${esc(c.categoria||'—')}</p>
              <p class="k num">${(c.superficie||0).toLocaleString('es-BO')} <small>m² de terreno</small></p>
            </div>

            <div class="grid2">
              <div class="caja">
                <p class="l">Aporte inicial <small>(${c.pctAporte}%)</small></p>
                <p class="v num">${fmt(c.montoAporte)}</p>
              </div>
              <div class="caja">
                <p class="l">Reserva <small>para separar el lote</small></p>
                <p class="v num">${fmt(c.reserva)}</p>
              </div>
            </div>

            <p class="sec">Tu precio</p>
            <div class="grid2" style="margin-top:8px;">
              <div class="fact">
                <div class="h" style="background:#2A736C;">Precio de lista</div>
                <div class="b">
                  <div><span>Terreno</span><span>${esc(c.terreno)}</span></div>
                  <div><span>Categoría</span><span>${esc(c.categoria||'—')}</span></div>
                  <div class="sep"></div>
                  <div class="tot"><span>Lista</span><b class="num" style="color:#2A736C;">${fmt(c.precioLista)}</b></div>
                </div>
                <div class="pie">${hayDescuento ? 'Precio de referencia' : 'Sin descuento'}</div>
              </div>
              <div class="fact">
                <div class="h" style="background:#D62828;">Tu precio final</div>
                <div class="b">
                  <div><span>Modalidad</span><span>${esc(c.modalidad)}</span></div>
                  <div><span>Descuento</span><span class="num">${fmtPct(c.descuentoPct)}%</span></div>
                  <div class="sep"></div>
                  <div class="tot"><span>Final</span><b class="num" style="color:#D62828;">${fmt(c.precioFinal)}</b></div>
                </div>
                <div class="pie ${hayDescuento?'rojo':''}">${hayDescuento ? (fmtPct(c.descuentoPct)+'% menos') : 'Precio de lista'}</div>
              </div>
            </div>
            <p class="nota">Cálculo preliminar en base a tu terreno y modalidad declarada.</p>

            <div class="precio">
              ${hayDescuento ? `<p class="tach num">${fmt(c.precioLista)}</p>` : ''}
              <div class="fila">
                <p class="usd num">${fmt(c.precioFinal)}</p>
                ${hayDescuento ? `<span class="badge-desc">−${fmtPct(c.descuentoPct)}% copropietarios Urubó Village</span>` : ''}
              </div>
              <p class="cuota">o ${c.nroCuotas} cuotas de <b class="num">${fmt(c.montoCuota)}/mes</b> · <span class="llave">Saldo financiado</span></p>
            </div>

            <p class="sec">Qué incluye tu terreno</p>
            <div class="equipos">
              <div class="eq">
                <div class="img"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6"/></svg></div>
                <div>
                  <p class="t">Lote residencial en Urubó Village</p>
                  <span class="badge">✓ Desarrollo de Grupo Roda</span>
                  <p class="d">Terreno ${esc(c.terreno)} · Categoría ${esc(c.categoria||'—')}</p>
                  <ul>
                    <li>${iconoCheck}<span><b>Acceso directo</b> al Country Club</span></li>
                    <li>${iconoCheck}<span><b>Titularidad en regla</b> y entrega inmediata</span></li>
                    <li>${iconoCheck}<span><b>Financiamiento directo</b> sin bancos</span></li>
                    ${c.conAccion==='SI' ? `<li>${iconoCheck}<span><b>Acción del Country Club incluida</b> — ${fmt(c.accionPrecio)}</span></li>` : ''}
                  </ul>
                </div>
              </div>
              <div class="eq">
                <div class="img"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20M2 12h20M4.9 4.9l14.2 14.2M19.1 4.9L4.9 19.1"/></svg></div>
                <div>
                  <p class="t">Country Club Urubó Village</p>
                  <p class="d">Vida de club incluida para toda la familia</p>
                  <ul>
                    <li>${iconoCheck}<span><b>Piscinas, canchas de tenis y fútbol</b></span></li>
                    <li>${iconoCheck}<span><b>Gimnasio y espacios sociales</b></span></li>
                    <li>${iconoCheck}<span><b>Seguridad perimetral 24/7</b></span></li>
                  </ul>
                </div>
              </div>
            </div>

            <div class="aviso">Precio en USD. <strong>No incluye costos de escrituración.</strong> Cotización preliminar sujeta a confirmación de tu asesor.</div>
            <p class="cierre">Tu asesor, ${esc(c.asesorNombre||'un ejecutivo de Urubó Village')}, se va a comunicar a la brevedad para continuar con el proceso.</p>

            <div class="btn wa ${waNumero?'':'desactivado'}">
              <a href="${waLink}" target="_blank">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.52 3.48A11.9 11.9 0 0 0 12 0C5.37 0 0 5.37 0 12c0 2.11.55 4.17 1.6 6L0 24l6.18-1.62a11.9 11.9 0 0 0 5.82 1.5h.01c6.63 0 12-5.37 12-12 0-3.21-1.25-6.22-3.49-8.4zM12 21.8h-.01a9.85 9.85 0 0 1-5.02-1.37l-.36-.21-3.66.96.98-3.57-.24-.37A9.85 9.85 0 0 1 2.2 12c0-5.41 4.4-9.8 9.8-9.8 2.62 0 5.08 1.02 6.93 2.87A9.74 9.74 0 0 1 21.8 12c0 5.41-4.4 9.8-9.8 9.8zm5.38-7.34c-.29-.15-1.74-.86-2-.96-.27-.1-.46-.15-.66.15-.19.29-.76.96-.93 1.15-.17.2-.34.22-.63.07-.29-.15-1.23-.45-2.34-1.44-.87-.77-1.45-1.73-1.62-2.02-.17-.29-.02-.45.13-.6.13-.13.29-.34.44-.51.15-.17.2-.29.29-.49.1-.2.05-.37-.02-.51-.07-.15-.66-1.6-.9-2.19-.24-.57-.48-.5-.66-.5-.17 0-.36-.02-.56-.02-.2 0-.51.07-.78.37-.27.29-1.02 1-1.02 2.44 0 1.44 1.05 2.83 1.2 3.02.15.2 2.06 3.15 4.99 4.42.7.3 1.24.48 1.67.62.7.22 1.34.19 1.85.12.56-.08 1.74-.71 1.99-1.4.24-.68.24-1.27.17-1.4-.07-.13-.27-.2-.56-.35z"/></svg>
                Hablar con tu asesor por WhatsApp
              </a>
            </div>

            <p class="foot">Cotización válida por ${c.vigenciaDias} días · Sujeta a disponibilidad del lote</p>
          </div>
        </div>
    ...
    <script>
      (function () {
        var slides = document.querySelectorAll('.marca .slide');
        var puntos = document.querySelectorAll('.puntos span');
        var i = 0;
        setInterval(function () {
          slides[i].classList.remove('activa'); puntos[i].classList.remove('activa');
          i = (i + 1) % slides.length;
          slides[i].classList.add('activa'); puntos[i].classList.add('activa');
        }, 5000);
      })();
    </script>`;
}

function _cotizPublicaErrorHtml(mensaje) {
  return `<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;color:#494C4A;font-family:sans-serif;">
    <div style="max-width:380px;">
      <div style="font-size:40px;margin-bottom:16px;">⚠️</div>
      <p style="font-size:16px;">${esc(mensaje)}</p>
    </div>
  </div>`;
}

/* ═══════════════════════════════════════════
   SIMULADOR DE CUOTAS — herramienta libre para vendedores, no guarda nada.
   El descuento sale de la política real (Modalidad × Categoría), no se tipea a mano.
═══════════════════════════════════════════ */
let simTerrenoElegido = null;

function buscarTerrenoSimulador() {
  const term = document.getElementById('sim-buscar-terreno').value.trim().toLowerCase();
  const cont = document.getElementById('sim-resultados-terreno');
  if (!term) { cont.innerHTML = ''; return; }
  const resultados = todosLotes.filter(l => String(_terrenoDeLote(l)).toLowerCase().includes(term)).slice(0,10);
  if (!resultados.length) { cont.innerHTML = '<div class="empty-state">Sin resultados.</div>'; return; }
  window._simLotesVisibles = resultados;
  cont.innerHTML = resultados.map(l => `
    <div onclick="seleccionarTerrenoSimulador('${l._key}')" style="padding:10px 14px;border:1.5px solid var(--line);border-radius:var(--r-sm);margin-bottom:6px;cursor:pointer;display:flex;justify-content:space-between;">
      <span><strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')}</span>
      <span style="color:var(--gris);font-size:var(--fs-sm);">${l.metraje||0} m² · Cat. ${esc(l.categoria||'—')}</span>
    </div>`).join('');
}

function seleccionarTerrenoSimulador(key) {
  const l = (window._simLotesVisibles||[]).find(x => x._key === key);
  if (!l) return;
  simTerrenoElegido = l;
  simPagosContado = [];
  const inpDescuento = document.getElementById('sim-descuento-aplicar');
  if (inpDescuento) delete inpDescuento.dataset.maxSugerido;
  document.getElementById('sim-resultados-terreno').innerHTML = '';
  document.getElementById('sim-buscar-terreno').value = _terrenoDeLote(l);
  document.getElementById('sim-terreno-elegido').style.display = '';
  document.getElementById('sim-terreno-elegido').innerHTML = `<strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')} · Categoría ${esc(l.categoria||'—')} · ${(l.metraje||0).toLocaleString('es-BO')} m²`;
  document.getElementById('sim-precio').value = l.precio || '';
  actualizarSimulador();
}

let simPagosContado = []; // líneas de pago dentro del plazo de días, cuando la modalidad es Contado

/* ── Sub-navegación dentro de Cotizaciones (vendedor): Simulador / Nueva / Mis cotizaciones ── */
function cambiarSubTabCotizVendedor(id) {
  const modos = { simulador:'cotiz-sub-simulador', generar:'cotiz-sub-generar', mias:'cotiz-sub-mias' };
  Object.entries(modos).forEach(([k, panelId]) => {
    const panel = document.getElementById(panelId);
    if (panel) panel.style.display = (k === id) ? '' : 'none';
    const btn = document.getElementById('btn-cotizsub-' + k);
    if (btn) {
      btn.style.background = (k === id) ? '#fff' : 'transparent';
      btn.style.boxShadow = (k === id) ? '0 1px 3px rgba(0,0,0,.1)' : 'none';
    }
  });
  if (id === 'mias') renderMisCotizacionesVendedor();
}

function actualizarSimulador(origen) {
  const precioLista = parseFloat(document.getElementById('sim-precio').value) || 0;
  const modalidad = document.getElementById('sim-modalidad').value;
  const categoria = simTerrenoElegido?.categoria || '';
  const f = n => '$' + Math.round(n).toLocaleString('es-BO');
  const redondear2 = n => Math.round(n*100)/100;

  // El % máximo sale de la política (no se tipea) — pero el asesor puede aplicar MENOS
  // que el máximo si quiere, cargándolo en "Descuento a aplicar". Nunca más del máximo.
  let pctMax = 0;
  const notaEl = document.getElementById('sim-politica-nota');
  if (modalidad && categoria) {
    const hoy = new Date();
    const nombresMes = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const politica = obtenerPoliticaDescuento(modalidad, categoria, hoy.getFullYear(), nombresMes[hoy.getMonth()]);
    if (politica) {
      pctMax = politica.pctMax;
      notaEl.innerHTML = `✅ Política vigente para <strong>${esc(modalidad)} · Categoría ${esc(categoria)}</strong>: hasta <strong style="color:var(--ok);">${pctMax}% de descuento</strong>.`;
    } else {
      notaEl.innerHTML = `⚠️ No hay política cargada para ${esc(modalidad)} · Categoría ${esc(categoria)} — se cotiza sin descuento.`;
    }
  } else if (!simTerrenoElegido) {
    notaEl.textContent = 'Elegí terreno y modalidad para ver el descuento autorizado.';
  } else if (!modalidad) {
    notaEl.textContent = 'Elegí una modalidad para ver el descuento autorizado.';
  }

  const inpDescuento = document.getElementById('sim-descuento-aplicar');
  const inpPrecioFinal = document.getElementById('sim-precio-final-editable');
  inpDescuento.max = pctMax;
  // La primera vez que aparece un máximo (o cambia), sugerimos el tope como valor de arranque —
  // pero si el asesor ya lo tocó a mano, no se lo pisamos.
  if (inpDescuento.dataset.maxSugerido !== String(pctMax)) {
    inpDescuento.value = pctMax;
    inpDescuento.dataset.maxSugerido = String(pctMax);
    origen = 'pct'; // recién sugerido — calculamos el precio a partir de este %, no al revés
  }

  let pctDescuentoInput, precioFinalBruto;
  if (origen === 'precio' && precioLista > 0) {
    // Cargó el precio final a mano → el % se calcula solo, a partir de ese precio.
    precioFinalBruto = parseFloat(inpPrecioFinal.value) || 0;
    pctDescuentoInput = redondear2((1 - precioFinalBruto/precioLista) * 100);
    inpDescuento.value = pctDescuentoInput;
  } else {
    // Cargó el % (o cambió terreno/modalidad) → el precio final se calcula a partir del %.
    pctDescuentoInput = parseFloat(inpDescuento.value) || 0;
  }

  const avisoEl = document.getElementById('sim-descuento-aviso');
  const seExcede = pctDescuentoInput > pctMax;
  avisoEl.style.display = seExcede ? '' : 'none';
  // Para el cálculo, nunca superamos el máximo autorizado, aunque el campo diga más.
  const pctDescuento = redondear2(Math.min(pctDescuentoInput, pctMax));

  const precioFinal = redondear2(precioLista * (1 - pctDescuento/100));
  if (origen !== 'precio' || seExcede) inpPrecioFinal.value = precioFinal ? precioFinal.toFixed(2) : '';

  // Contado se paga 100% dentro de un plazo en DÍAS (en los pagos que hagan falta) —
  // no tiene sentido de "cuotas mensuales", así que cambia toda la interacción.
  const esContado = modalidad === 'Contado';
  document.getElementById('sim-modo-contado').style.display = esContado ? '' : 'none';
  document.getElementById('sim-modo-cuotas').style.display = esContado ? 'none' : '';

  if (esContado) {
    if (!simPagosContado.length) simPagosContado = [{ dias:0, monto: precioFinal }];
    renderPagosSimuladorContado(precioFinal, pctDescuento);
    return;
  }

  // Resto de modalidades: aporte inicial + cuotas, con el plazo MÁXIMO real cargado en
  // Panel de Control para esta modalidad — no un número fijo igual para todas.
  const plazoInfo = modalidad ? obtenerPlazoModalidad(modalidad) : null;
  const selPlazo = document.getElementById('sim-plazo');
  const avisoSinConfig = document.getElementById('sim-plazo-sin-config');
  if (plazoInfo) {
    selPlazo.max = plazoInfo.plazoMax;
    if (selPlazo.max != selPlazo.dataset.maxAplicado) { selPlazo.value = plazoInfo.plazoMax; selPlazo.dataset.maxAplicado = selPlazo.max; }
    avisoSinConfig.style.display = 'none';
  } else if (modalidad) {
    selPlazo.max = 60; // rango provisorio, solo mientras no se cargue el plazo real
    avisoSinConfig.style.display = '';
  } else {
    avisoSinConfig.style.display = 'none';
  }

  const pctEntrega = parseInt(document.getElementById('sim-entrega').value);
  const plazo = parseInt(selPlazo.value);
  const entrega = precioFinal * pctEntrega/100;
  const saldo = precioFinal - entrega;
  const cuota = plazo ? saldo/plazo : 0;

  setText('sim-entrega-val', pctEntrega + '% · ' + f(entrega));
  setText('sim-plazo-val', plazo + ' cuotas' + (plazoInfo ? ' (máx. ' + plazoInfo.plazoMax + ')' : ''));
  setText('sim-cuota', f(cuota));
  setText('sim-sub', 'en ' + plazo + ' cuotas · aporte ' + pctEntrega + '%' + (pctDescuento ? ' · ' + pctDescuento + '% dcto.' : ''));
  setText('sim-mini-final', f(precioFinal));
  setText('sim-mini-entrega', f(entrega));
  setText('sim-mini-saldo', f(saldo));
  setText('sim-mini-cuota', f(cuota));
}

function agregarPagoSimuladorContado() {
  simPagosContado.push({ dias: 0, monto: 0 });
  actualizarSimulador();
}

function quitarPagoSimuladorContado(i) {
  simPagosContado.splice(i,1);
  actualizarSimulador();
}

function renderPagosSimuladorContado(precioFinal, pctDescuento) {
  const modalidad = document.getElementById('sim-modalidad').value;
  const plazoInfo = obtenerPlazoModalidad(modalidad);
  const plazoMax = plazoInfo ? plazoInfo.plazoMax : null;

  document.getElementById('sim-contado-plazo-nota').innerHTML = plazoMax
    ? `El cliente completa el pago en <strong>máximo ${plazoMax} días</strong>, en los pagos que necesite (no tiene que ser todo junto).`
    : `⚠️ No hay plazo máximo cargado para Contado en Panel de Control — completá los pagos con criterio, sin límite validado.`;

  const f = n => '$' + Math.round(n).toLocaleString('es-BO');
  const cont = document.getElementById('sim-contado-lista');
  cont.innerHTML = simPagosContado.map((p,i) => `
    <div style="display:grid;grid-template-columns:1fr 1fr 34px;gap:8px;margin-bottom:6px;align-items:center;">
      <div><label style="font-size:var(--fs-xs);">Día (desde hoy)</label><input type="number" min="0" value="${p.dias}" onchange="simPagosContado[${i}].dias=parseInt(this.value)||0;actualizarSimulador();"/></div>
      <div><label style="font-size:var(--fs-xs);">Monto ($)</label><input type="number" min="0" step="0.01" value="${p.monto||''}" onchange="simPagosContado[${i}].monto=parseFloat(this.value)||0;actualizarSimulador();"/></div>
      <button type="button" onclick="quitarPagoSimuladorContado(${i})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:18px;margin-top:14px;">×</button>
    </div>`).join('');

  const totalCargado = simPagosContado.reduce((s,p)=>s+(p.monto||0),0);
  const diaMasLargo = Math.max(0, ...simPagosContado.map(p=>p.dias||0));
  const diff = precioFinal - totalCargado;
  const cuadra = Math.abs(diff) < 0.5;
  const pasaDelPlazo = plazoMax && diaMasLargo > plazoMax;

  document.getElementById('sim-contado-resumen').innerHTML = `
    <div style="font-size:var(--fs-xs);color:var(--gris);text-transform:uppercase;letter-spacing:.08em;">Precio con descuento</div>
    <div style="font-family:'Space Grotesk',sans-serif;font-size:30px;color:var(--ok);margin:4px 0;">${f(precioFinal)}${pctDescuento?` <span style="font-size:16px;color:var(--gris);">(${pctDescuento}% dcto.)</span>`:''}</div>
    <div style="font-size:var(--fs-sm);margin-top:8px;color:${cuadra?'var(--ok)':'var(--danger)'};font-weight:600;">
      ${cuadra ? '✅ Los pagos suman el total.' : (diff>0 ? `⚠️ Faltan ${f(diff)} para completar el precio.` : `⚠️ Te pasaste por ${f(-diff)}.`)}
    </div>
    ${pasaDelPlazo ? `<div style="font-size:var(--fs-sm);color:var(--danger);font-weight:600;margin-top:4px;">⚠️ Hay un pago en el día ${diaMasLargo}, y el máximo autorizado es ${plazoMax} días.</div>` : ''}
  `;
}

// Toma exactamente lo que se armó en el simulador (terreno, modalidad, descuento REAL
// aplicado, plan de pago) y lo pasa al formulario de "Generar cotización" — así el
// descuento con el que el asesor cierra queda detallado en la cotización, sin re-tipear nada.
function generarCotizacionDesdeSimulador() {
  if (!simTerrenoElegido) { toastErr('Elegí un terreno primero.'); return; }
  const modalidad = document.getElementById('sim-modalidad').value;
  if (!modalidad) { toastErr('Elegí una modalidad primero.'); return; }

  const precioLista = parseFloat(document.getElementById('sim-precio').value) || 0;
  const pctDescuento = Math.round(Math.min(parseFloat(document.getElementById('sim-descuento-aplicar').value) || 0,
                                 parseFloat(document.getElementById('sim-descuento-aplicar').max) || 0) * 100) / 100;

  // Repetimos la búsqueda de terreno en el formulario de cotización, para que quede
  // con su propio estado (mismo patrón que "Generar cotización" ya usa).
  cotizUsuarioLoteElegido = simTerrenoElegido;
  document.getElementById('uc-buscar-terreno').value = _terrenoDeLote(simTerrenoElegido);
  document.getElementById('uc-resultados-terreno').innerHTML = '';
  document.getElementById('uc-terreno-elegido').style.display = '';
  document.getElementById('uc-terreno-elegido').innerHTML = `<strong>${esc(_terrenoDeLote(simTerrenoElegido))}</strong> — Módulo ${esc(simTerrenoElegido.modulo||'—')} · Categoría ${esc(simTerrenoElegido.categoria||'—')} · ${(simTerrenoElegido.metraje||0).toLocaleString('es-BO')} m²`;
  document.getElementById('uc-form-resto').style.display = '';

  document.getElementById('uc-precio-lista').value = precioLista;
  document.getElementById('uc-descuento-pct').value = pctDescuento;
  document.getElementById('uc-modalidad').value = modalidad;

  if (modalidad === 'Contado') {
    // El plan "por días" del simulador se pasa como cuotas con fecha real (hoy + N días),
    // conservando exactamente los montos que armó el asesor.
    document.getElementById('uc-pct-aporte').value = 0;
    document.getElementById('uc-reserva').value = 0;
    document.getElementById('uc-nro-cuotas').value = simPagosContado.length || 1;
  } else {
    document.getElementById('uc-pct-aporte').value = document.getElementById('sim-entrega').value;
    document.getElementById('uc-nro-cuotas').value = document.getElementById('sim-plazo').value;
  }

  recalcularCotizacionUsuario();
  toastOk('Datos del simulador cargados — completá el resto y generá el link.');
  cambiarSubTabCotizVendedor('generar');
  document.getElementById('uc-form-resto').scrollIntoView({behavior:'smooth', block:'start'});
}

/* ═══════════════════════════════════════════
   COTIZACIONES — versión para vendedores (panel usuario), mismo motor que el admin
   pero con sus propios IDs (uc-) para no chocar con el panel admin.
═══════════════════════════════════════════ */
let cotizUsuarioLoteElegido = null;

function buscarTerrenoCotizacionUsuario() {
  const term = document.getElementById('uc-buscar-terreno').value.trim().toLowerCase();
  const cont = document.getElementById('uc-resultados-terreno');
  if (!term) { cont.innerHTML = ''; return; }
  const resultados = todosLotes.filter(l => String(_terrenoDeLote(l)).toLowerCase().includes(term)).slice(0,10);
  if (!resultados.length) { cont.innerHTML = '<div class="empty-state">Sin resultados.</div>'; return; }
  window._ucLotesVisibles = resultados;
  cont.innerHTML = resultados.map(l => `
    <div onclick="seleccionarTerrenoCotizacionUsuario('${l._key}')" style="padding:10px 14px;border:1.5px solid var(--line);border-radius:var(--r-sm);margin-bottom:6px;cursor:pointer;display:flex;justify-content:space-between;">
      <span><strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')}</span>
      <span style="color:var(--gris);font-size:var(--fs-sm);">${l.metraje||0} m² · ${l.estado||'—'}</span>
    </div>`).join('');
}

function seleccionarTerrenoCotizacionUsuario(key) {
  const l = (window._ucLotesVisibles||[]).find(x => x._key === key);
  if (!l) return;
  cotizUsuarioLoteElegido = l;
  document.getElementById('uc-resultados-terreno').innerHTML = '';
  document.getElementById('uc-buscar-terreno').value = _terrenoDeLote(l);
  document.getElementById('uc-terreno-elegido').style.display = '';
  document.getElementById('uc-terreno-elegido').innerHTML = `
    <strong>${esc(_terrenoDeLote(l))}</strong> — Módulo ${esc(l.modulo||'—')} · Categoría ${esc(l.categoria||'—')} · ${(l.metraje||0).toLocaleString('es-BO')} m²
    ${l.estado && l.estado!=='Disponible' ? `<br><span style="color:var(--warn);font-weight:600;">⚠️ Figura como "${esc(l.estado)}" en Inventario — verificá antes de cotizar.</span>` : ''}`;
  document.getElementById('uc-precio-lista').value = l.precio || '';
  document.getElementById('uc-form-resto').style.display = '';
  recalcularCotizacionUsuario();
}

function toggleAccionCotizacionUsuario() {
  const si = document.getElementById('uc-con-accion').value === 'SI';
  document.getElementById('uc-accion-precio-cont').style.display = si ? '' : 'none';
}

function recalcularCotizacionUsuario() {
  const precioLista = parseFloat(document.getElementById('uc-precio-lista').value) || 0;
  const descPct = parseFloat(document.getElementById('uc-descuento-pct').value) || 0;
  const precioFinal = precioLista * (1 - descPct/100);
  document.getElementById('uc-precio-final').value = precioFinal ? precioFinal.toFixed(2) : '';

  const reserva = parseFloat(document.getElementById('uc-reserva').value) || 0;
  const pctAporte = parseFloat(document.getElementById('uc-pct-aporte').value) || 0;
  const montoAporte = precioFinal * (pctAporte/100);
  const montoSaldo = precioFinal - montoAporte;
  const nCuotas = parseInt(document.getElementById('uc-nro-cuotas').value) || 1;
  const montoCuota = nCuotas ? montoSaldo/nCuotas : 0;

  document.getElementById('uc-monto-aporte').value = montoAporte ? montoAporte.toFixed(2) : '';
  document.getElementById('uc-monto-saldo').value = montoSaldo ? montoSaldo.toFixed(2) : '';
  document.getElementById('uc-monto-cuota').value = montoCuota ? montoCuota.toFixed(2) : '';
}

async function guardarYGenerarCotizacionUsuario() {
  if (!cotizUsuarioLoteElegido) { toastErr('Elegí un terreno primero.'); return; }
  const cliente = document.getElementById('uc-cliente').value.trim();
  if (!cliente) { toastErr('Cargá el nombre del cliente.'); return; }

  const l = cotizUsuarioLoteElegido;
  const vigenciaDias = parseInt(document.getElementById('uc-vigencia-dias').value) || 5;
  const fechaVencimiento = Date.now() + vigenciaDias*24*60*60*1000;

  const data = {
    cliente, celular: document.getElementById('uc-celular').value, correo: document.getElementById('uc-correo').value,
    terreno: _terrenoDeLote(l), modulo: l.modulo||'', categoria: l.categoria||'', superficie: l.metraje||0,
    precioLista: parseFloat(document.getElementById('uc-precio-lista').value)||0,
    descuentoPct: Math.round((parseFloat(document.getElementById('uc-descuento-pct').value)||0)*100)/100,
    precioFinal: parseFloat(document.getElementById('uc-precio-final').value)||0,
    conAccion: document.getElementById('uc-con-accion').value,
    accionPrecio: parseFloat(document.getElementById('uc-accion-precio').value)||0,
    modalidad: document.getElementById('uc-modalidad').value,
    reserva: parseFloat(document.getElementById('uc-reserva').value)||0,
    pctAporte: parseFloat(document.getElementById('uc-pct-aporte').value)||0,
    montoAporte: parseFloat(document.getElementById('uc-monto-aporte').value)||0,
    montoSaldo: parseFloat(document.getElementById('uc-monto-saldo').value)||0,
    nroCuotas: parseInt(document.getElementById('uc-nro-cuotas').value)||0,
    montoCuota: parseFloat(document.getElementById('uc-monto-cuota').value)||0,
    vigenciaDias, fechaVencimiento,
    asesorCelular: document.getElementById('uc-asesor-celular').value,
    estado: 'pendiente',
    asesorNombre: asesorActual?.nombre || 'Asesor',
  };

  const res = await window._fbPushCotizacion(data);
  const link = window.location.origin + window.location.pathname + '?cotizacion=' + res.key;
  document.getElementById('uc-link-generado').style.display = '';
  document.getElementById('uc-link-texto').value = link;
  toastOk('Cotización guardada.');
}

function copiarLinkCotizacionUsuario() {
  const input = document.getElementById('uc-link-texto');
  input.select();
  navigator.clipboard?.writeText(input.value);
  toastOk('Link copiado.');
}

// Se llama desde "Mis Registros" — precarga el nombre del cliente y, si ya tenía un
// terreno de interés cargado, lo busca solo, para no escribir todo de nuevo.
function abrirCotizacionDesdeRegistro(registroKey) {
  const r = misRegs.find(x => x._key === registroKey);
  if (!r) return;
  switchUserTab('cotizaciones');
  setTimeout(() => {
    document.getElementById('uc-cliente').value = r.nombre || '';
    if (r.terrenoInteres) {
      document.getElementById('uc-buscar-terreno').value = r.terrenoInteres;
      buscarTerrenoCotizacionUsuario();
    }
    document.getElementById('uc-cliente').scrollIntoView({behavior:'smooth', block:'center'});
  }, 300);
}

function renderMisCotizacionesVendedor() {
  const tbody = document.getElementById('mis-cotiz-tbody');
  if (!tbody) return;
  const propias = cotizacionesData.filter(c => c.asesorNombre === asesorActual?.nombre);
  if (!propias.length) { tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Todavía no generaste ninguna cotización.</div></td></tr>'; return; }
  const badges = { pendiente:'background:var(--warn-bg);color:var(--warn-ink);', confirmado:'background:var(--ok-bg);color:var(--ok-ink);', vencido:'background:var(--danger-bg);color:var(--danger-ink);' };
  tbody.innerHTML = propias.map(c => {
    const estadoReal = _estadoRealCotizacion(c);
    const link = window.location.origin + window.location.pathname + '?cotizacion=' + c._key;
    return `<tr>
    <td style="font-size:var(--fs-sm);">${new Date(c.ts).toLocaleDateString('es-BO')}</td>
    ...
    <td style="font-weight:600;color:var(--cartera);">$${(c.precioFinal||0).toLocaleString('es-BO')}</td>
    <td><span style="padding:3px 10px;border-radius:999px;font-size:var(--fs-xs);font-weight:600;${badges[estadoReal]}">${estadoReal}</span></td>
    <td><button class="btn-clear" style="padding:5px 8px;font-size:var(--fs-sm);" onclick="navigator.clipboard.writeText('${link}');toastOk('Link copiado.');">📋 Copiar</button></td>
  </tr>`;
  }).join('');
}

async function confirmarCotizacionDesdeAdmin(key) {
  const c = cotizacionesData.find(x=>x._key===key);
  if (!c) return;
  if (!confirm('¿Confirmar la cotización de "'+c.cliente+'"? El cliente va a ver "Confirmada" al abrir su link.')) return;
  await window._fbUpdateCotizacion(key, { estado: 'confirmado', fechaConfirmacion: Date.now(), confirmadoPorNombre: asesorActual?.nombre || 'Admin' });
  toastOk('Cotización confirmada.');
}

async function eliminarCotizacion(key) {
  const c = cotizacionesData.find(x=>x._key===key);
  if (!c) return;
  if (!confirm('¿Eliminar la cotización de "'+c.cliente+'"? No se puede deshacer.')) return;
  await window._fbRemoveCotizacion(key);
  toastOk('Cotización eliminada.');
}
...
/* ── Gestión mes a mes: NO un total tapado, el detalle real de cada uno de los 12 meses ── */
function renderGestionMensualVentas() {
  const tbody = document.getElementById('ventas-gestion-mensual-tbody');
  if (!tbody) return;
  setText('ventas-gestion-mensual-label', _labelGestion(ventasFilGestion));

  const nombresMes = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesesGestion = [];
  for (let i = 3; i < 15; i++) mesesGestion.push({ mes: nombresMes[i%12], anio: i<12 ? ventasFilGestion : ventasFilGestion+1 });

  const internos = soloInternos(todosRegs);
  const filas = mesesGestion.map(({mes, anio}) => {
    const delMes = internos.filter(r => r.mes===mes && (!r.ts || new Date(r.ts).getFullYear()===anio));
    const cierres = delMes.filter(r => r.conclusion==='Reserva' || r.huboCierre==='SI');
    const vendido = cierres.reduce((s,r)=>s+(r.precioVenta||r.precio||0),0);
    return { mes, anio, registros: delMes.length, visitas: delMes.length, cierres: cierres.length, vendido };
  });

  const hoy = new Date();
  tbody.innerHTML = filas.map(f => {
    const esFuturo = (f.anio > hoy.getFullYear()) || (f.anio === hoy.getFullYear() && nombresMes.indexOf(f.mes) > hoy.getMonth());
    return `<tr style="${esFuturo?'color:var(--gris);':''}">
      <td style="font-weight:600;">${f.mes} ${f.anio}</td>
      <td>${f.registros}</td>
      <td>${f.visitas}</td>
      <td style="font-weight:600;color:var(--cartera);">${f.cierres}</td>
      <td>${f.vendido ? '$'+Math.round(f.vendido).toLocaleString('es-BO') : (esFuturo?'—':'$0')}</td>
    </tr>`;
  }).join('') + `<tr style="font-weight:700;background:var(--bg-soft);">
    <td>Total gestión</td>
    <td>${filas.reduce((s,f)=>s+f.registros,0)}</td>
    <td>${filas.reduce((s,f)=>s+f.visitas,0)}</td>
    <td>${filas.reduce((s,f)=>s+f.cierres,0)}</td>
    <td>$${Math.round(filas.reduce((s,f)=>s+f.vendido,0)).toLocaleString('es-BO')}</td>
  </tr>`;
}
...
function renderResumenCierres(regsPeriodo, metaC, labelPeriodo) {
  try {
    const cierres = (regsPeriodo||[]).filter(r => r.conclusion === 'Reserva' && r.precioVenta > 0);
...
function renderDetalleCierres() {
  ...
  if (!tbody) return;
  // Usa el mismo filtro global de arriba del panel (mes o gestión completa) — no uno propio separado.
  let cierres = todosRegs.filter(r => r.conclusion === 'Reserva' && r.precioVenta > 0 && _regEnPeriodoVentas(r));

  cierres = cierres.sort((a,b)=>(b.ts||0)-(a.ts||0));

  if (!cierres.length) {
    tbody.innerHTML = '<tr><td colspan="13"><div class="empty-state">Sin cierres para ' + _labelPeriodoVentas() + '.</div></td></tr>';
  ...
  const filasCalc = cierres.map(r => {
    ...
    if (r.terrenoInteres) {
      const buscado = String(r.terrenoInteres).trim().toLowerCase();
      loteActual = todosLotes.find(l => String(_terrenoDeLote(l)).trim().toLowerCase() === buscado);
    }
    if (!metraje && loteActual?.metraje) metraje = loteActual.metraje;
    const categoriaTerreno = loteActual?.categoria || '';
    ...
    return { r, metraje, m2Lista, m2Cierre, categoriaTerreno };
  });

  tbody.innerHTML = filasCalc.map(({r, metraje, m2Lista, m2Cierre, categoriaTerreno}) => {
    ...
    return `<tr${r.esExterno?' style="background:#fdf6e8;"':''}>
      ...
      <td style="text-align:center;font-weight:700;">${esc(categoriaTerreno||'—')}</td>
...
function renderAvancePorAsesor() {
  ...
  if (!tbody) return;
  const porAsesor = {};
  soloInternos(todosRegs).filter(_regEnPeriodoVentas).forEach(r => {
...
function renderClientesEnNegociacion() {
  ...
  if (!tbody) return;
  const pendientes = visitasProgramadasData.filter(v => v.estado === 'pendiente' || v.estado === 'en_proceso')
  ...
  tbody.innerHTML = pendientes.map(v => {
    ...
    return `<tr>
      <td style="font-weight:600;">${esc(v.nombre)} ${v.estado==='en_proceso'?'<span class="badge" style="font-size:var(--fs-xs);background:#fde68a;color:#92400e;">⚠️ A medias</span>':''} ${v.esDevolucion?'<span class="badge" style="font-size:var(--fs-xs);background:#fef3c7;color:#92400e;">↩️ Devuelta</span>':''}</td>
      <td>${esc(v.asesorNombre||'—')}</td>
...
// ── Filtro global del panel de Ventas — Por mes, Varios meses, o Gestión (mes a mes) ──
let ventasFilAnio = new Date().getFullYear();
let ventasFilMes  = PC_MESES[new Date().getMonth()]; // lista fija — antes usaba el idioma del navegador, que podía no coincidir letra por letra con el texto de los <option> y rompía en silencio la búsqueda de la meta
let ventasModoVista = 'mes'; // 'mes' | 'multimes' | 'gestion'
let ventasFilGestion = _gestionActual();
let ventasFilMesesSeleccionados = [ventasFilMes]; // para el modo 'multimes'

function cambiarModoVentas(modo) {
  ventasModoVista = modo;
  const btns = { mes: document.getElementById('btn-vmodo-mes'), multimes: document.getElementById('btn-vmodo-multimes'), gestion: document.getElementById('btn-vmodo-gestion') };
  const selMes = document.getElementById('fil-ventas-mes');
  const selGestion = document.getElementById('fil-ventas-gestion');
  const contMultimes = document.getElementById('fil-ventas-multimes-cont');

  Object.entries(btns).forEach(([k,btn]) => {
    if (!btn) return;
    if (k === modo) { btn.style.background = '#fff'; btn.style.boxShadow = '0 1px 3px rgba(0,0,0,.1)'; }
    else { btn.style.background = 'transparent'; btn.style.boxShadow = 'none'; }
  });

  if (selMes) selMes.style.display = (modo === 'mes') ? '' : 'none';
  if (selGestion) selGestion.style.display = (modo === 'gestion') ? '' : 'none';
  if (contMultimes) contMultimes.style.display = (modo === 'multimes') ? '' : 'none';
  if (modo === 'multimes') poblarChecksMultimes();

  renderDashboard();
}

function poblarChecksMultimes() {
  const cont = document.getElementById('fil-ventas-multimes-checks');
  if (!cont) return;
  const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  cont.innerHTML = meses.map(m => `
    <label style="display:flex;align-items:center;gap:5px;padding:6px 10px;background:var(--fill);border-radius:var(--r-sm);cursor:pointer;font-size:var(--fs-sm);">
      <input type="checkbox" value="${m}" ${ventasFilMesesSeleccionados.includes(m)?'checked':''} style="width:auto;"/> ${m}
    </label>`).join('');
}

function cambiarFiltroVentas() {
  ventasFilAnio = parseInt(document.getElementById('fil-ventas-anio').value);
  ventasFilMes  = document.getElementById('fil-ventas-mes').value;
  const selGestion = document.getElementById('fil-ventas-gestion');
  if (selGestion) ventasFilGestion = parseInt(selGestion.value);
  if (ventasModoVista === 'multimes') {
    ventasFilMesesSeleccionados = Array.from(document.querySelectorAll('#fil-ventas-multimes-checks input:checked')).map(c=>c.value);
    if (!ventasFilMesesSeleccionados.length) { toastErr('Elegí al menos un mes.'); return; }
  }
  renderDashboard();
}

// ¿Este registro cae dentro del período elegido? Sirve para los 3 modos.
function _regEnPeriodoVentas(r) {
  if (ventasModoVista === 'gestion') {
    if (!r.ts) return false;
    return _gestionDeFecha(new Date(r.ts)) === ventasFilGestion;
  }
  if (ventasModoVista === 'multimes') {
    return ventasFilMesesSeleccionados.includes(r.mes) && (!r.ts || new Date(r.ts).getFullYear() === ventasFilAnio);
  }
  return r.mes === ventasFilMes && (!r.ts || new Date(r.ts).getFullYear() === ventasFilAnio);
}

function _labelPeriodoVentas() {
  if (ventasModoVista === 'gestion') return 'Gestión ' + _labelGestion(ventasFilGestion);
  if (ventasModoVista === 'multimes') return ventasFilMesesSeleccionados.join(', ') + ' ' + ventasFilAnio;
  return ventasFilMes + ' ' + ventasFilAnio;
}

function renderDashboard() {
  const mes = ventasFilMes;
  const anioMeta = ventasFilAnio;

  // Poblar el selector de año la primera vez, con los años que realmente existan en los datos
  const selAnio = document.getElementById('fil-ventas-anio');
  if (selAnio && !selAnio.dataset.filled) {
    const anioHoy = new Date().getFullYear();
    const anios = [...new Set(todosRegs.map(r => r.ts ? new Date(r.ts).getFullYear() : null).filter(Boolean))];
    if (!anios.includes(anioHoy)) anios.push(anioHoy);
    anios.sort((a,b)=>b-a);
    selAnio.innerHTML = anios.map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.value = ventasFilAnio;
    selAnio.dataset.filled = '1';
  }
  const selMes = document.getElementById('fil-ventas-mes');
  if (selMes) selMes.value = ventasFilMes;
  const selGestion = document.getElementById('fil-ventas-gestion');
  if (selGestion && !selGestion.dataset.filled) {
    const gestionHoy = _gestionActual();
    const gestiones = [gestionHoy, gestionHoy-1, gestionHoy-2, gestionHoy-3];
    selGestion.innerHTML = gestiones.map(g=>`<option value="${g}">Gestión ${_labelGestion(g)}</option>`).join('');
    selGestion.value = ventasFilGestion;
    selGestion.dataset.filled = '1';
  }

  const regsPeriodoTodos = todosRegs.filter(_regEnPeriodoVentas);
  const vMes = soloInternos(regsPeriodoTodos);
  const todosRegsInternos = soloInternos(todosRegs);
  const labelPeriodo = _labelPeriodoVentas();
  ...
  setText('v-kpi-mes-sub', 'registros en ' + labelPeriodo);
  countUp(document.getElementById('v-kpi-visitas'), vMes.length);

  const contratosReservas = vMes.filter(r =>
  ...
  document.getElementById('kpi-contratos-sub').textContent =
    vMes.length ? `${Math.round(contratosReservas.length/vMes.length*100)}% de ${labelPeriodo}` : '—';

  const conPrecio = vMes.filter(r => r.precio > 0);
  ...
  document.getElementById('kpi-precio-sub').textContent = `${conPrecio.length} registros con precio, ${labelPeriodo}`;

  setText('v-analisis-periodo-label', labelPeriodo);

  // Meta — MISMA fuente que Marketing (metas/{anio}/{mes}/mkt_visitas y mkt_cierres).
  // "Por mes": una sola meta. "Gestión completa": se suman las metas de los 12 meses de la gestión.
  setText('v-meta-mes-label', labelPeriodo);
  const cierresReales = vMes.filter(r => r.conclusion==='Reserva' || r.huboCierre==='SI').length;

  const pintarMeta = (metaV, metaC) => {
  ...
  if (ventasModoVista === 'gestion') {
    const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const mesesGestion = []; // [{mes, anio}] de abril del año elegido a marzo del siguiente
    for (let i = 3; i < 15; i++) mesesGestion.push({ mes: meses[i%12], anio: i<12 ? ventasFilGestion : ventasFilGestion+1 });
    Promise.all(mesesGestion.map(({mes:m,anio:a}) => window._fbGetMetaOnce(a, m, 'mkt_visitas'))).then(vs => {
      Promise.all(mesesGestion.map(({mes:m,anio:a}) => window._fbGetMetaOnce(a, m, 'mkt_cierres'))).then(cs => {
        const metaV = vs.reduce((s,v)=>s+(v||0),0);
        const metaC = cs.reduce((s,v)=>s+(v||0),0);
        pintarMeta(metaV, metaC);
        renderResumenCierres(regsPeriodoTodos, metaC, labelPeriodo);
      });
    });
  } else {
  ...
  renderAvancePorAsesor(mes, anioMeta);
  ...
  renderResumenCierres(regsPeriodoTodos, 0, labelPeriodo); // pinta los conteos ya mismo; la meta se actualiza arriba cuando llegue de Firebase
  renderMetaVsEjecucion();
  const contGestionMensual = document.getElementById('ventas-gestion-mensual-cont');
  if (contGestionMensual) contGestionMensual.style.display = (ventasModoVista === 'gestion') ? '' : 'none';
  if (ventasModoVista === 'gestion') renderGestionMensualVentas();

  // Funnel de conversión (mes filtrado) — 100% conectado en tiempo real:
...
function renderAsesoresAdmin() {
  ...
  const mes = PC_MESES[new Date().getMonth()];
  const cuentas = {};
...
function cambiarPinAdmin() {
  ...
  pinAdminLocal = np;
  window._fbSetAdminPin(np); // centralizado — vale para cualquier computadora, no solo esta
  localStorage.removeItem('uv_planilla_pin'); // ya no hace falta guardarlo localmente
...
function iniciarInventario() {
  ...
  iniciarAutoSyncOrange();
}

// ── Auto-sincronización con Orange cada 10 minutos, mientras el sistema esté abierto ──
// No hace falta darle permiso de escritura a nada externo: simplemente apretamos el mismo
// botón de "Sincronizar" solos, cada 10 minutos, mientras esta pestaña siga abierta.
const ORANGE_AUTOSYNC_MIN = 10;
let _autoSyncOrangeInterval = null;
function iniciarAutoSyncOrange() {
  if (_autoSyncOrangeInterval) return; // ya está corriendo, no duplicar
  _autoSyncOrangeInterval = setInterval(() => {
    sincronizarInventarioOrange(true); // true = silencioso, sin popups molestos
  }, ORANGE_AUTOSYNC_MIN * 60 * 1000);
}

let _autoSyncCarteraOrangeInterval = null;
function iniciarAutoSyncCarteraOrange() {
  if (_autoSyncCarteraOrangeInterval) return; // ya está corriendo, no duplicar
  _autoSyncCarteraOrangeInterval = setInterval(() => {
    cargarDetalleCobrosCartera(true); // silencioso
  }, ORANGE_AUTOSYNC_MIN * 60 * 1000);
}
...
function renderInvStats() {
  ...
  const elBloq = document.getElementById('inv-bloqueados');
  if (elBloq) elBloq.textContent = todosLotes.filter(l=>l.estado==='Bloqueado').length;

  // Desglose por a quién pertenece el lote (UV directo vs. socios como LCK, GIR, etc.)
  const contPert = document.getElementById('inv-pertenencia');
  if (contPert) {
    const porPertenece = {};
    todosLotes.forEach(l => { const k = l.pertenece || 'UV'; porPertenece[k] = (porPertenece[k]||0)+1; });
    const entries = Object.entries(porPertenece).sort((a,b)=>b[1]-a[1]);
    contPert.innerHTML = entries.length ? entries.map(([k,n]) => `
      <div style="display:flex;align-items:center;gap:8px;padding:8px 14px;background:var(--bg-soft);border-radius:999px;">
        <span style="font-weight:700;font-size:var(--fs-base);">${k==='UV'?'🌳':'🤝'} ${esc(k)}</span>
        <span style="font-weight:700;color:var(--cartera);">${n}</span>
      </div>`).join('') : '<span style="color:var(--gris);font-size:var(--fs-sm);">Sin datos — sincronizá con Orange para completarlo.</span>';
  }

  // Desglose por categoría del terreno — Orange no la trae, esto es 100% lo que vos cargues acá.
  // "Sin categoría" destacado en naranja para que se note rápido a cuántos les falta.
  const contCateg = document.getElementById('inv-categorias');
  if (contCateg) {
    const porCateg = {};
    todosLotes.forEach(l => { const k = l.categoria || 'Sin categoría'; porCateg[k] = (porCateg[k]||0)+1; });
    const orden = ['A','B','C','Sin categoría'];
    const entries = Object.entries(porCateg).sort((a,b) => orden.indexOf(a[0]) - orden.indexOf(b[0]));
    contCateg.innerHTML = entries.length ? entries.map(([k,n]) => {
      const sinCateg = k === 'Sin categoría';
      const valorFiltro = sinCateg ? '__sin__' : k;
      return `<div onclick="document.getElementById('fil-inv-categoria').value='${valorFiltro}';renderInventario();document.getElementById('inv-tabla-body').scrollIntoView({behavior:'smooth'});" style="cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 14px;background:${sinCateg?'var(--warn-bg)':'var(--bg-soft)'};border-radius:999px;">
        <span style="font-weight:700;font-size:var(--fs-base);color:${sinCateg?'var(--warn-ink)':'var(--ink-900)'};">${sinCateg?'⚠️':'🏷️'} ${esc(k)}</span>
        <span style="font-weight:700;color:${sinCateg?'var(--warn-ink)':'var(--cartera)'};">${n}</span>
      </div>`;
    }).join('') : '<span style="color:var(--gris);font-size:var(--fs-sm);">Sin datos.</span>';
  }
}
...
function renderInventario() {
  ...
  const filtroPert = document.getElementById('fil-inv-pertenece')?.value || '';
  const filtroCateg = document.getElementById('fil-inv-categoria')?.value || '';
  let datos = todosLotes;
  if (filtroEst)  datos = datos.filter(l=>l.estado===filtroEst);
  if (filtroPert) datos = datos.filter(l=>(l.pertenece||'UV')===filtroPert);
  if (filtroCateg === '__sin__') datos = datos.filter(l=>!l.categoria);
  else if (filtroCateg) datos = datos.filter(l=>l.categoria===filtroCateg);

  // Poblar el filtro de pertenencia dinámicamente según lo que haya en los datos
  const selPert = document.getElementById('fil-inv-pertenece');
  if (selPert && !selPert.dataset.filled) {
    const valores = [...new Set(todosLotes.map(l=>l.pertenece||'UV'))].sort();
    if (valores.length) {
      selPert.innerHTML = '<option value="">Todos los dueños</option>' + valores.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
      selPert.dataset.filled = '1';
    }
  }
  ...
  if (!tb) return;
  if (!datos.length) {
    tb.innerHTML = '<tr><td colspan="8" class="empty-state">Sin lotes registrados todavía.</td></tr>';
    return;
  }
  ...
  const estadoColor = {
    ...
    'Bloqueado':     'background:var(--accent-bg);color:var(--accent-ink);',
  ...
  tb.innerHTML = datos.map(l => {
    ...
    const pertenece = l.pertenece || 'UV';
    return `<tr>
      ...
      <td style="text-align:center;font-size:var(--fs-sm);">${pertenece==='UV'?'🌳 UV':'🤝 '+esc(pertenece)}</td>
...
function mostrarFormLote() {
  ...
  document.getElementById('l-categoria').value = '';
  document.getElementById('l-pertenece').value = '';
...
function editarLote(key) {
  ...
  document.getElementById('l-categoria').value = lote.categoria || '';
  document.getElementById('l-pertenece').value = lote.pertenece || '';
...
/* ═══════════════════════════════════════════
   SINCRONIZACIÓN DE INVENTARIO CON ORANGE (GetLotes)
   Actualiza precio, metraje y estado real de cada lote — no toca la categoría
   ni las observaciones, esas quedan siempre a criterio de admin.
═══════════════════════════════════════════ */
async function sincronizarInventarioOrange(silencioso) {
  const btn = document.getElementById('btn-sync-inventario');
  const setBtn = (txt, disabled) => { if (btn && !silencioso) { btn.textContent = txt; btn.disabled = disabled; } };
  setBtn('🔄 Conectando con Orange...', true);

  try {
    const resp = await fetch(ORANGE_API_URL + '?op=GetLotes');
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    const json = await resp.json();
    if (!json.success || !Array.isArray(json.data)) throw new Error('Respuesta inesperada de Orange');

    // Disponibilidad de Orange → nuestro Estado. "Bloqueado" ahora es su propio estado
    // (antes se mezclaba con "No disponible" — separados para poder contarlos distinto).
    const mapEstado = { 'Vendido':'Vendido', 'Disponible':'Disponible', 'Reservado':'Reservado', 'Bloqueado':'Bloqueado' };

    // Traemos TODOS los lotes, sean de Urubó Village o de socios (LCK, GIR, etc.) —
    // siguen siendo parte del inventario físico, solo que con otro dueño.
    const todosLotesOrange = json.data;
    let creados = 0, actualizados = 0;

    for (let i = 0; i < todosLotesOrange.length; i++) {
      const l = todosLotesOrange[i];
      const terreno = String(l.Terreno || '').trim();
      if (!terreno) continue;

      setBtn(`🔄 Sincronizando ${i+1}/${todosLotesOrange.length}...`, true);

      // La categoría y las observaciones NUNCA se tocan acá — quedan siempre a cargo tuyo,
      // se combinan con los datos de Orange en vez de pisarse.
      const data = {
        terreno,
        metraje:   parseFloat(l.Metraje) || 0,
        precio:    parseFloat(l.PrecioLista) || 0,
        precioM2:  parseFloat(l.PrecioM2Lista) || 0,
        estado:    mapEstado[l.Disponibilidad] || 'Disponible',
        modulo:    l.Modulo || '',
        pertenece: l.Pertenece || 'UV',
        syncOrangeTs: Date.now(),
      };

      const existente = todosLotes.find(x => String(_terrenoDeLote(x)).trim().toLowerCase() === terreno.toLowerCase());
      if (existente) {
        await window._fbUpdateLote(existente._key, data);
        actualizados++;
      } else {
        await window._fbAddLote({ ...data, categoria:'', obs:'', updatedAt: new Date().toLocaleString('es-BO') });
        creados++;
      }
    }

    setBtn('🔄 Sincronizar con Orange', false);
    setText('inv-ultima-sync', 'Última sincronización: ' + new Date().toLocaleString('es-BO') + (silencioso ? ' (automática)' : ' (manual)'));
    if (!silencioso) toastOk(`Inventario sincronizado: ${creados} lotes nuevos, ${actualizados} actualizados.`);
  } catch (e) {
    setBtn('🔄 Sincronizar con Orange', false);
    if (!silencioso) toastErr('No se pudo sincronizar: ' + e.message);
  }
}
...
async function guardarLote() {
  ...
  const data = {
    ...
    pertenece:    document.getElementById('l-pertenece').value.trim() || 'UV',
...
/* ═══════════════════════════════════════════
   RESUMEN EJECUTIVO — storytelling: Qué pasó → Por qué → Qué hacemos → Impacto
   Compara este mes vs el anterior y busca automáticamente la causa principal.
═══════════════════════════════════════════ */
async function renderResumenEjecutivo(mesC, anioC) {
  const nombresMes = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const hoy = new Date();
  const dAnt = new Date(hoy.getFullYear(), hoy.getMonth()-1, 1);
  const mesAnt = nombresMes[dAnt.getMonth()];
  const anioAnt = dAnt.getFullYear();

  setText('re-mes-label', mesC + ' ' + anioC);

  const internos = soloInternos(todosRegs);
  const cierresEste = internos.filter(r => r.mes===mesC && (r.conclusion==='Reserva' || r.huboCierre==='SI'));
  const cierresAnt  = internos.filter(r => r.mes===mesAnt && (r.conclusion==='Reserva' || r.huboCierre==='SI'));
  const val = r => r.precioVenta || r.precio || 0;

  const totalEste = cierresEste.reduce((s,r)=>s+val(r),0);
  const totalAnt  = cierresAnt.reduce((s,r)=>s+val(r),0);
  const deltaPct = totalAnt ? Math.round((totalEste-totalAnt)/totalAnt*100) : null;
  const subio = deltaPct!==null && deltaPct >= 0;

  // ── PASO 1: ¿Qué pasó? ──
  const el1 = document.getElementById('re-que-paso');
  if (el1) {
    el1.innerHTML = `<div class="re-big">$${Math.round(totalEste).toLocaleString('es-BO')}
      <small>${cierresEste.length} cierre${cierresEste.length!==1?'s':''} este mes ·
        ${deltaPct===null ? 'sin datos del mes anterior' : `<span class="${subio?'re-delta-up':'re-delta-down'}">${subio?'↑':'↓'} ${Math.abs(deltaPct)}%</span> vs ${mesAnt}`}
      </small></div>`;
  }

  // ── PASO 2: ¿Por qué pasó? — buscamos quién explica más el cambio (asesor con mayor variación) ──
  function agruparVentas(regs) {
    const g = {};
    regs.forEach(r => { const k = r.asesorNombre||'—'; g[k] = (g[k]||0) + val(r); });
    return g;
  }
  const porAsesorEste = agruparVentas(cierresEste);
  const porAsesorAnt  = agruparVentas(cierresAnt);
  const asesoresSet = new Set([...Object.keys(porAsesorEste), ...Object.keys(porAsesorAnt)]);
  let causaAsesor = null, causaDelta = 0;
  asesoresSet.forEach(a => {
    const d = (porAsesorEste[a]||0) - (porAsesorAnt[a]||0);
    if (Math.abs(d) > Math.abs(causaDelta)) { causaDelta = d; causaAsesor = a; }
  });

  const el2 = document.getElementById('re-por-que');
  if (el2) {
    if (causaAsesor && Math.abs(causaDelta) > 0) {
      const pctExplicado = (deltaPct!==null && (totalEste-totalAnt)) ? Math.round(Math.abs(causaDelta)/Math.abs(totalEste-totalAnt)*100) : null;
      el2.innerHTML = `<div class="re-big" style="font-size:18px;">${esc(causaAsesor)}
        <small><span class="${causaDelta>=0?'re-delta-up':'re-delta-down'}">${causaDelta>=0?'+':'-'}$${Math.abs(Math.round(causaDelta)).toLocaleString('es-BO')}</span>
        ${pctExplicado!==null ? ' · explica ~'+pctExplicado+'% del cambio' : ''}</small></div>`;
    } else {
      el2.innerHTML = `<div class="re-empty">Sin variación relevante para atribuir a un asesor puntual.</div>`;
    }
  }

  // ── PASO 3: ¿Qué hacemos ahora? — reglas simples sobre datos reales ──
  const sugerencias = [];
  if (deltaPct !== null && deltaPct <= -15) {
    sugerencias.push(`Ventas cayeron ${Math.abs(deltaPct)}% — revisar seguimiento de ${esc(causaAsesor||'el equipo')} esta semana.`);
  }
  const descPcts = cierresEste.filter(r=>r.descuentoPct).map(r=>r.descuentoPct);
  const descProm = descPcts.length ? descPcts.reduce((s,v)=>s+v,0)/descPcts.length : 0;
  if (descProm >= 10) {
    sugerencias.push(`Descuento promedio del mes en ${descProm.toFixed(1)}% — revisar la política en Panel de Control.`);
  }
  const porAsesorConv = {};
  internos.filter(r=>r.mes===mesC).forEach(r => {
    const k = r.asesorNombre||'—'; (porAsesorConv[k] ||= {tot:0,cierres:0});
    porAsesorConv[k].tot++;
    if (r.conclusion==='Reserva'||r.huboCierre==='SI') porAsesorConv[k].cierres++;
  });
  Object.entries(porAsesorConv).forEach(([a,d]) => {
    if (d.tot >= 4 && d.cierres===0) sugerencias.push(`${esc(a)} lleva ${d.tot} visitas este mes sin ningún cierre — vale la pena una revisión 1 a 1.`);
  });
  const diaDelMes = hoy.getDate();
  const diasDelMesTotal = new Date(hoy.getFullYear(), hoy.getMonth()+1, 0).getDate();
  let metaCPasoTres = null;
  if (diaDelMes/diasDelMesTotal > 0.6) {
    metaCPasoTres = await window._fbGetMetaOnce(anioC, mesC, 'mkt_cierres');
    if (metaCPasoTres && cierresEste.length < metaCPasoTres * (diaDelMes/diasDelMesTotal) * 0.7) {
      sugerencias.push(`Vamos avanzado ${Math.round(diaDelMes/diasDelMesTotal*100)}% del mes y solo ${Math.round(cierresEste.length/metaCPasoTres*100)}% de la meta — riesgo de no llegar.`);
    }
  }

  const el3 = document.getElementById('re-que-hacemos');
  if (el3) {
    el3.innerHTML = sugerencias.length
      ? sugerencias.slice(0,3).map(s => `<div class="re-suggestion">${s}</div>`).join('')
      : `<div class="re-empty">Sin alertas por ahora — todo dentro de lo esperado.</div>`;
  }

  // ── PASO 4: Impacto — meta vs ejecución ──
  const el4 = document.getElementById('re-impacto');
  if (el4) {
    const metaC = metaCPasoTres!==null ? metaCPasoTres : await window._fbGetMetaOnce(anioC, mesC, 'mkt_cierres');
    if (metaC) {
      const pct = Math.round(cierresEste.length/metaC*100);
      el4.innerHTML = `<div class="re-big" style="color:${pct>=100?'var(--ok)':pct>=70?'var(--warn)':'var(--danger)'};">${pct}%
        <small>${cierresEste.length} de ${metaC} reservas · faltan ${Math.max(metaC-cierresEste.length,0)}</small></div>`;
    } else {
      el4.innerHTML = `<div class="re-empty">Sin meta cargada este mes — <a href="#" onclick="switchAdminTab('panelcontrol');return false;" style="color:var(--info);font-weight:600;">cargarla acá →</a></div>`;
    }
  }
}
...
function renderHome() {
  ...
  renderResumenEjecutivo(mesC, anioC);
...
function switchUserTab(id, scroll = true) {
  ['ventas','callcenter','expensas','marketing','fichero','cotizaciones'].forEach(t => {
  ...
  if (id === 'cotizaciones') { cambiarSubTabCotizVendedor('simulador'); actualizarSimulador(); }
}
...
function entrarUsuario() {
  ...
  const tabConfig = [
    ...
    { id:'cotizaciones', label:'💰 Cotizaciones', check:'ventas' },
  ...
  const tabConfigFinal = tabConfig.filter(t => {
    ...
    if (t.id==='cotizaciones' && !modulos.includes('ventas'))   return false;
  ...
  if (modulos.includes('ventas')) window._fbListenCotizaciones(arr => { cotizacionesData = arr; renderMisCotizacionesVendedor(); });
  window._fbListenPoliticaDescuento(arr => { politicaDescuentoData = arr; });
  window._fbListenPlazosModalidad(data => { plazosModalidadData = data; });
  window._fbListenVisitasProgramadasTodas(arr => {
    visitasProgramadasData = arr;
    misVisitasProgramadas = arr.filter(v => v.asesorKey === a._key && (v.estado === 'pendiente' || v.estado === 'en_proceso'));
    try { renderVisitasProgramadasAsesor(); } catch(e) { console.error('renderVisitasProgramadasAsesor:', e); }
    try { renderVisitasProgramadasCallCenter(); } catch(e) { console.error('renderVisitasProgramadasCallCenter:', e); }
    try { poblarSelectAsesorCallCenter(); } catch(e) { console.error('poblarSelectAsesorCallCenter:', e); }
  });
  ...
  if (modulos.includes('ventas')) {
    window._fbListenRegistros(a._key, arr => {
      misRegs = arr;
      try { renderMisRegistros(); } catch(e) { console.error('renderMisRegistros:', e); }
      try { renderFicheroVendedor(); } catch(e) { console.error('renderFicheroVendedor:', e); }
      try { poblarSugerenciasClientes(); } catch(e) { console.error('poblarSugerenciasClientes:', e); }
    });
...
async function guardarExpensaUsuario() {
  ...
  const mesCap = PC_MESES[new Date().getMonth()];
...
async function guardarLead() {
  ...
  const mes = document.getElementById('mkt-lead-mes')?.value || PC_MESES[new Date().getMonth()];
...
═══════════════════════════════════════════ */
// Antes iba directo a Orange (http://) — el navegador lo bloqueaba porque nuestro sistema
// vive en https:// y no deja mezclar contenido inseguro dentro de una página segura
// ("contenido mixto"), aparte del bloqueo de CORS. Por eso pasa por este puente propio
// (Cloudflare Worker) que le habla a Orange por detrás y nos devuelve la respuesta en https.
const ORANGE_API_URL = 'https://proud-tooth-742a.uv-cartera.workers.dev';
let orangeMoraData = [];
let orangeCarteraData = [];
let orangeCarteraAgregados = null;
let carteraCategoriasData = {};
...
/* ═══════════════════════════════════════════
   COBRANZA DE EXPENSAS EN VIVO — mismo criterio que Cartera:
   Fecha de pago + monto, ignorando Estado de Cuota y Estado de Pago.
═══════════════════════════════════════════ */
let orangeExpensasDetalleRaw = [];
let expensasCategoriasData = {};
let expFilAnio = new Date().getFullYear();
let expFilMes  = PC_MESES[new Date().getMonth()]; // lista fija — este era el causante directo de que la meta no apareciera: el mes inicial venía del idioma del navegador y no coincidía carácter por carácter con "Septiembre" del <option>, así que la ruta de Firebase para guardar y para leer la meta terminaban siendo distintas sin que se viera ningún error
let expFilGestion = _gestionActual();

const EXPENSAS_CATEGORIAS = ['Vigente', 'Mora', 'Adelanto', 'Otros Ingresos'];

function cambiarFiltroExpensas() {
  expFilAnio = parseInt(document.getElementById('fil-exp-anio').value);
  expFilMes  = document.getElementById('fil-exp-mes-global').value;
  expFilGestion = parseInt(document.getElementById('fil-exp-gestion').value);
  renderCobranzaExpensasKPIs(); // mantiene poblados los selectores de Año/Gestión
  renderDashboardExpensas();
  renderResumenMensualExpensas();
  renderSemanalExpensas();
}

let _autoSyncExpensasOrangeInterval = null;
function iniciarAutoSyncExpensasOrange() {
  if (_autoSyncExpensasOrangeInterval) return;
  _autoSyncExpensasOrangeInterval = setInterval(() => { cargarDetalleExpensas(true); }, ORANGE_AUTOSYNC_MIN * 60 * 1000);
}


/* ═══════════════════════════════════════════════════════════════
   DASHBOARD ANALÍTICO — COBRANZA EN VIVO (Expensas)
   Motor de cálculo + ApexCharts. Todo lo que dice el "Análisis" bajo
   cada gráfico sale de una cuenta real sobre los datos, no es texto
   armado — si el número cambia, el texto cambia con él.
═══════════════════════════════════════════════════════════════ */
let dbeCharts = {};

function _agruparPorSemanaISO_dbe(datos, nSemanas) {
  const hoy = new Date();
  const diaSemanaHoy = (hoy.getDay()+6)%7;
  const inicioSemanaActual = new Date(hoy); inicioSemanaActual.setDate(hoy.getDate()-diaSemanaHoy); inicioSemanaActual.setHours(0,0,0,0);
  const buckets = [];
  for (let i = nSemanas-1; i >= 0; i--) {
    const inicio = new Date(inicioSemanaActual); inicio.setDate(inicioSemanaActual.getDate()-i*7);
    const fin = new Date(inicio); fin.setDate(inicio.getDate()+7);
    buckets.push({ inicio, fin, label: inicio.getDate()+'/'+(inicio.getMonth()+1), total: 0 });
  }
  datos.forEach(r => {
    if (!r.fechaOrden) return;
    const b = buckets.find(x => r.fechaOrden >= x.inicio.getTime() && r.fechaOrden < x.fin.getTime());
    if (b) b.total += r.monto;
  });
  return buckets;
}

function _agruparPorMes_dbe(datos, nMeses) {
  const hoy = new Date();
  const buckets = [];
  for (let i = nMeses-1; i >= 0; i--) {
    const d = new Date(hoy.getFullYear(), hoy.getMonth()-i, 1);
    buckets.push({ anio: d.getFullYear(), mes: d.getMonth(), label: ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'][d.getMonth()]+' '+String(d.getFullYear()).slice(2), total: 0 });
  }
  datos.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    const b = buckets.find(x => x.anio===f.getFullYear() && x.mes===f.getMonth());
    if (b) b.total += r.monto;
  });
  return buckets;
}

function _agruparPorDia_dbe(datos, nDias) {
  // O = primer cobro del día según el orden en que Orange los entrega (no tenemos hora exacta),
  // C = último, H = el cobro más grande del día, L = el más chico, V = cantidad de cobros.
  const porDia = {};
  datos.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    const key = f.getFullYear()+'-'+String(f.getMonth()+1).padStart(2,'0')+'-'+String(f.getDate()).padStart(2,'0');
    (porDia[key] ||= { fechaOrden: new Date(f.getFullYear(),f.getMonth(),f.getDate()).getTime(), montos: [] }).montos.push(r.monto);
  });
  const dias = Object.values(porDia).sort((a,b)=>a.fechaOrden-b.fechaOrden).slice(-nDias);
  return dias.map(d => ({
    x: d.fechaOrden,
    o: Math.round(d.montos[0]),
    h: Math.round(Math.max(...d.montos)),
    l: Math.round(Math.min(...d.montos)),
    c: Math.round(d.montos[d.montos.length-1]),
    v: d.montos.length,
  }));
}
...
async function renderDashboardExpensas() {
  const root = document.getElementById('dbe-resumen-ejecutivo');
  if (!root) return;
  if (!orangeExpensasDetalleRaw.length) return;

  const fmtBsK = n => {
    const v = Math.round(n||0);
    if (Math.abs(v) >= 1000000) return 'Bs'+Math.round(v/1000000)+'M';
    if (Math.abs(v) >= 1000) return 'Bs'+Math.round(v/1000)+'K';
    return 'Bs'+v;
  };

  // (el label de sincronización ya lo actualiza cargarDetalleExpensas al terminar de cargar)

  /* ── KPI 1 y 2: semana y mes, con % vs período anterior ── */
  const semanas10 = _agruparPorSemanaISO_dbe(orangeExpensasDetalleRaw, 10);
  const semanaActual = semanas10[semanas10.length-1].total;
  const semanaAnterior = semanas10[semanas10.length-2]?.total || 0;
  const deltaSemana = semanaAnterior ? ((semanaActual-semanaAnterior)/semanaAnterior*100) : null;

  const meses12 = _agruparPorMes_dbe(orangeExpensasDetalleRaw, 12);
  const mesActual = meses12[meses12.length-1].total;
  const mesAnterior = meses12[meses12.length-2]?.total || 0;
  const deltaMes = mesAnterior ? ((mesActual-mesAnterior)/mesAnterior*100) : null;

  setText('dbe-kpi-semana', fmtBsK(semanaActual));
  const elDeltaSem = document.getElementById('dbe-kpi-semana-delta');
  if (deltaSemana !== null) {
    elDeltaSem.textContent = (deltaSemana>=0?'▲ ':'▼ ')+Math.abs(deltaSemana).toFixed(1)+'% vs semana anterior';
    elDeltaSem.className = 'bi-kpi-delta ' + (deltaSemana>=0?'pos':'neg');
  } else { elDeltaSem.textContent = 'sin semana anterior'; elDeltaSem.className='bi-kpi-delta muted'; }

  setText('dbe-kpi-mes', fmtBsK(mesActual));
  const elDeltaMes = document.getElementById('dbe-kpi-mes-delta');
  if (deltaMes !== null) {
    elDeltaMes.textContent = (deltaMes>=0?'▲ ':'▼ ')+Math.abs(deltaMes).toFixed(1)+'% vs mes anterior';
    elDeltaMes.className = 'bi-kpi-delta ' + (deltaMes>=0?'pos':'neg');
  } else { elDeltaMes.textContent = 'sin mes anterior'; elDeltaMes.className='bi-kpi-delta muted'; }

  const nombresMesLargoKpi = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const [metaVigenteKpi, metaMoraKpi] = await Promise.all([
    window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_vigente'),
    window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_mora'),
  ]);
  const metaMesKpi = (metaVigenteKpi||0) + (metaMoraKpi||0);
  const elMetaMes = document.getElementById('dbe-kpi-mes-meta');
  if (metaMesKpi) {
    const pctVsMeta = Math.round(mesActual/metaMesKpi*100);
    elMetaMes.textContent = `🎯 ${pctVsMeta}% de la meta (${fmtBsK(metaMesKpi)})`;
    elMetaMes.className = 'bi-kpi-delta ' + (pctVsMeta>=100?'pos':pctVsMeta>=70?'warn':'neg');
  } else {
    elMetaMes.textContent = 'sin meta cargada';
    elMetaMes.className = 'bi-kpi-delta muted';
  }

  /* ── KPI 3: gestión + proyección de cierre (regla de tres simple sobre lo transcurrido) ── */
  const gestionActualN = _gestionActual();
  let cobradoGestion = 0;
  orangeExpensasDetalleRaw.forEach(r => { if (r.fechaOrden && _gestionDeFecha(new Date(r.fechaOrden)) === gestionActualN) cobradoGestion += r.monto; });
  const inicioGestion = new Date(gestionActualN, 3, 1); // 1° de abril
  const finGestion = new Date(gestionActualN+1, 2, 31); // 31 de marzo
  const hoy = new Date();
  const diasTranscurridos = Math.max(1, Math.round((hoy-inicioGestion)/86400000));
  const diasTotalesGestion = Math.round((finGestion-inicioGestion)/86400000);
  const proyeccionCierre = (cobradoGestion / diasTranscurridos) * diasTotalesGestion;

  setText('dbe-kpi-gestion', fmtBsK(cobradoGestion));
  setText('dbe-kpi-gestion-proy', 'proyección de cierre: ' + fmtBsK(proyeccionCierre));

  /* ── KPI 4: Índice de Volatilidad = Coeficiente de Variación de las últimas 10 semanas ── */
  const valoresSemanales = semanas10.map(s=>s.total);
  const mediaSemanal = valoresSemanales.reduce((s,v)=>s+v,0)/valoresSemanales.length;
  const desvio = _desviacionEstandar(valoresSemanales);
  const cv = mediaSemanal ? (desvio/mediaSemanal*100) : 0;

  setText('dbe-kpi-volatilidad', cv.toFixed(1)+'%');
  const elVol = document.getElementById('dbe-kpi-volatilidad-tag');
  if (cv > 30) { elVol.textContent = '🔴 Riesgo Alto — muy irregular'; elVol.className = 'bi-kpi-delta neg'; }
  else if (cv > 15) { elVol.textContent = '🟡 Riesgo Moderado'; elVol.className = 'bi-kpi-delta warn'; }
  else { elVol.textContent = '🟢 Cobranza estable'; elVol.className = 'bi-kpi-delta pos'; }

  /* ── Resumen Ejecutivo — combinando liquidez, tendencia, volatilidad y riesgo ── */
  const tendenciaTxt = deltaMes===null ? 'sin datos suficientes del mes anterior para medir tendencia'
    : deltaMes >= 5 ? `una tendencia positiva (+${deltaMes.toFixed(1)}% vs el mes anterior)`
    : deltaMes <= -5 ? `una caída de ${Math.abs(deltaMes).toFixed(1)}% respecto al mes anterior, que conviene revisar`
    : `un comportamiento estable respecto al mes anterior (${deltaMes>=0?'+':''}${deltaMes.toFixed(1)}%)`;
  const volatilidadTxt = cv > 30 ? `una volatilidad alta (CV ${cv.toFixed(1)}%) — la cobranza semanal varía mucho, lo que dificulta proyectar con precisión`
    : cv > 15 ? `una volatilidad moderada (CV ${cv.toFixed(1)}%)`
    : `una cobranza semanal consistente (CV ${cv.toFixed(1)}%)`;
  const proyeccionTxt = `Al ritmo actual, la proyección de cierre de gestión es de ${fmtBsK(proyeccionCierre)}.`;
  setText('dbe-resumen-ejecutivo',
    `La cartera de Expensas muestra ${tendenciaTxt} y ${volatilidadTxt}. Esta semana se cobraron ${fmtBsK(semanaActual)}` +
    (deltaSemana!==null ? ` (${deltaSemana>=0?'+':''}${deltaSemana.toFixed(1)}% vs la semana pasada)` : '') +
    `. ${proyeccionTxt}`
  );

  /* ── Gráfico 1: Tendencia Semanal (barras + media móvil 4 semanas) ── */
  const mediaMovil4 = semanas10.map((_,i,arr) => {
    const desde = Math.max(0, i-3);
    const ventana = arr.slice(desde, i+1).map(s=>s.total);
    return Math.round(ventana.reduce((s,v)=>s+v,0)/ventana.length);
  });
  if (dbeCharts.semanal) dbeCharts.semanal.destroy();
  dbeCharts.semanal = new ApexCharts(document.querySelector('#dbe-chart-semanal'), {
    chart: { type:'line', height:260, toolbar:{show:false}, background:'transparent', foreColor:'#667085' },
    series: [
      { name:'Cobro semanal', type:'column', data: semanas10.map(s=>Math.round(s.total)) },
      { name:'Media móvil (4 sem.)', type:'line', data: mediaMovil4 },
    ],
    xaxis: { categories: semanas10.map(s=>s.label), labels:{style:{fontSize:'10px'}} },
    yaxis: { labels: { formatter: v => fmtBsK(v) }, max: Math.max(...semanas10.map(s=>s.total), ...mediaMovil4) * 1.25 },
    colors: ['#2451FF', '#F79009'],
    stroke: { width:[0,3], curve:'smooth' },
    grid: { borderColor:'#EAECF0', strokeDashArray:3 },
    legend: { fontSize:'11px' },
    dataLabels: {
      enabled: true, enabledOnSeries: [0],
      formatter: v => fmtBsK(v),
      style: { fontSize:'10px', colors:['#344054'] },
      background: { enabled:true, foreColor:'#fff', borderRadius:3, opacity:.85, borderWidth:0 },
      offsetY: -4,
    },
    tooltip: { theme:'light', y:{formatter: v=>'Bs '+Math.round(v).toLocaleString('es-BO')} },
  });
  dbeCharts.semanal.render();

  const semanaMax = semanas10.reduce((m,s)=>s.total>m.total?s:m, semanas10[0]);
  setText('dbe-insight-semanal',
    `Diagnóstico: la semana de mayor cobranza en este período fue ${semanaMax.label} con ${fmtBsK(semanaMax.total)}. ` +
    `Causa probable: ${deltaSemana!==null && deltaSemana<-15 ? 'la caída reciente puede deberse a menos categorización/seguimiento esta semana, no necesariamente a menos pagos reales.' : 'el patrón sigue el ritmo habitual de cobro semanal.'} ` +
    `Prescripción: ${cv>30 ? 'con esta volatilidad, conviene enviar recordatorios de pago distribuidos en la semana, no concentrados en un solo día.' : 'mantener el ritmo actual de seguimiento.'}`
  );

  /* ── Gráfico 2: Tendencia Mensual (área + meta punteada) ── */
  const nombresMesLargo = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const metasPorMes = await Promise.all(meses12.map(m =>
    Promise.all([
      window._fbGetMetaOnce(m.anio, nombresMesLargo[m.mes], 'expensas_vigente'),
      window._fbGetMetaOnce(m.anio, nombresMesLargo[m.mes], 'expensas_mora'),
    ]).then(([v,mo])=>(v||0)+(mo||0))
  ));
  if (dbeCharts.mensual) dbeCharts.mensual.destroy();
  const mesConMeta = metasPorMes[metasPorMes.length-1];
  const pctMetaMes = mesConMeta ? (mesActual/mesConMeta*100) : null;
  const techoMensual = Math.max(...meses12.map(m=>m.total), ...metasPorMes) * 1.3;

  dbeCharts.mensual = new ApexCharts(document.querySelector('#dbe-chart-mensual'), {
    chart: { type:'area', height:260, toolbar:{show:false}, background:'transparent', foreColor:'#667085' },
    series: [
      { name:'Cobrado', type:'area', data: meses12.map(m=>Math.round(m.total)) },
      { name:'Meta', type:'line', data: metasPorMes.map(v=>Math.round(v)) },
    ],
    title: {
      text: pctMetaMes!==null ? `${pctMetaMes.toFixed(0)}% de la meta de este mes` : 'Sin meta cargada este mes',
      style: { fontSize:'13px', fontWeight:700, color: pctMetaMes===null?'#64748b':pctMetaMes>=100?'#10b981':pctMetaMes>=70?'#f59e0b':'#ef4444' },
    },
    xaxis: { categories: meses12.map(m=>m.label), labels:{style:{fontSize:'10px'}} },
    yaxis: { labels: { formatter: v => fmtBsK(v) }, max: techoMensual },
    colors: ['#2451FF', '#F04438'],
    stroke: { width:[2,2], curve:'smooth', dashArray:[0,6] },
    ...
    dataLabels: {
      enabled: true, enabledOnSeries: [0],
      formatter: v => fmtBsK(v),
      style: { fontSize:'10px', colors:['#1D3ECC'] },
      background: { enabled:true, foreColor:'#fff', borderRadius:3, opacity:.85, borderWidth:0 },
      offsetY: -6,
    },
    tooltip: { theme:'light', y:{formatter: v=>'Bs '+Math.round(v).toLocaleString('es-BO')} },
  });
  dbeCharts.mensual.render();

  setText('dbe-insight-mensual',
    pctMetaMes !== null
      ? `Diagnóstico: este mes se lleva cobrado el ${pctMetaMes.toFixed(0)}% de la meta (${fmtBsK(mesActual)} de ${fmtBsK(mesConMeta)}). ` +
        `Causa probable: ${pctMetaMes<70 ? 'todavía queda categorizar cobros pendientes, o la cobranza real está por debajo de lo esperado.' : 'el ritmo de cobro viene alineado con lo proyectado.'} ` +
        `Prescripción: ${pctMetaMes<70 ? 'revisar la cola de "Categorizar cobros" — puede haber plata ya cobrada que todavía no está contando para la meta.' : 'mantener el seguimiento actual.'}`
      : 'Diagnóstico: no hay meta cargada para este mes — no se puede medir el avance contra un objetivo. Prescripción: cargar la meta desde el Resumen Mensual para poder comparar.'
  );

  /* ── Gráfico 3: Distribución por Modalidad (donut, este mes) ── */
  const mesesNombre = nombresMesLargo;
  const mesFilNum = mesesNombre.indexOf(expFilMes);
  const porModalidad = {};
  orangeExpensasDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (f.getFullYear()!==expFilAnio || f.getMonth()!==mesFilNum) return;
    porModalidad[r.modalidad] = (porModalidad[r.modalidad]||0) + r.monto;
  });
  const modalidadesOrdenadas = Object.entries(porModalidad).sort((a,b)=>b[1]-a[1]);
  if (dbeCharts.modalidad) dbeCharts.modalidad.destroy();
  dbeCharts.modalidad = new ApexCharts(document.querySelector('#dbe-chart-modalidad'), {
    chart: { type:'donut', height:260, background:'transparent', foreColor:'#667085' },
    series: modalidadesOrdenadas.map(([,v])=>Math.round(v)),
    labels: modalidadesOrdenadas.map(([k])=>k),
    colors: ['#2451FF','#F79009','#12B76A','#7C3AED'],
    legend: {
      fontSize:'11px', position:'bottom',
      formatter: (nombre, opts) => nombre + ' — ' + fmtBsK(opts.w.globals.series[opts.seriesIndex]),
    },
    dataLabels: {
      enabled:true,
      formatter: (val, opts) => fmtBsK(opts.w.globals.series[opts.seriesIndex]) + ' (' + val.toFixed(0) + '%)',
      style: { fontSize:'11px', fontWeight:700 },
    },
    tooltip: { theme:'light', y:{formatter: v=>'Bs '+Math.round(v).toLocaleString('es-BO')} },
  });
  dbeCharts.modalidad.render();

  if (modalidadesOrdenadas.length) {
    const totalModalidad = modalidadesOrdenadas.reduce((s,[,v])=>s+v,0);
    const [nombreTop, montoTop] = modalidadesOrdenadas[0];
    const pctTop = (montoTop/totalModalidad*100);
    setText('dbe-insight-modalidad',
      `Diagnóstico: "${nombreTop}" concentra el ${pctTop.toFixed(0)}% de la cobranza de este mes (${fmtBsK(montoTop)}). ` +
      `Causa probable: ${pctTop>70 ? 'es la modalidad predominante entre los clientes de expensas — la cartera está concentrada en una sola forma de pago.' : 'la cobranza está repartida entre varias modalidades, sin una que domine claramente.'} ` +
      `Prescripción: ${pctTop>70 ? 'si esa modalidad depende de un proceso manual (ej: transferencia), vale la pena automatizar recordatorios específicos para ella.' : 'mantener el seguimiento diferenciado por modalidad.'}`
    );
  } else {
    setText('dbe-insight-modalidad', 'Sin cobros categorizados por modalidad este mes todavía.');
  }

  /* ── Gráfico 4: Candlestick + Volumen UNIFICADOS en un solo panel (doble eje) —
     las velas (Cobro Diario) y el volumen (N° de cobros) comparten el mismo gráfico,
     estilo bolsa de valores real, no dos paneles apilados. ── */
  const velas20 = _agruparPorDia_dbe(orangeExpensasDetalleRaw, 20);
  const techoVelas = Math.max(...velas20.map(d=>d.h)) * 1.15;
  const techoVolumen = Math.max(...velas20.map(d=>d.v)) * 3.2; // el volumen queda "bajo" (chico), como en el ejemplo

  if (dbeCharts.velas) dbeCharts.velas.destroy();
  dbeCharts.velas = new ApexCharts(document.querySelector('#dbe-chart-velas'), {
    chart: { height:340, toolbar:{show:false}, background:'transparent', foreColor:'#667085', parentHeightOffset:0 },
    series: [
      { name:'Cobro Diario', type:'candlestick', data: velas20.map(d=>({ x:d.x, y:[d.o,d.h,d.l,d.c] })) },
      { name:'Volumen (Transacciones)', type:'column', data: velas20.map(d=>({x:d.x, y:d.v})) },
    ],
    xaxis: { type:'datetime', labels:{style:{fontSize:'10px'}} },
    yaxis: [
      { seriesName:'Cobro Diario', min:0, max:techoVelas, labels:{ formatter: v => fmtBsK(Math.round(v)) }, title:{text:'Monto (Bs)', style:{fontSize:'10px'}} },
      { seriesName:'Volumen (Transacciones)', opposite:true, min:0, max:techoVolumen, labels:{ formatter: v => Math.round(v) }, title:{text:'N° cobros', style:{fontSize:'10px'}} },
    ],
    plotOptions: {
      candlestick: { colors: { upward:'#12B76A', downward:'#F04438' }, wick:{useFillColor:true} },
      bar: { columnWidth:'55%' },
    },
    fill: { opacity:[1,.35] },
    colors: ['#12B76A', '#7C3AED'],
    grid: { borderColor:'#EAECF0', strokeDashArray:3 },
    legend: { fontSize:'11px', labels:{colors:'#475467'} },
    tooltip: { theme:'light', shared:true, x:{format:'dd MMM'} },
  });
  dbeCharts.velas.render();

  if (velas20.length) {
    const diaMayorVolumen = velas20.reduce((m,d)=>d.v>m.v?d:m, velas20[0]);
    const fechaMayorVol = new Date(diaMayorVolumen.x);
    // Chequeo real de concentración en los primeros días del mes (típico de expensas)
    const enPrimeraQuincena = velas20.filter(d => new Date(d.x).getDate() <= 15);
    const pctPrimeraQuincena = velas20.length ? (enPrimeraQuincena.reduce((s,d)=>s+d.v,0) / velas20.reduce((s,d)=>s+d.v,0) * 100) : 0;
    setText('dbe-insight-velas',
      `Diagnóstico: el día con más transacciones fue el ${fechaMayorVol.getDate()}/${fechaMayorVol.getMonth()+1}, con ${diaMayorVolumen.v} cobros. ` +
      `Causa probable: ${pctPrimeraQuincena>65 ? `el ${pctPrimeraQuincena.toFixed(0)}% de las transacciones de este período cayeron en la primera quincena — patrón típico de pago de expensas a principio de mes.` : 'las transacciones están repartidas a lo largo del mes, sin concentrarse en la primera quincena.'} ` +
      `Prescripción: ${pctPrimeraQuincena>65 ? 'reforzar los recordatorios automáticos entre el día 1 y el 5, que es cuando más responden los clientes.' : 'mantener recordatorios distribuidos durante todo el mes.'}`
    );
  }

  // Guardamos lo ya calculado para que el Modal 3D (Vista ampliada + Estadísticas)
  // lo reuse tal cual, sin tener que recalcular nada de nuevo.
  window._hudData = { semanas10, meses12, modalidadesOrdenadas, velas20, fmtBsK };
}

/* ═══════════════════════════════════════════
   PARTÍCULAS FLOTANTES (fondo del dashboard)
═══════════════════════════════════════════ */
function _generarParticulasHud() {
  const cont = document.getElementById('dbe-particles');
  if (!cont || cont.dataset.armado) return;
  cont.dataset.armado = '1';
  const colores = ['#4A8EFF', '#22D3EE', '#A855F7', '#EC4899'];
  let html = '';
  for (let i = 0; i < 30; i++) {
    ...
    html += `<div class="hud-particle" style="left:${left}%;color:${color};animation-duration:${dur}s;animation-delay:${delay}s;"></div>`;
  }
  cont.innerHTML = html;
}

/* ═══════════════════════════════════════════
   MOTOR ESTADÍSTICO — real, sobre los datos que ya tenemos.
═══════════════════════════════════════════ */
const HudStats = {
  ...
  cv(arr) { const m = this.mean(arr); return m ? (this.stddev(arr)/m*100) : 0; },
  quantile(arr, q) {
    if (!arr.length) return 0;
    const s = [...arr].sort((a,b)=>a-b);
  ...
  iqr(arr) { return this.q3(arr) - this.q1(arr); },
};

/* ═══════════════════════════════════════════
   MODAL 3D — Vista ampliada + Estadísticas, para cada uno de los 4 gráficos.
═══════════════════════════════════════════ */
let hudModalChartActual = null;
let hudChartVistaAmpliada = null;

const HUD_CHART_META = {
  semanal:   { titulo: 'Tendencia Semanal', sub: 'Cobro semanal + media móvil de 4 semanas', serieLabel: 'Cobro semanal' },
  mensual:   { titulo: 'Tendencia Mensual', sub: 'Cobrado vs. Meta, últimos 12 meses', serieLabel: 'Cobrado' },
  modalidad: { titulo: 'Distribución por Modalidad', sub: 'Cobranza de este mes, por modalidad', serieLabel: 'Modalidad' },
  velas:     { titulo: 'Análisis Micro-Estructural', sub: 'Velas diarias + volumen, estilo bolsa de valores', serieLabel: 'Cobro Diario' },
};

function abrirHudModal(chartId) {
  if (!window._hudData) return;
  hudModalChartActual = chartId;
  const meta = HUD_CHART_META[chartId];
  document.getElementById('hud-modal-title').textContent = meta.titulo;
  document.getElementById('hud-modal-sub').textContent = meta.sub;
  document.getElementById('hud-modal').classList.add('open');
  cambiarHudTab('vista');
}

function cerrarHudModal() {
  document.getElementById('hud-modal').classList.remove('open');
  if (hudChartVistaAmpliada) { hudChartVistaAmpliada.destroy(); hudChartVistaAmpliada = null; }
}

function cambiarHudTab(tab) {
  document.querySelectorAll('.hud-mtab').forEach(b => b.classList.toggle('active', b.dataset.mtab === tab));
  document.getElementById('hud-modal-vista').style.display = tab === 'vista' ? '' : 'none';
  document.getElementById('hud-modal-stats').style.display = tab === 'stats' ? '' : 'none';
  if (tab === 'vista') renderHudVistaAmpliada();
  else renderHudEstadisticas();
}

function renderHudVistaAmpliada() {
  const cont = document.getElementById('hud-modal-vista');
  const d = window._hudData;
  const id = hudModalChartActual;
  if (hudChartVistaAmpliada) { hudChartVistaAmpliada.destroy(); hudChartVistaAmpliada = null; }
  cont.innerHTML = '';

  const baseOpts = {
    chart: { fontFamily:'Inter, sans-serif', toolbar:{show:true}, zoom:{enabled:false}, background:'transparent', foreColor:'#667085', height:420 },
    grid: { borderColor:'#EAECF0', strokeDashArray:3 },
    tooltip: { theme:'light' },
  };

  if (id === 'semanal') {
    const mediaMovil4 = d.semanas10.map((_,i,arr) => { const desde=Math.max(0,i-3); const v=arr.slice(desde,i+1).map(s=>s.total); return Math.round(v.reduce((s,x)=>s+x,0)/v.length); });
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts,
      series: [{ name:'Cobro semanal', type:'column', data:d.semanas10.map(s=>Math.round(s.total)) }, { name:'Media móvil (4 sem.)', type:'line', data:mediaMovil4 }],
      xaxis: { categories: d.semanas10.map(s=>s.label) }, yaxis: { labels:{formatter:v=>d.fmtBsK(v)} },
      colors: ['#2451FF','#F79009'], stroke:{width:[0,3], curve:'smooth'},
    });
  } else if (id === 'mensual') {
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts,
      series: [{ name:'Cobrado', type:'area', data:d.meses12.map(m=>Math.round(m.total)) }],
      xaxis: { categories: d.meses12.map(m=>m.label) }, yaxis: { labels:{formatter:v=>d.fmtBsK(v)} },
      colors: ['#2451FF'], stroke:{width:2, curve:'smooth'}, fill:{type:'gradient', gradient:{opacityFrom:.45,opacityTo:0}},
    });
  } else if (id === 'modalidad') {
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts, chart:{...baseOpts.chart, type:'donut'},
      series: d.modalidadesOrdenadas.map(([,v])=>Math.round(v)), labels: d.modalidadesOrdenadas.map(([k])=>k),
      colors: ['#2451FF','#F79009','#12B76A','#7C3AED'],
      plotOptions:{ pie:{ donut:{ size:'65%' } } },
      dataLabels:{ formatter:v=>v.toFixed(0)+'%' }, legend:{ position:'bottom', labels:{colors:'#475467'} },
    });
  } else if (id === 'velas') {
    hudChartVistaAmpliada = new ApexCharts(cont, { ...baseOpts, chart:{...baseOpts.chart, height:460},
      series: [
        { name:'Cobro Diario', type:'candlestick', data: d.velas20.map(x=>({x:x.x, y:[x.o,x.h,x.l,x.c]})) },
        { name:'Volumen', type:'column', data: d.velas20.map(x=>({x:x.x, y:x.v})) },
      ],
      xaxis: { type:'datetime' },
      yaxis: [
        { seriesName:'Cobro Diario', labels:{formatter:v=>d.fmtBsK(Math.round(v))} },
        { seriesName:'Volumen', opposite:true, labels:{formatter:v=>Math.round(v)} },
      ],
      plotOptions:{ candlestick:{ colors:{ upward:'#12B76A', downward:'#F04438' } } },
      fill:{ opacity:[1,.35] }, colors:['#12B76A','#7C3AED'],
    });
  }
  hudChartVistaAmpliada.render();
}

function renderHudEstadisticas() {
  const grid = document.getElementById('hud-modal-stats-grid');
  const diagEl = document.getElementById('hud-modal-diagnostico');
  const d = window._hudData;
  const id = hudModalChartActual;
  let valores = [], labels = [];
  if (id === 'semanal') { valores = d.semanas10.map(s=>s.total); labels = d.semanas10.map(s=>s.label); }
  else if (id === 'mensual') { valores = d.meses12.map(m=>m.total); labels = d.meses12.map(m=>m.label); }
  else if (id === 'modalidad') { valores = d.modalidadesOrdenadas.map(([,v])=>v); labels = d.modalidadesOrdenadas.map(([k])=>k); }
  else if (id === 'velas') { valores = d.velas20.map(x=>x.c); labels = d.velas20.map(x=>new Date(x.x).toLocaleDateString('es-BO')); }

  if (!valores.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos suficientes.</div>'; diagEl.innerHTML=''; return; }
  const box = (label, value, destacado, tono) => `<div class="bi-stat-box${destacado?' hl':''}${tono==='warn'?' warn':''}"><div class="bi-stat-label">${label}</div><div class="bi-stat-value">${value}</div></div>`;
  const diag = (html, tono) => { diagEl.className = 'bi-insight' + (tono?' '+tono:''); diagEl.innerHTML = html; };

  if (id === 'modalidad') {
    const total = valores.reduce((s,v)=>s+v,0);
    const pares = labels.map((l,i)=>({l,v:valores[i]})).sort((a,b)=>b.v-a.v);
    const pctTop = total ? (pares[0].v/total*100) : 0;
    diag(`<strong>Diagnóstico:</strong> "${pares[0].l}" concentra el ${pctTop.toFixed(0)}% de la cobranza de expensas de este mes. ` +
      `<strong>Causa probable:</strong> ${pctTop>70?'es la forma de pago que eligió la mayoría de los propietarios — si depende de un trámite manual (ej: transferencia con comprobante), conviene reforzar el seguimiento específico de esa vía.':'la cobranza está repartida entre varias modalidades, sin una que domine.'} ` +
      `<strong>Prescripción:</strong> ${pctTop>70?'automatizar recordatorios específicos para esa modalidad, ya que es donde está la mayor parte del dinero por cobrar.':'mantener el seguimiento parejo entre modalidades.'}`, pctTop>70?'warn':'pos');
    grid.innerHTML = [box('Total', d.fmtBsK(valores.reduce((s,v)=>s+v,0)), true), box('Líder', `${pares[0].l} (${pctTop.toFixed(0)}%)`), box('N° modalidades', pares.length)].join('');
    return;
  }

  const primero = valores[0], ultimo = valores[valores.length-1];
  const crecimiento = primero ? ((ultimo-primero)/Math.abs(primero)*100) : 0;
  const idxMax = valores.indexOf(Math.max(...valores));
  const cv = HudStats.cv(valores);
  const esVolatil = cv > 30;
  const esVelas = id === 'velas';

  diag(`<strong>Diagnóstico:</strong> ${esVelas?'la cobranza diaria':'el valor'} actual es ${d.fmtBsK(ultimo)}, con una variación del ${cv.toFixed(0)}% entre ${esVelas?'días':'períodos'} (${esVolatil?'alta':'baja'} volatilidad). El mejor momento fue ${labels[idxMax]}. ` +
    `<strong>Causa probable:</strong> ${esVolatil ? 'la cobranza de expensas suele concentrarse en ciertos días del mes (típicamente principio de mes) — esa concentración natural explica buena parte de la variación.' : 'el ritmo de cobro es consistente, sin picos ni caídas marcadas.'} ` +
    `<strong>Prescripción:</strong> ${esVolatil ? 'reforzar los recordatorios automáticos en los días de mayor respuesta histórica, en vez de distribuir el esfuerzo parejo en todo el mes.' : 'mantener el seguimiento actual, viene funcionando de forma predecible.'}`,
    esVolatil ? 'warn' : 'pos');

  grid.innerHTML = [
    box('Valor actual', d.fmtBsK(ultimo), true),
    box('Coef. Variación', cv.toFixed(1)+'%', false, esVolatil?'warn':null),
    box('Mejor momento', labels[idxMax]||'—'),
    box('N° de datos', valores.length),
  ].join('');
}

/* ═══════════════════════════════════════════════════════════════
   MODAL GENÉRICO — para CUALQUIER gráfico Chart.js que ya exista en
   el sistema. No cambia el gráfico ni su lógica de datos: toma la
   instancia real (Chart.getChart lo busca por el id del canvas),
   la agranda, y calcula estadísticas REALES Y RELEVANTES según qué
   tipo de pregunta de negocio responde ese gráfico puntual — no la
   misma lista de media/mediana para todos.

   tipoAnalisis:
     'tendencia'   — algo que evoluciona en el tiempo (mes a mes, semana a semana)
     'composicion' — partes de un todo (donut: qué % es cada categoría)
     'ranking'     — comparación entre personas/categorías (quién es 1°, quién último)
     'dispersion'  — relación entre 2 variables (scatter)
═══════════════════════════════════════════════════════════════ */
let cjsModalChart = null;
let _cjsModalOrigenId = null;
let _cjsModalTipoAnalisis = 'tendencia';
let _cjsModalTipoVistaActual = null; // tipo de gráfico elegido en el selector (o null = el original)

function abrirModalGraficoJS(canvasId, titulo, sub, tipoAnalisis) {
  try {
    console.log('[GRAFICO] abrir ->', canvasId, tipoAnalisis);
    const original = (typeof Chart !== 'undefined') ? Chart.getChart(canvasId) : null;
    if (!original) { console.warn('[GRAFICO] Chart.getChart no encontró nada para', canvasId); toastErr('Ese gráfico todavía no cargó — esperá un segundo y probá de nuevo.'); return; }
    _cjsModalOrigenId = canvasId;
    _cjsModalTipoAnalisis = tipoAnalisis || 'tendencia';
    _cjsModalTipoVistaActual = null;
    const modal = document.getElementById('cjs-modal');
    if (!modal) { console.error('[GRAFICO] no existe #cjs-modal en el HTML'); return; }
    document.getElementById('cjs-modal-title').textContent = titulo;
    document.getElementById('cjs-modal-sub').textContent = sub || '';
    modal.classList.add('open');
    cambiarCjsTab('vista');
    console.log('[GRAFICO] modal abierto OK, display calculado:', getComputedStyle(modal).display);
  } catch (e) {
    console.error('[GRAFICO] Error real:', e);
    alert('Error al abrir el gráfico: ' + e.message);
  }
}

function cerrarModalGraficoJS() {
  document.getElementById('cjs-modal').classList.remove('open');
  if (cjsModalChart) { cjsModalChart.destroy(); cjsModalChart = null; }
}

function cambiarCjsTab(tab) {
  try {
    document.querySelectorAll('#cjs-modal .bi-mtab').forEach(b => b.classList.toggle('active', b.dataset.mtab === tab));
    document.getElementById('cjs-modal-vista').style.display = tab === 'vista' ? '' : 'none';
    document.getElementById('cjs-modal-stats').style.display = tab === 'stats' ? '' : 'none';
    if (tab === 'vista') renderCjsVistaAmpliada();
    else renderCjsEstadisticas();
  } catch (e) {
    console.error('[GRAFICO] Error en cambiarCjsTab:', e);
  }
}

// Qué tipos de gráfico tiene sentido ofrecer como alternativa, según el tipo original —
// no ofrecemos "torta" para una tendencia en el tiempo (no tendría sentido de negocio),
// ni "línea" para una composición donde lo que importa es la proporción.
function _cjsTiposAlternativosValidos(tipoOriginal, tipoAnalisis) {
  if (tipoAnalisis === 'dispersion') return ['scatter']; // el scatter no tiene alternativa razonable
  if (tipoAnalisis === 'composicion') return ['doughnut', 'pie', 'bar'];
  if (tipoAnalisis === 'ranking') return ['bar', 'line'];
  return ['line', 'bar', 'area']; // tendencia
}

function cambiarCjsTipoVista(tipo) {
  _cjsModalTipoVistaActual = tipo;
  renderCjsVistaAmpliada();
}

function renderCjsVistaAmpliada() {
  const original = Chart.getChart(_cjsModalOrigenId);
  if (!original) return;
  if (cjsModalChart) { cjsModalChart.destroy(); cjsModalChart = null; }
  const cont = document.getElementById('cjs-modal-vista');

  const tipoOriginal = original.config.type;
  const alternativas = _cjsTiposAlternativosValidos(tipoOriginal, _cjsModalTipoAnalisis);
  const nombresTipo = { line:'Líneas', bar:'Barras', area:'Área', doughnut:'Anillo', pie:'Torta', scatter:'Dispersión' };

  // Selector de "ver como" — solo si hay más de 1 alternativa razonable.
  let selectorHtml = '';
  if (alternativas.length > 1) {
    selectorHtml = `<div style="display:flex;gap:6px;margin-bottom:14px;">` +
      alternativas.map(t => {
        const activo = (_cjsModalTipoVistaActual || tipoOriginal) === t;
        return `<button onclick="cambiarCjsTipoVista('${t}')" style="padding:6px 13px;border-radius:8px;border:1px solid ${activo?'var(--brand)':'var(--line)'};background:${activo?'var(--brand)':'var(--surface)'};color:${activo?'#fff':'var(--ink-700)'};font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">${nombresTipo[t]||t}</button>`;
      }).join('') + `</div>`;
  }

  cont.innerHTML = selectorHtml + '<canvas id="cjs-modal-canvas"></canvas>';
  cont.style.height = alternativas.length > 1 ? '500px' : '460px';

  // Clonamos los datos tal cual (mismo dato real, misma fuente) — las funciones de formato
  // (tooltips/ejes con $/Bs) del original no siempre se pueden clonar, así que en la vista
  // ampliada puede verse el número sin el formato especial, pero el dato es exactamente el mismo.
  const tipoAMostrar = _cjsModalTipoVistaActual || (tipoOriginal === 'area' ? 'line' : tipoOriginal);
  const esArea = (_cjsModalTipoVistaActual === 'area');
  const datosClon = JSON.parse(JSON.stringify(original.config.data));
  if (esArea) datosClon.datasets.forEach(ds => { ds.fill = true; });

  cjsModalChart = new Chart(document.getElementById('cjs-modal-canvas'), {
    type: tipoAMostrar, data: datosClon,
    options: {
      responsive:true, maintainAspectRatio:false,
      indexAxis: (tipoAMostrar==='bar' && original.options?.indexAxis==='y') ? 'y' : undefined,
      plugins:{ legend:{ display:true, position:'top' }, datalabels:{ display:false } },
      scales: (tipoAMostrar==='pie'||tipoAMostrar==='doughnut') ? undefined :
              (tipoAMostrar==='scatter') ? { x:{title:{display:true,text:original.options?.scales?.x?.title?.text||''}}, y:{title:{display:true,text:original.options?.scales?.y?.title?.text||''}} } :
              { y:{ beginAtZero:true } },
    },
  });
}

// Detecta de qué habla el gráfico (por su título) para que el diagnóstico use el vocabulario
// correcto del rubro — no es lo mismo un ranking de "asesores" que uno de "módulos" o "canales".
function _cjsContextoNegocio(tituloLower) {
  if (/asesor|vendedor/.test(tituloLower)) return { sujeto:'asesor', sujetoPl:'asesores', accion:'vender/cobrar' };
  if (/modalidad|forma de pago|pago/.test(tituloLower)) return { sujeto:'modalidad de pago', sujetoPl:'modalidades de pago', accion:'elegir' };
  if (/categor/.test(tituloLower)) return { sujeto:'categoría de terreno', sujetoPl:'categorías', accion:'vender' };
  if (/m[oó]dulo/.test(tituloLower)) return { sujeto:'módulo', sujetoPl:'módulos', accion:'vender' };
  if (/canal|origen/.test(tituloLower)) return { sujeto:'canal de captación', sujetoPl:'canales', accion:'traer clientes' };
  if (/mora/.test(tituloLower)) return { sujeto:'nivel de mora', sujetoPl:'niveles de mora', accion:'atrasarse' };
  if (/estado/.test(tituloLower)) return { sujeto:'estado', sujetoPl:'estados', accion:'avanzar' };
  if (/m2|m²|precio/.test(tituloLower)) return { sujeto:'precio', sujetoPl:'precios', accion:'variar' };
  return { sujeto:'categoría', sujetoPl:'categorías', accion:'variar' };
}

function renderCjsEstadisticas() {
  const grid = document.getElementById('cjs-modal-stats-grid');
  const diagEl = document.getElementById('cjs-modal-diagnostico');
  const original = Chart.getChart(_cjsModalOrigenId);
  if (!original) { grid.innerHTML = '<div class="bi-stat-box">Sin datos.</div>'; diagEl.innerHTML=''; return; }

  const labels = original.data.labels || [];
  const ds = original.data.datasets.find(d => Array.isArray(d.data));
  if (!ds || !ds.data.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos para analizar.</div>'; diagEl.innerHTML=''; return; }

  const f = n => Math.round(n).toLocaleString('es-BO');
  const box = (label, value, destacado, tono) => `<div class="bi-stat-box${destacado?' hl':''}${tono==='warn'?' warn':''}"><div class="bi-stat-label">${label}</div><div class="bi-stat-value">${value}</div></div>`;
  const diag = (html, tono) => { diagEl.className = 'bi-insight' + (tono?' '+tono:''); diagEl.innerHTML = html; };

  const tipo = _cjsModalTipoAnalisis;
  const tituloActual = (document.getElementById('cjs-modal-title')?.textContent || '').toLowerCase();
  const ctx = _cjsContextoNegocio(tituloActual);

  /* ── DISPERSIÓN: relación entre 2 variables (ej: precio de venta vs. $/m²) ── */
  if (tipo === 'dispersion') {
    const puntos = ds.data.filter(p => p && typeof p.x === 'number' && typeof p.y === 'number');
    if (!puntos.length) { grid.innerHTML = '<div class="bi-stat-box">Sin puntos numéricos.</div>'; diagEl.innerHTML=''; return; }
    const xs = puntos.map(p=>p.x), ys = puntos.map(p=>p.y);
    const mx = HudStats.mean(xs), my = HudStats.mean(ys);
    const covar = puntos.reduce((s,p)=>s+(p.x-mx)*(p.y-my),0)/puntos.length;
    const sx = HudStats.stddev(xs), sy = HudStats.stddev(ys);
    const correlacion = (sx && sy) ? covar/(sx*sy) : 0;
    const fuerza = Math.abs(correlacion) > .7 ? 'fuerte' : Math.abs(correlacion) > .4 ? 'moderada' : 'débil';
    const direccion = correlacion > .05 ? 'positiva' : correlacion < -.05 ? 'negativa' : 'prácticamente nula';

    diag(`<strong>Diagnóstico:</strong> la relación entre ambas variables es <strong>${fuerza}</strong> y ${direccion} (r=${correlacion.toFixed(2)}). ` +
      `<strong>Causa probable:</strong> ${Math.abs(correlacion)>.6 ? 'el precio de venta sigue de cerca al valor por m² — la política de precios se está aplicando de forma consistente en las ventas.' : 'hay ventas con precios que no siguen el patrón esperado por m² — puede haber descuentos discrecionales fuera de la política, o terrenos con condiciones especiales (esquina, cercanía a amenities) que justifican el desvío.'} ` +
      `<strong>Prescripción:</strong> ${Math.abs(correlacion)<.4 ? 'revisar caso por caso los puntos que más se alejan de la tendencia — puede haber descuentos que excedieron lo autorizado en Panel de Control.' : 'la política de precios está funcionando bien, mantener el criterio actual.'}`,
      Math.abs(correlacion)<.4 ? 'warn' : 'pos');

    grid.innerHTML = [
      box('Correlación', correlacion.toFixed(2), true, Math.abs(correlacion)<.3?'warn':null),
      box('Promedio eje X', f(mx)),
      box('Promedio eje Y', f(my)),
      box('N° de puntos', puntos.length),
    ].join('');
    return;
  }

  /* ── COMPOSICIÓN: partes de un todo — qué modalidad/categoría/canal domina ── */
  if (tipo === 'composicion') {
    const valores = ds.data.map(v=>Number(v)||0);
    const total = valores.reduce((s,v)=>s+v,0);
    const pares = labels.map((l,i)=>({l, v:valores[i]})).sort((a,b)=>b.v-a.v);
    const top = pares[0], top2 = pares[1];
    const pctTop = total ? (top.v/total*100) : 0;
    const pctTop2Juntas = total ? ((top.v+(top2?top2.v:0))/total*100) : 0;
    const concentrado = pctTop2Juntas > 80;

    diag(`<strong>Diagnóstico:</strong> "<strong>${top?.l||'—'}</strong>" concentra el ${pctTop.toFixed(0)}% del total (${f(top?.v||0)} de ${f(total)}) entre las ${ctx.sujetoPl}. ` +
      `<strong>Causa probable:</strong> ${concentrado ? `casi todo el volumen está en 1 o 2 ${ctx.sujetoPl} — puede ser una preferencia genuina del mercado, o que las demás opciones no se están ofreciendo/promocionando con la misma fuerza.` : `el volumen está bien repartido entre varias ${ctx.sujetoPl}, sin una que domine claramente.`} ` +
      `<strong>Prescripción:</strong> ${concentrado ? `si depende de un proceso manual, vale la pena reforzar seguimiento ahí específicamente — es donde está concentrado el riesgo y la oportunidad.` : `mantener el seguimiento parejo entre todas las ${ctx.sujetoPl}.`}`,
      concentrado ? 'warn' : 'pos');

    grid.innerHTML = [
      box('Total', f(total), true),
      box('Líder', `${top?.l||'—'} (${pctTop.toFixed(0)}%)`),
      box('N° de categorías', pares.length),
    ].join('');
    return;
  }

  /* ── RANKING: comparación entre personas/categorías — quién lidera, cuál es la brecha ── */
  if (tipo === 'ranking') {
    const valores = ds.data.map(v=>Number(v)||0);
    const pares = labels.map((l,i)=>({l, v:valores[i]})).sort((a,b)=>b.v-a.v);
    const lider = pares[0], ultimo = pares[pares.length-1];
    const media = HudStats.mean(valores);
    const sobreLaMedia = valores.filter(v=>v>media).length;
    const brechaRelativa = media ? ((lider?.v||0)-(ultimo?.v||0))/media : 0;
    const brechaGrande = brechaRelativa > 1.5;

    diag(`<strong>Diagnóstico:</strong> <strong>${lider?.l||'—'}</strong> lidera con ${f(lider?.v||0)}, muy por encima del promedio del grupo (${f(media)}). El último de la lista, ${ultimo?.l||'—'}, tiene ${f(ultimo?.v||0)}. ` +
      `<strong>Causa probable:</strong> ${brechaGrande ? `la diferencia entre el primero y el último es muy grande — puede deberse a antigüedad/experiencia distinta, calidad de los leads asignados, o zona/cartera de cada ${ctx.sujeto}.` : `el grupo está relativamente parejo, sin una brecha que llame la atención.`} ` +
      `<strong>Prescripción:</strong> ${brechaGrande ? `vale la pena que ${lider?.l||'el líder'} comparta su método con el resto del equipo, y revisar si a los de abajo les está faltando algo (leads, capacitación, seguimiento).` : `el desempeño del grupo es consistente, mantener el acompañamiento actual.`}`,
      brechaGrande ? 'warn' : 'pos');

    grid.innerHTML = [
      box('Líder', `${lider?.l||'—'}`, true),
      box('Último lugar', `${ultimo?.l||'—'}`),
      box('Promedio del grupo', f(media)),
      box('Por encima del promedio', `${sobreLaMedia} de ${pares.length}`),
    ].join('');
    return;
  }

  /* ── TENDENCIA (por defecto): cómo evoluciona en el tiempo — cobranza, ventas, registros ── */
  const valores = ds.data.map(v => (v && typeof v==='object') ? Number(v.y) : Number(v)).filter(v=>!isNaN(v));
  if (!valores.length) { grid.innerHTML = '<div class="bi-stat-box">Sin datos numéricos.</div>'; diagEl.innerHTML=''; return; }
  const primero = valores[0], ultimo = valores[valores.length-1];
  const crecimientoTotal = primero ? ((ultimo-primero)/Math.abs(primero)*100) : 0;
  const idxMax = valores.indexOf(Math.max(...valores));
  const idxMin = valores.indexOf(Math.min(...valores));
  const xs = valores.map((_,i)=>i);
  const mx = HudStats.mean(xs), my = HudStats.mean(valores);
  const num = xs.reduce((s,x,i)=>s+(x-mx)*(valores[i]-my),0);
  const den = xs.reduce((s,x)=>s+(x-mx)**2,0);
  const pendiente = den ? num/den : 0;
  const subiendo = pendiente > my*0.02, bajando = pendiente < -my*0.02;
  const direccionTendencia = subiendo ? 'creciendo' : bajando ? 'bajando' : 'estable';
  const proyeccionSiguiente = Math.max(0, ultimo + pendiente);

  diag(`<strong>Diagnóstico:</strong> el valor actual es ${f(ultimo)}, con una tendencia <strong>${direccionTendencia}</strong> (${crecimientoTotal>=0?'+':''}${crecimientoTotal.toFixed(1)}% en todo el período mostrado). El mejor momento fue ${labels[idxMax]||'—'} (${f(valores[idxMax])}); el más flojo, ${labels[idxMin]||'—'} (${f(valores[idxMin])}). ` +
    `<strong>Causa probable:</strong> ${bajando ? 'una caída sostenida suele venir de menos seguimiento/categorización pendiente, estacionalidad del rubro, o menos entradas nuevas en ese período.' : subiendo ? 'el crecimiento sostenido normalmente refleja más captación, mejor seguimiento, o una campaña/promoción que está funcionando.' : 'el comportamiento estable indica un ritmo consolidado, sin sobresaltos.'} ` +
    `<strong>Prescripción:</strong> ${bajando ? 'revisar qué cambió justo antes de la caída — a partir de ' + (labels[idxMax]||'ese punto') + ' — y reforzar ahí el seguimiento.' : 'mantener lo que se está haciendo — la proyección para el próximo período, siguiendo este ritmo, es de ' + f(proyeccionSiguiente) + '.'}`,
    bajando ? 'warn' : subiendo ? 'pos' : null);

  grid.innerHTML = [
    box('Valor actual', f(ultimo), true),
    box('Crecimiento del período', (crecimientoTotal>=0?'+':'')+crecimientoTotal.toFixed(1)+'%', false, Math.abs(crecimientoTotal)>25?'warn':null),
    box('Mejor momento', `${labels[idxMax]||'—'}`),
    box('Proyección próximo período', f(proyeccionSiguiente)),
  ].join('');
}
// Compara el PERÍODO que la cuota corresponde pagar (Mes/Anio que da Orange) contra el mes
// en que efectivamente se pagó (FechaDePago) — esto es un dato directo y confiable, distinto
// del EstadoCuota/EstadoPago de Orange (que ya sabemos que no es confiable).
//   Paga el mismo mes que le toca         -> Vigente
//   Paga un mes ANTERIOR al que le toca   -> Mora (se atrasó)
//   Paga un mes POSTERIOR al que le toca  -> Adelantado (pagó por delante)
function _calcularCategoriaAutoExpensa(mesCuota, anioCuota, fechaPagoMs) {
  if (!mesCuota || !anioCuota || !fechaPagoMs) return null;
  const nombresMes = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
  const mesCuotaNum = nombresMes.indexOf(String(mesCuota).toLowerCase().trim());
  if (mesCuotaNum === -1) return null;
  const anioCuotaNum = parseInt(anioCuota);
  if (isNaN(anioCuotaNum)) return null;

  const fPago = new Date(fechaPagoMs);
  const cuotaComoIndice = anioCuotaNum*12 + mesCuotaNum;
  const pagoComoIndice = fPago.getFullYear()*12 + fPago.getMonth();

  if (cuotaComoIndice === pagoComoIndice) return 'Vigente';
  if (cuotaComoIndice < pagoComoIndice) return 'Mora';
  return 'Adelantado';
}

// Categoriza automáticamente todo lo que todavía no tenga categoría asignada y sí tenga
// un mes/año de cuota confiable — usa el mismo criterio que definió el cliente:
// paga el mes que le toca = Vigente, paga uno anterior = Mora, uno posterior = Adelantado.
// Queda registrado como "automático" — si algo no cuadra, se puede recategorizar a mano
// desde la cola de "Categorizar cobros" en cualquier momento, no es definitivo ni oculta nada.
// Busca por nombre de cliente o terreno, y muestra CADA cuota que pagó, con su categoría —
// esto es lo que en la versión anterior era "elegís un cliente y se desglosa todo lo que pagó".
function renderBusquedaClienteExpensas() {
  const cont = document.getElementById('exp-busqueda-cliente-resultado');
  if (!cont) return;
  const term = (document.getElementById('exp-buscar-cliente')?.value||'').trim().toLowerCase();
  if (!term || term.length < 3) { cont.innerHTML = term ? '<div class="empty-state">Seguí escribiendo (mínimo 3 letras)...</div>' : ''; return; }

  const coincidencias = orangeExpensasDetalleRaw.filter(r =>
    r.cliente.toLowerCase().includes(term) || r.terreno.toLowerCase().includes(term)
  );
  if (!coincidencias.length) { cont.innerHTML = '<div class="empty-state">Sin coincidencias.</div>'; return; }

  // Agrupamos por cliente+terreno, porque un mismo cliente puede tener más de un lote.
  const grupos = {};
  coincidencias.forEach(r => {
    const clave = r.cliente + '|' + r.terreno;
    (grupos[clave] ||= { cliente:r.cliente, terreno:r.terreno, cuotas:[], total:0 });
    grupos[clave].cuotas.push(r);
    grupos[clave].total += r.monto;
  });

  const badges = { Vigente:'background:var(--ok-bg);color:var(--ok-ink);', Mora:'background:var(--danger-bg);color:var(--danger-ink);',
    Adelantado:'background:var(--info-bg);color:var(--info-ink);', 'Otros Ingresos':'background:var(--fill);color:var(--ink-700);' };

  cont.innerHTML = Object.values(grupos).map((g, i) => {
    const cuotasOrdenadas = [...g.cuotas].sort((a,b)=>(b.fechaOrden||0)-(a.fechaOrden||0));
    return `
    <div style="border:1px solid var(--line);border-radius:var(--r-md);margin-bottom:10px;overflow:hidden;">
      <div style="padding:12px 16px;background:var(--fill);cursor:pointer;display:flex;justify-content:space-between;align-items:center;" onclick="document.getElementById('exp-busq-det-${i}').style.display = document.getElementById('exp-busq-det-${i}').style.display==='none'?'':'none';">
        <div><strong>${esc(g.cliente)}</strong> — <span style="font-family:monospace;color:var(--gris);">${esc(g.terreno)}</span></div>
        <div style="font-weight:700;color:var(--cartera);">Bs${g.total.toLocaleString('es-BO',{minimumFractionDigits:2})} <span style="color:var(--gris);font-weight:400;font-size:var(--fs-xs);">(${g.cuotas.length} cuota${g.cuotas.length!==1?'s':''}) ▾</span></div>
      </div>
      <div id="exp-busq-det-${i}" style="display:none;">
        <table style="font-size:var(--fs-sm);"><thead><tr><th>Fecha de pago</th><th>Mes que paga</th><th>Monto</th><th>Categoría</th></tr></thead>
        <tbody>
          ${cuotasOrdenadas.map(c => {
            const cat = expensasCategoriasData[c.key]?.categoria;
            return `<tr>
              <td>${esc(c.fecha)}</td>
              <td>${c.mesCuota ? esc(c.mesCuota)+' '+esc(c.anioCuota) : '—'}</td>
              <td style="font-weight:600;">Bs${c.monto.toLocaleString('es-BO',{minimumFractionDigits:2})}</td>
              <td>${cat ? `<span style="padding:2px 9px;border-radius:999px;font-size:var(--fs-xs);font-weight:600;${badges[cat]||''}">${esc(cat)}</span>` : '<span style="color:var(--warn);font-size:var(--fs-xs);">sin categorizar</span>'}</td>
            </tr>`;
          }).join('')}
        </tbody></table>
      </div>
...
async function categorizarAutomaticamenteExpensas() {
  const paraGuardar = {};
  orangeExpensasDetalleRaw.forEach(r => {
    if (expensasCategoriasData[r.key]) return; // ya tiene categoría (manual o automática previa)
    if (!r.categoriaAuto) return; // sin mes/año de cuota confiable — se deja pendiente, para revisar a mano
    paraGuardar[r.key] = r.categoriaAuto;
  });
  const claves = Object.keys(paraGuardar);
  if (!claves.length) return;

  // Guardamos en lotes chicos (no los 14.000 juntos en una sola escritura) — así cada
  // lote es rápido, y si uno falla no se pierde lo que ya se guardó de los anteriores.
  const TAMANO_LOTE = 500;
  let guardados = 0;
  for (let i = 0; i < claves.length; i += TAMANO_LOTE) {
    const loteClaves = claves.slice(i, i + TAMANO_LOTE);
    const lote = {};
    loteClaves.forEach(k => { lote[k] = paraGuardar[k]; });
    try {
      await window._fbSetCategoriasExpensaEnLote(lote);
      guardados += loteClaves.length;
    } catch (e) {
      console.error('[CATEGORIZACION-AUTO] Error en el lote', i, '-', i+TAMANO_LOTE, ':', e);
      break; // paramos ahí — lo ya guardado queda bien, el resto se reintenta en la próxima sincronización
    }
    await new Promise(r => setTimeout(r, 50)); // pausa chica entre lotes, para no saturar
  }
  if (guardados > 0) {
    toastOk(`${guardados} cobro${guardados!==1?'s':''} categorizado${guardados!==1?'s':''} automáticamente (mes de cuota vs. mes de pago).`);
  }
}

async function cargarDetalleExpensas(silencioso) {
  const btn = document.getElementById('btn-refrescar-cobranza-exp');
  const setBtn = (txt, disabled) => { if (!silencioso && btn) { btn.textContent = txt; btn.disabled = disabled; } };
  setBtn('🔄 Cargando...', true);
  try {
    const resp = await fetch(ORANGE_API_URL + '?op=GetExpensas');
    if (!resp.ok) throw new Error('HTTP '+resp.status);
    ...
    try {
      ...
      // Orange no devolvió JSON — mostramos el principio de lo que sí mandó, así se ve
      // si es un error de SQL de ellos, un timeout, una página de error, etc.
      console.error('[ORANGE-EXPENSAS] Respuesta no es JSON. Primeros 300 caracteres:', textoCrudo.slice(0,300));
      throw new Error('Orange no devolvió datos válidos — empieza con: "' + textoCrudo.slice(0,80) + '..." (mirá la consola para verlo completo, esto parece un error del lado de Orange, no nuestro)');
    }
    if (!json.success || !Array.isArray(json.data)) throw new Error('Respuesta inesperada de Orange');

    orangeExpensasDetalleRaw = json.data.map(r => {
      const monto = r.Cobranza_BS || 0;
      const key = `${r.Terreno}_${r.FechaDePago}_${monto}`.replace(/[.#$\[\]\/\s]/g,'-');
      const fechaOrden = _parseFechaOrange(r.FechaDePago)?.getTime() || 0;
      return {
        key, fecha: r.FechaDePago || '', fechaOrden,
        cliente: r.NombreCliente||'', terreno: r.Terreno||'', asesor: r.Asesor||'',
        modalidad: r.Modalidad||'Sin modalidad', modulo: r.Modulo||'Sin módulo', monto,
        mesCuota: r.Mes||'', anioCuota: r.Anio||'', // el período que esta cuota corresponde pagar — no cuándo se pagó
        categoriaAuto: _calcularCategoriaAutoExpensa(r.Mes, r.Anio, fechaOrden),
      };
    }).filter(r => r.monto > 0);

    categorizarAutomaticamenteExpensas(); // sin await — corre en segundo plano, no bloquea el resto
    await renderCobranzaExpensasKPIs(); // mantiene poblados los selectores de Año/Gestión de arriba
    renderDashboardExpensas();
    renderCategorizacionExpensas();
    renderResumenMensualExpensas();
    renderHistoricoMensualExpensas();
    renderSemanalExpensas();
    renderDetalleSemanaExpensas();
    renderBusquedaClienteExpensas();

    const lbl = new Date().toLocaleString('es-BO') + (silencioso ? ' (automática)' : ' (manual)');
    setText('dbe-sync-label', lbl);
    if (!silencioso) toastOk('Cobranza actualizada: ' + orangeExpensasDetalleRaw.length + ' cobros cargados.');
  } catch(e) {
    if (!silencioso) toastErr('No se pudo cargar el detalle: ' + e.message);
  } finally {
    setBtn('🔄 Actualizar ahora', false);
  }
}

async function renderCobranzaExpensasKPIs() {
  const hoy = new Date();
  const selAnio = document.getElementById('fil-exp-anio');
  if (selAnio && !selAnio.dataset.filled) {
    const anioHoy = hoy.getFullYear();
    const anios = [...new Set(orangeExpensasDetalleRaw.map(r => r.fechaOrden ? new Date(r.fechaOrden).getFullYear() : null).filter(Boolean))];
    if (!anios.includes(anioHoy)) anios.push(anioHoy);
    anios.sort((a,b)=>b-a);
    selAnio.innerHTML = anios.map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.value = expFilAnio;
    selAnio.dataset.filled = '1';
  }
  const selMes = document.getElementById('fil-exp-mes-global');
  if (selMes) selMes.value = expFilMes;
  const selGestion = document.getElementById('fil-exp-gestion');
  if (selGestion && !selGestion.dataset.filled) {
    const gestionHoy = _gestionActual();
    if (expFilGestion === null) expFilGestion = gestionHoy;
    const gestiones = [gestionHoy, gestionHoy-1, gestionHoy-2, gestionHoy-3];
    selGestion.innerHTML = gestiones.map(g=>`<option value="${g}">Gestión ${_labelGestion(g)}</option>`).join('');
    selGestion.value = expFilGestion;
    selGestion.dataset.filled = '1';
  }
  ...
  const inicioSemana = new Date(hoy); inicioSemana.setDate(hoy.getDate()-diaSemana); inicioSemana.setHours(0,0,0,0);
  const finSemana = new Date(inicioSemana); finSemana.setDate(inicioSemana.getDate()+7);

  let cobradoSemana = 0, cobradoMes = 0, cobradoGestion = 0;
  orangeExpensasDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (f.getFullYear() === expFilAnio && f.getMonth() === mesFilNum) cobradoMes += r.monto;
    if (_gestionDeFecha(f) === expFilGestion) cobradoGestion += r.monto;
    if (r.fechaOrden >= inicioSemana.getTime() && r.fechaOrden < finSemana.getTime()) cobradoSemana += r.monto;
  });

  setText('exp-anio-actual', _labelGestion(expFilGestion));
  setText('exp-kpi-semana', _fmtMontoBs(cobradoSemana));
  setText('exp-kpi-mes', _fmtMontoBs(cobradoMes));
  setText('exp-kpi-gestion', _fmtMontoBs(cobradoGestion));

  const metaC = await window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_vigente');
  const metaMora = await window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_mora');
  const metaTotal = (metaC||0) + (metaMora||0);
  const elMeta = document.getElementById('exp-kpi-mes-meta');
  if (elMeta) {
    elMeta.innerHTML = metaTotal
      ? Math.round(cobradoMes/metaTotal*100) + '% de la meta (Bs'+metaTotal.toLocaleString('es-BO')+')'
      : '<a href="#" onclick="abrirModalMetaExpensasCategorias();return false;" style="color:var(--info);font-weight:600;">Sin meta — cargarla acá →</a>';
  }
}

let _chartExpensasSemanal = null;
function renderCobranzaExpensasSemanal() {
  const canvas = document.getElementById('exp-chart-semanal');
  if (!canvas) return;
  const hoy = new Date();
  const diaSemanaHoy = (hoy.getDay()+6)%7;
  const inicioSemanaActual = new Date(hoy); inicioSemanaActual.setDate(hoy.getDate()-diaSemanaHoy); inicioSemanaActual.setHours(0,0,0,0);
  ...
  if (_chartExpensasSemanal) _chartExpensasSemanal.destroy();
  _chartExpensasSemanal = new Chart(canvas, {
    type: 'bar',
    data: { labels: buckets.map(b=>b.label), datasets: [{ label:'Cobrado', data: buckets.map(b=>Math.round(b.total)), backgroundColor:'#7C3AED', borderRadius:5, maxBarThickness:28 }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>_fmtMontoBs(ctx.parsed.y)}} }, scales:{ x:{grid:{display:false}}, y:{beginAtZero:true, ticks:{callback:v=>_fmtMontoBs(v)}} } }
  });
}

let _chartExpensasMensual = null;
function renderCobranzaExpensasTendenciaMensual() {
  const canvas = document.getElementById('exp-chart-mensual');
  if (!canvas) return;
  const nombresMes = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const hoy = new Date();
  const buckets = [];
  for (let i = 11; i >= 0; i--) {
    const d = new Date(hoy.getFullYear(), hoy.getMonth()-i, 1);
    buckets.push({ anio: d.getFullYear(), mes: d.getMonth(), label: nombresMes[d.getMonth()]+' '+String(d.getFullYear()).slice(2), total: 0 });
  }
  orangeExpensasDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
  ...
  if (_chartExpensasMensual) _chartExpensasMensual.destroy();
  _chartExpensasMensual = new Chart(canvas, {
    type: 'line',
    data: { labels: buckets.map(b=>b.label), datasets: [{ label:'Cobrado', data: buckets.map(b=>Math.round(b.total)), borderColor:'#2451FF', backgroundColor:'#2451FF', tension:.3, pointRadius:4 }] },
    options: {
      responsive:true, maintainAspectRatio:false, layout:{padding:{top:20}},
      plugins:{ legend:{display:false}, tooltip:{callbacks:{label:ctx=>_fmtMontoBs(ctx.parsed.y)}},
        datalabels:{ display:true, align:'top', anchor:'end', offset:5, color:'#0A0A0A', font:{weight:'700',size:10}, formatter:v=>_fmtMontoBs(v) } },
      scales:{ x:{grid:{display:false}}, y:{beginAtZero:true, ticks:{callback:v=>_fmtMontoBs(v)}} }
    }
  });
}

function renderCobranzaExpensasPorModalidad() {
  const cont = document.getElementById('exp-por-modalidad');
  if (!cont) return;
  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(expFilMes);
  const porModalidad = {};
  ...
  const entries = Object.entries(porModalidad).sort((a,b)=>b[1]-a[1]);
  if (!entries.length) { cont.innerHTML = '<div class="empty-state">Sin cobros en ' + expFilMes + ' ' + expFilAnio + '.</div>'; return; }
  ...
  cont.innerHTML = entries.map(([k,v]) => `
    <div style="margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;font-size:var(--fs-sm);margin-bottom:3px;">
        ...
        <span style="color:var(--cartera);font-weight:700;">${_fmtMontoBs(v)}</span>
      </div>
      <div style="height:9px;background:var(--fill);border-radius:99px;overflow:hidden;">
        <div style="height:100%;width:${(v/max*100).toFixed(1)}%;background:var(--cartera);"></div>
      </div>
    </div>`).join('');
}

/* ── Categorización manual de cobros de Expensas ── */
async function asignarCategoriaExpensa(cobroKey, categoria) {
  if (!categoria) return;
  await window._fbSetCategoriaExpensa(cobroKey, { categoria, asignadoPorNombre: asesorActual?.nombre || 'Admin' });
  toastOk('Categorizado como "' + categoria + '".');
}

function renderCategorizacionExpensas() {
  const tbody = document.getElementById('exp-categ-tbody');
  const contador = document.getElementById('exp-categ-pendientes');
  if (!tbody) return;
  const pendientes = orangeExpensasDetalleRaw.filter(r => !expensasCategoriasData[r.key]);
  if (contador) contador.textContent = pendientes.length + ' pendiente' + (pendientes.length!==1?'s':'') + ' por categorizar';
  if (!pendientes.length) { tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">✅ Todos los cobros cargados están categorizados.</div></td></tr>'; return; }
  const opciones = (auto) => '<option value="">Elegir categoría...</option>' + EXPENSAS_CATEGORIAS.map(c=>`<option value="${c}"${c===auto?' selected':''}>${c}${c===auto?' (sugerido)':''}</option>`).join('');
  tbody.innerHTML = pendientes.slice(0,200).map(r => `<tr>
    <td style="font-size:var(--fs-sm);">${esc(r.fecha)}</td>
    <td>${esc(r.cliente)}</td>
    <td style="font-family:monospace;font-size:var(--fs-sm);">${esc(r.terreno)}</td>
    <td style="font-size:var(--fs-sm);">${r.mesCuota ? esc(r.mesCuota)+' '+esc(r.anioCuota) : '<span style="color:var(--gris);">sin dato</span>'}</td>
    <td style="font-weight:600;color:var(--cartera);">Bs${r.monto.toLocaleString('es-BO',{minimumFractionDigits:2})}</td>
    <td><select onchange="asignarCategoriaExpensa('${r.key}', this.value)" style="padding:6px 10px;border:1.5px solid var(--line);border-radius:var(--r-sm);font-size:var(--fs-sm);font-family:'DM Sans',sans-serif;">${opciones(r.categoriaAuto)}</select></td>
  </tr>`).join('') + (pendientes.length>200 ? `<tr><td colspan="6" style="text-align:center;color:var(--gris);padding:10px;">Mostrando 200 de ${pendientes.length}.</td></tr>` : '');
}

function _sumaCategoriaExp(cobros, categorias) {
  return cobros.filter(r => categorias.includes(expensasCategoriasData[r.key]?.categoria)).reduce((s,r)=>s+r.monto,0);
}

// Histórico mensual: Mes | Cobranza | Meta | % Cumplimiento — varios meses juntos, para
// comparar la evolución mes a mes de un vistazo (no solo el mes que tenés filtrado arriba).
async function renderHistoricoMensualExpensas() {
  const tbody = document.getElementById('exp-historico-mensual-tbody');
  if (!tbody) return;
  if (!orangeExpensasDetalleRaw.length) { tbody.innerHTML = '<tr><td colspan="4"><div class="empty-state">Sin datos todavía — sincronizá arriba.</div></td></tr>'; return; }

  const meses12 = _agruparPorMes_dbe(orangeExpensasDetalleRaw, 12);
  const nombresMesLargo = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const filas = await Promise.all(meses12.map(async m => {
    const [metaV, metaM] = await Promise.all([
      window._fbGetMetaOnce(m.anio, nombresMesLargo[m.mes], 'expensas_vigente'),
      window._fbGetMetaOnce(m.anio, nombresMesLargo[m.mes], 'expensas_mora'),
    ]);
    const meta = (metaV||0)+(metaM||0);
    const pct = meta ? Math.round(m.total/meta*100) : null;
    return { ...m, meta, pct };
  }));

  tbody.innerHTML = [...filas].reverse().map(f => `<tr>
    <td style="font-weight:600;">${nombresMesLargo[f.mes]} ${f.anio}</td>
    <td style="font-weight:600;color:var(--cartera);">${_fmtMontoBs(f.total)}</td>
    <td>${f.meta ? _fmtMontoBs(f.meta) : '<span style="color:var(--gris);">sin meta</span>'}</td>
    <td>${f.pct!==null ? `<span style="font-weight:700;color:${f.pct>=100?'var(--ok)':f.pct>=75?'var(--warn)':'var(--danger)'}">${f.pct}%</span>` : '—'}</td>
  </tr>`).join('');
}

async function renderResumenMensualExpensas() {
  const cont = document.getElementById('exp-resumen-mensual');
  if (!cont) return;
  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(expFilMes);
  const delMes = orangeExpensasDetalleRaw.filter(r => {
    if (!r.fechaOrden) return false;
    const f = new Date(r.fechaOrden);
    return f.getFullYear()===expFilAnio && f.getMonth()===mesFilNum;
  });

  const metaVigente = await window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_vigente') || 0;
  const metaMora    = await window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_mora') || 0;
  const metaVM = metaVigente + metaMora;

  const cVigente = _sumaCategoriaExp(delMes, ['Vigente']);
  const cMora    = _sumaCategoriaExp(delMes, ['Mora']);
  const cVigenteMora = cVigente + cMora;
  const cAdelanto = _sumaCategoriaExp(delMes, ['Adelanto']);
  const cOtros    = _sumaCategoriaExp(delMes, ['Otros Ingresos']);
  const totalCobranza = cVigenteMora + cAdelanto + cOtros;
  const pctMeta = metaVM ? Math.round(cVigenteMora/metaVM*100) : null;

  const fila = (label, valor, destacado) => `
    <tr style="${destacado?'background:var(--ok-bg,#e8f5e9);font-weight:700;':''}">
      <td style="padding:8px 12px;">${label}</td>
      <td style="padding:8px 12px;text-align:right;">${_fmtMontoBs(valor)}</td>
    </tr>`;

  cont.innerHTML = `
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:var(--ink-900);color:#fff;"><th style="padding:8px 12px;text-align:left;">Meta Expensas ${expFilMes} ${expFilAnio}</th><th style="padding:8px 12px;text-align:right;">Bs</th></tr></thead>
      <tbody>
        ${fila('Meta Cobranza Vigente', metaVigente)}
        ${fila('Meta Cobranza Mora', metaMora)}
        ${fila('Meta Cobranza Vigente y Mora', metaVM, true)}
        ${fila('Cobranza Vigente', cVigente)}
        ${fila('Cobranza Mora', cMora)}
        ${fila('Cobranza Vigente y Mora' + (pctMeta!==null ? ' — '+pctMeta+'% de la meta' : ''), cVigenteMora, true)}
        ${fila('Cobranza Adelantado', cAdelanto)}
        ${fila('Otros Ingresos', cOtros)}
        ${fila('TOTAL COBRANZA', totalCobranza, true)}
      </tbody>
    </table>
    <div style="margin-top:12px;">
      <button onclick="abrirModalMetaExpensasCategorias()" class="btn-secondary btn-sm">⚙ Cargar meta Vigente/Mora de ${expFilMes}</button>
    </div>
    <p style="font-size:10px;color:#999;margin-top:10px;font-family:monospace;">🔍 Diagnóstico — buscando en: metas/${expFilAnio}/${expFilMes}/expensas_vigente (leyó: ${metaVigente}) y metas/${expFilAnio}/${expFilMes}/expensas_mora (leyó: ${metaMora})</p>`;
}

let _chartSemanalExpensas = null;
async function renderSemanalExpensas() {
  const tbody = document.getElementById('exp-semanal-tbody');
  const canvas = document.getElementById('exp-chart-semanal-meta');
  if (!tbody) return;
  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(expFilMes);
  const delMes = orangeExpensasDetalleRaw.filter(r => {
  ...
  const porSemana = {};
  delMes.forEach(r => {
    const { anio, semana } = _semanaISO(new Date(r.fechaOrden));
    const key = anio+'-'+semana;
    (porSemana[key] ||= { anio, semana, total:0, cobranzaMeta:0 });
    porSemana[key].total += r.monto;
    const cat = expensasCategoriasData[r.key]?.categoria;
    if (cat === 'Vigente' || cat === 'Mora') porSemana[key].cobranzaMeta += r.monto;
  });
  const semanas = Object.values(porSemana).sort((a,b)=>a.semana-b.semana);
  if (!semanas.length) { tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Sin cobros en ' + expFilMes + ' ' + expFilAnio + '.</div></td></tr>'; return; }

  // La meta MENSUAL (Vigente+Mora) se reparte entre las semanas según cuántos días de
  // cada semana caen dentro de ESTE mes — antes esta columna no existía de verdad, mostraba
  // por error el mismo número que "Cobranza meta" (el monto cobrado, no un objetivo).
  const [metaVigenteMes, metaMoraMes] = await Promise.all([
    window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_vigente'),
    window._fbGetMetaOnce(expFilAnio, expFilMes, 'expensas_mora'),
  ]);
  const metaMensualTotal = (metaVigenteMes||0) + (metaMoraMes||0);
  const primerDiaMes = new Date(expFilAnio, mesFilNum, 1);
  const ultimoDiaMes = new Date(expFilAnio, mesFilNum+1, 0);
  const diasTotalesMes = ultimoDiaMes.getDate();

  semanas.forEach(s => {
    // Reconstruimos el rango real (lunes a domingo) de esta semana ISO para saber
    // cuántos de sus días caen dentro del mes que estamos mirando.
    const lunes = new Date(s.anio, 0, 1 + (s.semana-1)*7);
    while ((lunes.getDay()+6)%7 !== 0) lunes.setDate(lunes.getDate()-1); // retrocede al lunes
    let diasEnEsteMes = 0;
    for (let i=0;i<7;i++) {
      const d = new Date(lunes); d.setDate(lunes.getDate()+i);
      if (d.getMonth()===mesFilNum && d.getFullYear()===expFilAnio) diasEnEsteMes++;
    }
    s.metaAsignada = metaMensualTotal * (diasEnEsteMes/diasTotalesMes);
    s.pctMeta = s.metaAsignada ? Math.round(s.cobranzaMeta/s.metaAsignada*100) : null;
  });

  tbody.innerHTML = semanas.map(s => `<tr>
    <td style="font-weight:600;">Semana ${s.semana}</td>
    <td style="font-weight:600;color:var(--cartera);">${_fmtMontoBs(s.total)}</td>
    <td>${_fmtMontoBs(s.metaAsignada)}</td>
    <td style="color:var(--ok);">${_fmtMontoBs(s.cobranzaMeta)}</td>
    <td style="font-weight:600;color:${s.pctMeta===null?'var(--gris)':s.pctMeta>=100?'var(--ok)':s.pctMeta>=70?'var(--warn)':'var(--danger)'};">${s.pctMeta===null?'—':s.pctMeta+'%'}</td>
    <td style="color:var(--gris);">${_fmtMontoBs(s.total-s.cobranzaMeta)}</td>
  </tr>`).join('') + `<tr style="font-weight:700;background:var(--bg-soft);">
    <td>Total</td>
    <td>${_fmtMontoBs(semanas.reduce((s,x)=>s+x.total,0))}</td>
    <td>${_fmtMontoBs(semanas.reduce((s,x)=>s+x.metaAsignada,0))}</td>
    <td>${_fmtMontoBs(semanas.reduce((s,x)=>s+x.cobranzaMeta,0))}</td>
    <td>${metaMensualTotal ? Math.round(semanas.reduce((s,x)=>s+x.cobranzaMeta,0)/metaMensualTotal*100)+'%' : '—'}</td>
    <td>${_fmtMontoBs(semanas.reduce((s,x)=>s+(x.total-x.cobranzaMeta),0))}</td></tr>`;

  if (canvas) {
    if (_chartSemanalExpensas) _chartSemanalExpensas.destroy();
    _chartSemanalExpensas = new Chart(canvas, {
      type: 'line',
      data: { labels: semanas.map(s=>'Sem. '+s.semana), datasets: [
        { label:'Cobranza total', data: semanas.map(s=>Math.round(s.total)), borderColor:'#E8590C', backgroundColor:'#E8590C', tension:.3 },
        { label:'Meta de la semana', data: semanas.map(s=>Math.round(s.metaAsignada)), borderColor:'#666', backgroundColor:'#666', borderDash:[5,4], tension:.3 },
        { label:'Cobranza para meta', data: semanas.map(s=>Math.round(s.cobranzaMeta)), borderColor:'#2451FF', backgroundColor:'#2451FF', tension:.3 },
      ]},
      options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{position:'top', labels:{boxWidth:10,font:{size:10}}}, datalabels:{display:false} }, scales:{ y:{beginAtZero:true, ticks:{callback:v=>_fmtMontoBs(v)}} } }
    });
  }
}

function renderDetalleSemanaExpensas() {
  const tbody = document.getElementById('exp-detalle-semana-tbody');
  if (!tbody) return;
  const selAnio = document.getElementById('fil-exp-semana-anio');
  const selSemana = document.getElementById('fil-exp-semana-num');
  const hoyIso = _semanaISO(new Date());
  if (selAnio && !selAnio.dataset.filled) {
    selAnio.innerHTML = [hoyIso.anio, hoyIso.anio-1, hoyIso.anio-2].map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.dataset.filled = '1';
  }
  if (selSemana && !selSemana.dataset.filled) {
    let opts = ''; for (let i=1;i<=53;i++) opts += `<option value="${i}">Semana ${i}</option>`;
    selSemana.innerHTML = opts; selSemana.value = hoyIso.semana; selSemana.dataset.filled = '1';
  }
  const anio = parseInt(document.getElementById('fil-exp-semana-anio')?.value) || new Date().getFullYear();
  const semana = parseInt(document.getElementById('fil-exp-semana-num')?.value) || 1;
  const cobros = orangeExpensasDetalleRaw.filter(r => {
    if (!r.fechaOrden) return false;
    const iso = _semanaISO(new Date(r.fechaOrden));
    return iso.anio === anio && iso.semana === semana;
  });
  if (!cobros.length) { tbody.innerHTML = '<tr><td colspan="3"><div class="empty-state">Sin cobros en la Semana ' + semana + '/' + String(anio).slice(2) + '.</div></td></tr>'; return; }
  const porCategoria = {};
  cobros.forEach(r => { const cat = expensasCategoriasData[r.key]?.categoria || 'Sin categorizar'; (porCategoria[cat] ||= []).push(r); });
  const ordenCat = [...EXPENSAS_CATEGORIAS, 'Sin categorizar'];
  const catsOrdenadas = Object.keys(porCategoria).sort((a,b)=>ordenCat.indexOf(a)-ordenCat.indexOf(b));
  let html = ''; let totalGeneral = 0;
  catsOrdenadas.forEach(cat => {
    const items = porCategoria[cat]; const subtotal = items.reduce((s,r)=>s+r.monto,0); totalGeneral += subtotal;
    html += `<tr style="background:var(--bg-soft);"><td style="font-weight:700;" colspan="2">${esc(cat)}</td><td></td></tr>`;
    items.forEach(r => { html += `<tr><td></td><td>${esc(r.cliente)}</td><td style="text-align:right;">Bs${r.monto.toLocaleString('es-BO')}</td></tr>`; });
    html += `<tr style="background:var(--ok-bg,#e8f5e9);font-weight:600;"><td colspan="2">Sub Total</td><td style="text-align:right;">${_fmtMontoBs(subtotal)}</td></tr>`;
  });
  html += `<tr style="background:var(--ink-900);color:#fff;font-weight:700;"><td colspan="2">Total general</td><td style="text-align:right;">${_fmtMontoBs(totalGeneral)}</td></tr>`;
  tbody.innerHTML = html;
}

/* ── Modal de Meta Vigente/Mora para Expensas ── */
function abrirModalMetaExpensasCategorias() {
  console.log('[DIAG] abrirModalMetaExpensasCategorias — se ejecutó el click');
  try {
    const selAnio = document.getElementById('mec-anio');
    console.log('[DIAG] selAnio encontrado:', !!selAnio);
    if (!selAnio.options.length) {
      const anioHoy = new Date().getFullYear();
      selAnio.innerHTML = [anioHoy+1, anioHoy, anioHoy-1, anioHoy-2].map(a=>`<option value="${a}">${a}</option>`).join('');
    }
    selAnio.value = expFilAnio;
    document.getElementById('mec-mes').value = expFilMes;
    cargarMetaExpensasCategoriasExistente();
    const modal = document.getElementById('modal-meta-expensas-categorias');
    console.log('[DIAG] modal encontrado:', !!modal, '— clases antes:', modal?.className);
    modal.classList.add('open');
    console.log('[DIAG] clases después de abrir:', modal.className, '— display calculado:', getComputedStyle(modal).display);
  } catch (e) {
    console.error('[DIAG] Error real al abrir el modal:', e);
    alert('Error al abrir el modal: ' + e.message);
  }
}
async function cargarMetaExpensasCategoriasExistente() {
  const anio = document.getElementById('mec-anio')?.value;
  const mes  = document.getElementById('mec-mes')?.value;
  if (!anio || !mes) return;
  document.getElementById('mec-vigente').value = await window._fbGetMetaOnce(anio, mes, 'expensas_vigente') || '';
  document.getElementById('mec-mora').value = await window._fbGetMetaOnce(anio, mes, 'expensas_mora') || '';
}
function cerrarModalMetaExpensasCategorias() {
  document.getElementById('modal-meta-expensas-categorias')?.classList.remove('open');
}
async function guardarMetaExpensasCategorias() {
  const anio = document.getElementById('mec-anio')?.value;
  const mes  = document.getElementById('mec-mes')?.value;
  const vigente = parseFloat(document.getElementById('mec-vigente')?.value) || 0;
  const mora = parseFloat(document.getElementById('mec-mora')?.value) || 0;
  if (!anio || !mes) { toastErr('Elegí un período.'); return; }
  await window._fbSetMeta(anio, mes, 'expensas_vigente', vigente);
  await window._fbSetMeta(anio, mes, 'expensas_mora', mora);
  toastOk('Guardado en metas/' + anio + '/' + mes + '/expensas_vigente (' + vigente + ') y .../expensas_mora (' + mora + ')');
  cerrarModalMetaExpensasCategorias();
  // El modal tiene su propio Año/Mes, aparte del filtro de arriba — si guardaste para un
  // período distinto al que estabas mirando, sincronizamos el filtro principal con lo que
  // recién guardaste, así lo ves reflejado ya mismo (antes quedaba "guardado pero invisible").
  expFilAnio = parseInt(anio);
  expFilMes = mes;
  const selAnioGlobal = document.getElementById('fil-exp-anio');
  const selMesGlobal = document.getElementById('fil-exp-mes-global');
  if (selAnioGlobal) selAnioGlobal.value = anio;
  if (selMesGlobal) selMesGlobal.value = mes;
  renderResumenMensualExpensas();
  renderHistoricoMensualExpensas();
  renderSemanalExpensas();
  renderCobranzaExpensasKPIs();
  renderDashboardExpensas();
}
...
function renderOrangeCarteraSyncInfo(info) {
  const lbl = document.getElementById('orange-cart-sync-label');
  if (!lbl) return;
  lbl.textContent = info?.ts ? new Date(info.ts).toLocaleString('es-BO') + ' (' + info.cantidad + ' lotes)' : 'nunca';
}
...
function renderOrangeMoraTabla() {
  ...
  // Esta tabla viene del cronograma de Orange (Estado de Cuota/Pago), que ya sabemos que
  // no siempre refleja pagos reales. Cruzamos contra lo que SÍ categorizamos a mano en
  // "Categorizar cobros" — si hay algún cobro de ese terreno marcado "Vigente" en los
  // últimos 60 días, mostramos un aviso: puede que ya esté al día aunque acá diga mora.
  const hoy = Date.now();
  const terrenosConVigenteReciente = new Set();
  orangeExpensasDetalleRaw.forEach(r => {
    const cat = expensasCategoriasData[r.key]?.categoria;
    if (cat === 'Vigente' && r.fechaOrden && (hoy - r.fechaOrden) < 60*24*60*60*1000) {
      terrenosConVigenteReciente.add(r.terreno);
    }
  });

  tbody.innerHTML = datos.map(r => {
    const posibleDesactualizado = terrenosConVigenteReciente.has(r.terreno);
    return `<tr${posibleDesactualizado?' style="background:var(--ok-bg,#e8f5e9);"':''}>
    <td style="font-weight:600;font-family:monospace;">${esc(r.terreno)}</td>
    <td>${esc(r.cliente)} ${posibleDesactualizado?'<span title="Tiene un cobro categorizado como Vigente en los últimos 60 días — este dato de Orange puede estar desactualizado" style="font-size:var(--fs-xs);font-weight:700;color:var(--ok);">✅ ¿Ya pagó? Verificar</span>':''}</td>
...
async function sincronizarOrangeCartera() {
  const btn = document.getElementById('btn-sync-orange-cartera');
  if (btn) { btn.disabled = true; btn.textContent = '🔄 Sincronizando...'; }
  try {
    const resp = await fetch(ORANGE_API_URL + '?op=GetCartera');
    if (!resp.ok) throw new Error('HTTP '+resp.status);
    const json = await resp.json();
    if (!json.success || !Array.isArray(json.data)) throw new Error('Respuesta inesperada de Orange');

    // Por cada Terreno: Orange trae Terrenos y Acciones como DOS obligaciones separadas,
    // cada una con su propia mora — un cliente puede estar al día en el terreno
    // pero atrasado en la acción del club (o viceversa), así que las trackeamos aparte.
    const datos = {};
    json.data.forEach(r => {
      const t = r.Terreno; if (!t) return;
      (datos[t] ||= {
        cliente: r.NombreCliente||'', asesor: r.Asesor||'', modalidad: r.Modalidad||'', modulo: r.Modulo||'',
        peorTerreno: null, adeudadoTerreno: 0,
        peorAccion: null, adeudadoAccion: 0,
      });
      const d = datos[t];
      if (r.TipoPago === 'TERRENOS' || (r.CobranzaTerreno||0) > 0) {
        d.adeudadoTerreno += (r.CobranzaTerreno||0);
        if (!d.peorTerreno || (r.DiasMoraTerreno||0) > d.peorTerreno) d.peorTerreno = r.DiasMoraTerreno||0;
      }
      if (r.TipoPago === 'ACCIONES' || (r.CobranzaAccion||0) > 0) {
        d.adeudadoAccion += (r.CobranzaAccion||0);
        if (!d.peorAccion || (r.DiasMoraAccion||0) > d.peorAccion) d.peorAccion = r.DiasMoraAccion||0;
      }
    });

    let n = 0;
    for (const [terreno, d] of Object.entries(datos)) {
      const key = terreno.replace(/[^a-zA-Z0-9]/g,'-');
      // Estado general: el peor de los dos (terreno o acción) — así en el KPI general
      // no se "esconde" un atraso de acción detrás de un terreno al día.
      const peorDias = Math.max(d.peorTerreno||0, d.peorAccion||0);
      await window._fbSetOrangeCartera(key, {
        terreno, cliente: d.cliente, asesor: d.asesor, modalidad: d.modalidad, modulo: d.modulo,
        diasMoraTerreno: d.peorTerreno||0, adeudadoTerreno: Math.round(d.adeudadoTerreno*100)/100,
        diasMoraAccion: d.peorAccion||0, adeudadoAccion: Math.round(d.adeudadoAccion*100)/100,
        diasMora: peorDias, estado: estadoPorDiasMora(peorDias),
      });
      n++;
    }

    // Universo general: cada Terreno en GetCartera es un lote con plan de pago activo —
    // es decir, VENDIDO. Contarlos por Modalidad y por Módulo nos da el panorama completo
    // de los 785 lotes vendidos: qué modalidad predomina, qué módulo tiene más movimiento.
    const porModalidad = {}, porModulo = {};
    Object.values(datos).forEach(d => {
      const modKey = d.modalidad || 'Sin modalidad';
      (porModalidad[modKey] ||= {lotes:0, montoAdeudado:0});
      porModalidad[modKey].lotes++;
      porModalidad[modKey].montoAdeudado += (d.adeudadoTerreno||0)+(d.adeudadoAccion||0);

      const modu = d.modulo || 'Sin módulo';
      (porModulo[modu] ||= {lotes:0, montoAdeudado:0});
      porModulo[modu].lotes++;
      porModulo[modu].montoAdeudado += (d.adeudadoTerreno||0)+(d.adeudadoAccion||0);
    });
    await window._fbSetOrangeCarteraAgregados({
      totalLotesVendidos: Object.keys(datos).length,
      porModalidad, porModulo,
    });
    await window._fbSetOrangeCarteraSyncInfo({ ts: Date.now(), cantidad: n });
    toastOk('Sincronizado: ' + n + ' lotes de cartera actualizados desde Orange.');
  } catch(e) {
    toastErr('No se pudo conectar con Orange (' + e.message + ').');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '🔄 Sincronizar con Orange'; }
  }
}

function renderOrangeCarteraPanel() {
  renderOrangeCarteraKPIs();
  renderOrangeCarteraTabla();
}

function renderOrangeCarteraKPIs() {
  const tiers = { 'Mora 1-30':'c1', 'Mora 31-60':'c2', 'Mora 61-90':'c3', 'Mora 91-180':'c4', 'Mora +180':'c5' };
  const acc = {}; Object.values(tiers).forEach(id => acc[id] = {n:0, monto:0});
  orangeCarteraData.forEach(r => {
    const id = tiers[r.estado];
    if (!id) return;
    acc[id].n++; acc[id].monto += ((r.adeudadoTerreno||0) + (r.adeudadoAccion||0));
  });
  Object.entries(tiers).forEach(([label,id]) => {
    setText('orange-cart-kpi-'+id+'-val', '$'+Math.round(acc[id].monto).toLocaleString('es-BO'));
    setText('orange-cart-kpi-'+id+'-n', acc[id].n+' lote(s)');
  });
}

function renderOrangeCarteraTabla() {
  const tbody = document.getElementById('orange-cartera-tbody');
  if (!tbody) return;
  const term = (document.getElementById('orange-cart-buscar')?.value||'').trim().toLowerCase();
  let datos = orangeCarteraData;
  if (term) datos = datos.filter(r =>
    (r.cliente||'').toLowerCase().includes(term) ||
    (r.terreno||'').toLowerCase().includes(term) ||
    (r.asesor||'').toLowerCase().includes(term)
  ...
  if (!datos.length) { tbody.innerHTML = '<tr><td colspan="8"><div class="empty-state">Sin datos — sincronizá con Orange arriba.</div></td></tr>'; return; }
  ...
  tbody.innerHTML = datos.map(r => `<tr>
    ...
    <td style="font-size:var(--fs-sm);">${esc(r.asesor||'—')}</td>
    <td>${_pintarEstadoExp(r.estado)}</td>
    <td style="text-align:center;">${r.diasMoraTerreno||0}</td>
    <td style="font-weight:600;color:var(--cartera);">${r.adeudadoTerreno ? '$'+r.adeudadoTerreno.toLocaleString('es-BO',{minimumFractionDigits:2}) : '—'}</td>
    <td style="text-align:center;">${r.diasMoraAccion||0}</td>
    <td style="font-weight:600;color:var(--info);">${r.adeudadoAccion ? '$'+r.adeudadoAccion.toLocaleString('es-BO',{minimumFractionDigits:2}) : '—'}</td>
  </tr>`).join('');
}

/* ═══════════════════════════════════════════
   UNIVERSO GENERAL — lotes vendidos por Modalidad y por Módulo
   (cada Terreno en GetCartera = un lote con plan de pago = vendido)
═══════════════════════════════════════════ */
function renderOrangeCarteraAgregados() {
  const cont = document.getElementById('orange-cart-universo');
  if (!cont) return;
  if (!orangeCarteraAgregados) {
    cont.innerHTML = '<div class="empty-state">Sincronizá con Orange arriba para ver el universo completo.</div>';
    return;
  }
  const { totalLotesVendidos, porModalidad, porModulo } = orangeCarteraAgregados;

  const bloque = (titulo, obj) => {
    const entries = Object.entries(obj||{}).sort((a,b)=>b[1].lotes-a[1].lotes);
    const max = Math.max(...entries.map(([,v])=>v.lotes), 1);
    return `<div>
      <h4 style="margin:0 0 10px;font-size:var(--fs-base);">${titulo}</h4>
      ${entries.map(([k,v]) => `
        <div style="margin-bottom:8px;">
          <div style="display:flex;justify-content:space-between;font-size:var(--fs-sm);margin-bottom:3px;">
            <span style="font-weight:600;">${esc(k)}</span>
            <span style="color:var(--gris);">${v.lotes} lote${v.lotes!==1?'s':''} · $${Math.round(v.montoAdeudado).toLocaleString('es-BO')} adeudado</span>
          </div>
          <div style="height:8px;background:var(--fill);border-radius:99px;overflow:hidden;">
            <div style="height:100%;width:${(v.lotes/max*100).toFixed(1)}%;background:var(--cartera);"></div>
          </div>
        </div>`).join('')}
    </div>`;
  };
  ...
  cont.innerHTML = `
    <div style="text-align:center;margin-bottom:18px;">
      <div style="font-family:'Space Grotesk',sans-serif;font-size:36px;color:var(--cartera);">${totalLotesVendidos||0}</div>
      <div style="font-size:var(--fs-sm);color:var(--gris);">lotes vendidos con plan de pago activo (Orange)</div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;">
      ${bloque('Por Modalidad', porModalidad)}
      ${bloque('Por Módulo', porModulo)}
    </div>`;
}

/* ═══════════════════════════════════════════
   DETALLE CRUDO DE COBROS — Fecha + Tipo de pago (Terreno/Acción) + Monto
   A propósito NO se usa EstadoCuota ni EstadoPago acá — solo el hecho de
   cobranza en sí: quién pagó, qué pagó, y cuándo.
═══════════════════════════════════════════ */
let orangeCarteraDetalleRaw = [];

// Gestión fiscal: 1° de abril al 31 de marzo del año siguiente.
// Devuelve el AÑO EN QUE EMPIEZA la gestión (ej: abril 2026 a marzo 2027 = gestión "2026").
function _gestionDeFecha(fecha) {
  if (!fecha) return null;
  const anio = fecha.getFullYear(), mes = fecha.getMonth(); // 0=enero, 3=abril
  return mes >= 3 ? anio : anio - 1;
}
function _gestionActual() {
  return _gestionDeFecha(new Date());
}
function _labelGestion(anioInicio) {
  return 'Abr ' + anioInicio + ' – Mar ' + (anioInicio+1);
}

let _terrenoGestionVentaMap = {}; // terreno -> año de inicio de gestión en que se vendió

async function cargarDetalleCobrosCartera(silencioso) {
  const btn1 = document.getElementById('btn-cargar-detalle-cartera');
  const btn2 = document.getElementById('btn-refrescar-cobranza-cartera');
  const setBtns = (txt, disabled) => {
    if (!silencioso) {
      if (btn1) { btn1.textContent = txt; btn1.disabled = disabled; }
      if (btn2) { btn2.textContent = txt; btn2.disabled = disabled; }
    }
  };
  setBtns('🔄 Cargando...', true);
  try {
    const [respCart, respVentas, respReservas] = await Promise.all([
      fetch(ORANGE_API_URL + '?op=GetCartera'),
      fetch(ORANGE_API_URL + '?op=GetVentas'),
      fetch(ORANGE_API_URL + '?op=GetReservas'),
    ]);
    if (!respCart.ok) throw new Error('HTTP '+respCart.status);
    const textoCrudoCart = await respCart.text();
    let json;
    try {
      json = JSON.parse(textoCrudoCart);
    } catch (eParse) {
      console.error('[ORANGE-CARTERA] Respuesta no es JSON. Primeros 300 caracteres:', textoCrudoCart.slice(0,300));
      throw new Error('Orange no devolvió datos válidos — empieza con: "' + textoCrudoCart.slice(0,80) + '..." (mirá la consola para verlo completo, esto parece un error del lado de Orange, no nuestro)');
    }
    if (!json.success || !Array.isArray(json.data)) throw new Error('Respuesta inesperada de Orange');

    // Mapa Terreno → gestión en que se concretó la venta (para clasificar cada cobro después).
    // Si GetVentas falla, seguimos igual — todo quedará como "Cartera" por defecto (no rompe nada).
    _terrenoGestionVentaMap = {};
    try {
      const jsonVentas = await respVentas.json();
      if (jsonVentas.success && Array.isArray(jsonVentas.data)) {
        jsonVentas.data.forEach(r => {
          if (!r.Terreno) return;
          const f = _parseFechaOrange(r.FechaFirma) || _parseFechaOrange(r.Fechacreacion);
          if (f) _terrenoGestionVentaMap[r.Terreno] = _gestionDeFecha(f);
        });
      }
    } catch(e) { console.error('No se pudo cruzar con GetVentas:', e); }

    const gestionActualN = _gestionActual();
    orangeCarteraDetalleRaw = json.data.map(r => {
      const esTerreno = r.TipoPago === 'TERRENOS';
      const monto = esTerreno ? (r.CobranzaTerreno||0) : (r.CobranzaAccion||0);
      const gestionVenta = _terrenoGestionVentaMap[r.Terreno];
      // Ingreso por Venta Nueva: el terreno se vendió DENTRO de la gestión fiscal en curso
      // (1° abril - 31 marzo). Todo lo demás (ventas de gestiones anteriores) es Cartera.
      const tipoIngreso = (gestionVenta !== undefined && gestionVenta === gestionActualN) ? 'Venta Nueva' : 'Cartera';
      const tipoPagoTxt = esTerreno ? 'Terreno' : 'Acción';
      // Clave estable para poder guardar la categoría de este cobro puntual (no hay ID único de Orange).
      const key = `${r.Terreno}_${tipoPagoTxt}_${r.FechaDePago}_${monto}`.replace(/[.#$\[\]\/\s]/g,'-');
      return {
        key, fecha: r.FechaDePago || '', fechaOrden: _parseFechaOrange(r.FechaDePago)?.getTime() || 0,
        cliente: r.NombreCliente||'', terreno: r.Terreno||'', asesor: r.Asesor||'',
        modalidad: r.Modalidad||'Sin modalidad', modulo: r.Modulo||'Sin módulo',
        tipoPago: tipoPagoTxt, monto, tipoIngreso,
      };
    }).filter(r => r.monto > 0); // solo filas con un cobro real asociado

    // Señas de Reservas CONCRETADAS (GetReservas) — la plata del primer pago de una venta
    // nueva, antes de que exista un plan de cuotas en GetCartera. Se suma al mismo total.
    try {
      const jsonReservas = await respReservas.json();
      if (jsonReservas.success && Array.isArray(jsonReservas.data)) {
        const senas = jsonReservas.data.filter(r => r.Estado === 'Concretado' && (r.MontoReserva||0) > 0).map(r => {
          const gestionVenta = _terrenoGestionVentaMap[r.Terreno];
          const tipoIngreso = (gestionVenta !== undefined && gestionVenta === gestionActualN) ? 'Venta Nueva' : 'Cartera';
          const key = `${r.Terreno}_Sena_${r.FechaPago}_${r.MontoReserva}`.replace(/[.#$\[\]\/\s]/g,'-');
          return {
            key, fecha: r.FechaPago||'', fechaOrden: _parseFechaOrange(r.FechaPago)?.getTime() || 0,
            cliente: r.Cliente||'', terreno: r.Terreno||'', asesor: r.AsesorComercial||'',
            modalidad: 'Seña / Reserva', modulo: 'Sin módulo',
            tipoPago: 'Seña', monto: r.MontoReserva||0, tipoIngreso,
          };
        }).filter(r => r.fechaOrden > 0);
        orangeCarteraDetalleRaw = orangeCarteraDetalleRaw.concat(senas);
      }
    } catch(e) { console.error('No se pudo cruzar con GetReservas:', e); }

    poblarFiltrosDetalleCartera();
    renderDetalleCobrosCartera();
    await renderCobranzaCarteraKPIs();
    renderCobranzaCarteraTendencia();
    renderCobranzaCarteraSemanal();
    renderCobranzaPorTerrenoAccion();
    renderCobranzaPorModalidad();
    renderIngresosVentaNuevaVsCartera();
    renderTendenciaVentaNuevaVsCartera();
    renderTendenciaTerrenoAccion();
    renderCategorizacionCobros();
    renderResumenMensualCartera();
    renderSemanalCartera();
    renderDetalleSemanaCartera();
    const lbl = new Date().toLocaleString('es-BO') + (silencioso ? ' (automática)' : ' (manual)');
    setText('orange-cart-cobranza-sync-label', lbl);
    if (!silencioso) toastOk('Cobranza actualizada: ' + orangeCarteraDetalleRaw.length + ' cobros cargados.');
  } catch(e) {
    if (!silencioso) toastErr('No se pudo cargar el detalle: ' + e.message);
  } finally {
    setBtns('🔄 Actualizar ahora', false);
  }
}

/* ── KPIs de Cobranza: semana actual, mes actual (vs. meta), gestión (año) actual ── */
// ── Filtro global del panel de Cartera ──
let carteraFilAnio = new Date().getFullYear();
let carteraFilMes  = PC_MESES[new Date().getMonth()]; // lista fija, mismo motivo que en Expensas
let carteraFilGestion = _gestionActual();

function cambiarFiltroCartera() {
  carteraFilAnio = parseInt(document.getElementById('fil-cart-anio').value);
  carteraFilMes  = document.getElementById('fil-cart-mes-global').value;
  carteraFilGestion = parseInt(document.getElementById('fil-cart-gestion').value);
  renderCobranzaCarteraKPIs();
  renderCobranzaPorTerrenoAccion();
  renderCobranzaPorModalidad();
  renderIngresosVentaNuevaVsCartera();
  renderTendenciaVentaNuevaVsCartera();
...
async function renderCobranzaCarteraKPIs() {
  const hoy = new Date();

  // Poblar selectores la primera vez
  const selAnio = document.getElementById('fil-cart-anio');
  if (selAnio && !selAnio.dataset.filled) {
    const anioHoy = hoy.getFullYear();
    const anios = [...new Set(orangeCarteraDetalleRaw.map(r => r.fechaOrden ? new Date(r.fechaOrden).getFullYear() : null).filter(Boolean))];
    if (!anios.includes(anioHoy)) anios.push(anioHoy);
    anios.sort((a,b)=>b-a);
    selAnio.innerHTML = anios.map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.value = carteraFilAnio;
    selAnio.dataset.filled = '1';
  }
  const selMes = document.getElementById('fil-cart-mes-global');
  if (selMes) selMes.value = carteraFilMes;
  const selGestion = document.getElementById('fil-cart-gestion');
  if (selGestion && !selGestion.dataset.filled) {
    const gestionHoy = _gestionActual();
    if (carteraFilGestion === null) carteraFilGestion = gestionHoy;
    const gestiones = [gestionHoy, gestionHoy-1, gestionHoy-2, gestionHoy-3];
    selGestion.innerHTML = gestiones.map(g=>`<option value="${g}">Gestión ${_labelGestion(g)}</option>`).join('');
    selGestion.value = carteraFilGestion;
    selGestion.dataset.filled = '1';
  }

  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(carteraFilMes);

  // Semana actual: siempre la de HOY (no tiene sentido "filtrar" una semana de un mes pasado)
  const diaSemana = (hoy.getDay()+6)%7; // 0=lunes
  const inicioSemana = new Date(hoy); inicioSemana.setDate(hoy.getDate()-diaSemana); inicioSemana.setHours(0,0,0,0);
  const finSemana = new Date(inicioSemana); finSemana.setDate(inicioSemana.getDate()+7);

  let cobradoSemana = 0, cobradoMes = 0, cobradoGestion = 0;
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (f.getFullYear() === carteraFilAnio && f.getMonth() === mesFilNum) cobradoMes += r.monto;
    if (_gestionDeFecha(f) === carteraFilGestion) cobradoGestion += r.monto;
    if (r.fechaOrden >= inicioSemana.getTime() && r.fechaOrden < finSemana.getTime()) cobradoSemana += r.monto;
  });

  setText('orange-cart-anio-actual', _labelGestion(carteraFilGestion));
  setText('orange-cart-kpi-semana', '$'+Math.round(cobradoSemana).toLocaleString('es-BO'));
  setText('orange-cart-kpi-mes', '$'+Math.round(cobradoMes).toLocaleString('es-BO'));
  setText('orange-cart-kpi-gestion', '$'+Math.round(cobradoGestion).toLocaleString('es-BO'));

  const metaC = await window._fbGetMetaOnce(carteraFilAnio, carteraFilMes, 'cartera');
  const elMeta = document.getElementById('orange-cart-kpi-mes-meta');
  if (elMeta) {
    elMeta.innerHTML = metaC
      ? Math.round(cobradoMes/metaC*100) + '% de la meta ($'+metaC.toLocaleString('es-BO')+')'
      : '<a href="#" onclick="abrirModalMetaCartera();return false;" style="color:var(--info);font-weight:600;">Sin meta — cargarla acá →</a>';
  }
}

/* ── Tendencia mensual: últimos 12 meses de cobranza real (Orange) ── */
let _chartCobranzaCarteraTendencia = null;
function renderCobranzaCarteraTendencia() {
  const canvas = document.getElementById('orange-cart-chart-tendencia');
  if (!canvas) return;
  const nombresMes = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const hoy = new Date();
  const buckets = [];
  ...
  if (_chartCobranzaCarteraTendencia) _chartCobranzaCarteraTendencia.destroy();
  _chartCobranzaCarteraTendencia = new Chart(canvas, {
    type: 'line',
    data: {
      labels: buckets.map(b=>b.label),
      datasets: [{ label:'Cobrado', data: buckets.map(b=>Math.round(b.total)), borderColor:'#2451FF', backgroundColor:'#2451FF', tension:.3, pointRadius:4, pointHoverRadius:6 }]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      layout:{ padding:{ top:22 } },
      plugins:{ legend:{display:false}, tooltip:{callbacks:{label:ctx=>_fmtMonto(ctx.parsed.y)}},
        datalabels:{ display:true, align:'top', anchor:'end', offset:6, color:'#0A0A0A', font:{weight:'700',size:10.5}, formatter:v=>_fmtMonto(v) } },
      scales:{ x:{grid:{display:false}}, y:{beginAtZero:true, ticks:{callback:v=>'$'+(v/1000)+'K'}} }
    }
  });
}

/* ── Tendencia semanal: últimas 10 semanas de cobranza real (Orange) ── */
let _chartCobranzaCarteraSemanal = null;
function renderCobranzaCarteraSemanal() {
  const canvas = document.getElementById('orange-cart-chart-semanal');
  if (!canvas) return;
  const hoy = new Date();
  const diaSemanaHoy = (hoy.getDay()+6)%7;
  const inicioSemanaActual = new Date(hoy); inicioSemanaActual.setDate(hoy.getDate()-diaSemanaHoy); inicioSemanaActual.setHours(0,0,0,0);
  ...
  if (_chartCobranzaCarteraSemanal) _chartCobranzaCarteraSemanal.destroy();
  _chartCobranzaCarteraSemanal = new Chart(canvas, {
    type: 'bar',
    data: {
      labels: buckets.map(b=>b.label),
      datasets: [{ label:'Cobrado', data: buckets.map(b=>Math.round(b.total)), backgroundColor:'#7C3AED', borderRadius:5, maxBarThickness:28 }]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{display:false}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>_fmtMonto(ctx.parsed.y)}} },
      scales:{ x:{grid:{display:false}}, y:{beginAtZero:true, ticks:{callback:v=>'$'+(v/1000)+'K'}} }
    }
  });
}

/* ── Cobrado en Terreno vs Cobrado en Acción — según el período elegido arriba ── */
/* ── Tendencia mensual de Venta Nueva vs Cartera, los 12 meses de la gestión elegida ── */
let _chartTendenciaVentaNueva = null;
function renderTendenciaVentaNuevaVsCartera() {
  const canvas = document.getElementById('orange-cart-chart-venta-nueva-tendencia');
  if (!canvas) return;
  const nombresMesCorto = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const gestion = carteraFilGestion;
  const buckets = [];
  for (let i = 3; i < 15; i++) {
    const mesIdx = i % 12, anio = i < 12 ? gestion : gestion + 1;
    buckets.push({ mesIdx, anio, label: nombresMesCorto[mesIdx]+' '+String(anio).slice(2), ventaNueva:0, cartera:0 });
  }
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    const b = buckets.find(x => x.mesIdx===f.getMonth() && x.anio===f.getFullYear());
    if (!b) return;
    const gestionVenta = _terrenoGestionVentaMap[r.terreno];
    if (gestionVenta !== undefined && gestionVenta === gestion) b.ventaNueva += r.monto; else b.cartera += r.monto;
  });

  if (_chartTendenciaVentaNueva) _chartTendenciaVentaNueva.destroy();
  _chartTendenciaVentaNueva = new Chart(canvas, {
    type: 'bar',
    data: {
      labels: buckets.map(b=>b.label),
      datasets: [
        { label:'Venta Nueva', data: buckets.map(b=>Math.round(b.ventaNueva)), backgroundColor:'#1E8E3E', borderRadius:4, maxBarThickness:26, stack:'s' },
        { label:'Cartera', data: buckets.map(b=>Math.round(b.cartera)), backgroundColor:'#2451FF', borderRadius:4, maxBarThickness:26, stack:'s' },
      ]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{position:'top', labels:{boxWidth:10,font:{size:10.5}}}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>ctx.dataset.label+': '+_fmtMonto(ctx.parsed.y)}} },
      scales:{ x:{stacked:true, grid:{display:false}, ticks:{font:{size:9.5}}}, y:{stacked:true, beginAtZero:true, ticks:{callback:v=>_fmtMonto(v)}} }
    }
  });
}

/* ── Tendencia mensual de Terreno vs Acción, los 12 meses de la gestión elegida ── */
let _chartTendenciaTerrenoAccion = null;
function renderTendenciaTerrenoAccion() {
  const canvas = document.getElementById('orange-cart-chart-terreno-accion-tendencia');
  if (!canvas) return;
  const nombresMesCorto = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const gestion = carteraFilGestion;
  const buckets = [];
  for (let i = 3; i < 15; i++) {
    const mesIdx = i % 12, anio = i < 12 ? gestion : gestion + 1;
    buckets.push({ mesIdx, anio, label: nombresMesCorto[mesIdx]+' '+String(anio).slice(2), terreno:0, accion:0 });
  }
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    const b = buckets.find(x => x.mesIdx===f.getMonth() && x.anio===f.getFullYear());
    if (!b) return;
    if (r.tipoPago==='Terreno') b.terreno += r.monto; else b.accion += r.monto;
  });

  if (_chartTendenciaTerrenoAccion) _chartTendenciaTerrenoAccion.destroy();
  _chartTendenciaTerrenoAccion = new Chart(canvas, {
    type: 'line',
    data: {
      labels: buckets.map(b=>b.label),
      datasets: [
        { label:'Terreno', data: buckets.map(b=>Math.round(b.terreno)), borderColor:'#1E8E3E', backgroundColor:'#1E8E3E', tension:.3, pointRadius:3 },
        { label:'Acción', data: buckets.map(b=>Math.round(b.accion)), borderColor:'#2451FF', backgroundColor:'#2451FF', tension:.3, pointRadius:3 },
      ]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{position:'top', labels:{boxWidth:10,font:{size:10.5}}}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>ctx.dataset.label+': '+_fmtMonto(ctx.parsed.y)}} },
      scales:{ x:{grid:{display:false}, ticks:{font:{size:9.5}}}, y:{beginAtZero:true, ticks:{callback:v=>_fmtMonto(v)}} }
    }
  });
}

let _chartCarteraTerrenoAccion = null;
function renderCobranzaPorTerrenoAccion() {
  const cont = document.getElementById('orange-cart-terreno-accion');
  if (!cont) return;
  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(carteraFilMes);
  let terreno = 0, accion = 0, terrenoGestion = 0, accionGestion = 0, nCuotasTerreno = 0, nCuotasAccion = 0;
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    if (r.tipoPago !== 'Terreno' && r.tipoPago !== 'Acción') return; // las Señas no entran acá, tienen su propio origen
    const f = new Date(r.fechaOrden);
    if (f.getFullYear()===carteraFilAnio && f.getMonth()===mesFilNum) {
      if (r.tipoPago==='Terreno') { terreno += r.monto; nCuotasTerreno++; } else { accion += r.monto; nCuotasAccion++; }
    }
    if (_gestionDeFecha(f) === carteraFilGestion) {
      if (r.tipoPago==='Terreno') terrenoGestion += r.monto; else accionGestion += r.monto;
    }
  });
  setText('orange-cart-kpi-terreno-mes', _fmtMonto(terreno));
  setText('orange-cart-kpi-accion-mes', _fmtMonto(accion));
  setText('orange-cart-kpi-terreno-gestion', _fmtMonto(terrenoGestion) + ' en la gestión');
  setText('orange-cart-kpi-accion-gestion', _fmtMonto(accionGestion) + ' en la gestión');
  setText('orange-cart-kpi-terreno-cuotas', nCuotasTerreno + ' cuota' + (nCuotasTerreno!==1?'s':'') + ' cobradas');
  setText('orange-cart-kpi-accion-cuotas', nCuotasAccion + ' cuota' + (nCuotasAccion!==1?'s':'') + ' cobradas');

  const canvas = document.getElementById('orange-cart-chart-terreno-accion');
  if (canvas) {
    if (_chartCarteraTerrenoAccion) _chartCarteraTerrenoAccion.destroy();
    _chartCarteraTerrenoAccion = new Chart(canvas, {
      type: 'doughnut',
      data: { labels: ['Terreno','Acción'], datasets: [{ data: [terreno||0, accion||0], backgroundColor: ['#1E8E3E','#2451FF'], borderColor:'#FFFDF7', borderWidth:2 }] },
      options: { responsive:true, maintainAspectRatio:false, cutout:'65%', plugins:{ legend:{display:false}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>ctx.label+': '+_fmtMonto(ctx.parsed)}} } }
    });
  }
}

/* ── Cobrado por Modalidad — según el período elegido arriba ── */
function renderCobranzaPorModalidad() {
  const cont = document.getElementById('orange-cart-por-modalidad');
  if (!cont) return;
  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(carteraFilMes);
  const porModalidad = {};
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (f.getFullYear()!==carteraFilAnio || f.getMonth()!==mesFilNum) return;
    porModalidad[r.modalidad] = (porModalidad[r.modalidad]||0) + r.monto;
  });
  const entries = Object.entries(porModalidad).sort((a,b)=>b[1]-a[1]);
  if (!entries.length) { cont.innerHTML = '<div class="empty-state">Sin cobros en ' + carteraFilMes + ' ' + carteraFilAnio + '.</div>'; return; }
  const max = Math.max(...entries.map(([,v])=>v), 1);
  cont.innerHTML = entries.map(([k,v]) => `
    <div style="margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;font-size:var(--fs-sm);margin-bottom:3px;">
        <span style="font-weight:600;">${esc(k)}</span>
        <span style="color:var(--cartera);font-weight:700;">${_fmtMonto(v)}</span>
      </div>
      <div style="height:9px;background:var(--fill);border-radius:99px;overflow:hidden;">
        <div style="height:100%;width:${(v/max*100).toFixed(1)}%;background:var(--cartera);"></div>
      </div>
...
/* ── Ingresos por Venta Nueva vs Cartera — gestión fiscal 1° abril a 31 marzo ── */
let _chartCarteraVentaNueva = null;
function renderIngresosVentaNuevaVsCartera() {
  const cont = document.getElementById('orange-cart-venta-nueva-vs-cartera');
  if (!cont) return;
  const gestionFiltrada = carteraFilGestion;
  let ventaNueva = 0, cartera = 0;
  const terrenosVentaNueva = new Set(), terrenosCartera = new Set();
  orangeCarteraDetalleRaw.forEach(r => {
    if (!r.fechaOrden) return;
    const f = new Date(r.fechaOrden);
    if (_gestionDeFecha(f) !== gestionFiltrada) return; // solo cobros DENTRO de la gestión elegida
    // Recalculado contra la gestión ELEGIDA (no la de hoy) — para poder mirar gestiones pasadas también.
    const gestionVenta = _terrenoGestionVentaMap[r.terreno];
    const esVentaNueva = (gestionVenta !== undefined && gestionVenta === gestionFiltrada);
    if (esVentaNueva) { ventaNueva += r.monto; terrenosVentaNueva.add(r.terreno); }
    else { cartera += r.monto; terrenosCartera.add(r.terreno); }
  });
  const total = ventaNueva + cartera;
  const pctNueva = total ? Math.round(ventaNueva/total*100) : 0;

  setText('orange-cart-gestion-label', _labelGestion(gestionFiltrada));
  setText('orange-cart-kpi-venta-nueva', _fmtMonto(ventaNueva));
  setText('orange-cart-kpi-cartera-vieja', _fmtMonto(cartera));
  setText('orange-cart-kpi-venta-nueva-clientes', terrenosVentaNueva.size + ' lote' + (terrenosVentaNueva.size!==1?'s':''));
  setText('orange-cart-kpi-cartera-clientes', terrenosCartera.size + ' lote' + (terrenosCartera.size!==1?'s':''));

  const canvas = document.getElementById('orange-cart-chart-venta-nueva');
  if (canvas) {
    if (_chartCarteraVentaNueva) _chartCarteraVentaNueva.destroy();
    _chartCarteraVentaNueva = new Chart(canvas, {
      type: 'doughnut',
      data: { labels: ['Venta Nueva','Cartera'], datasets: [{ data: [ventaNueva||0, cartera||0], backgroundColor: ['#1E8E3E','#2451FF'], borderColor:'#FFFDF7', borderWidth:2 }] },
      options: { responsive:true, maintainAspectRatio:false, cutout:'65%', plugins:{ legend:{display:false}, datalabels:{display:false}, tooltip:{callbacks:{label:ctx=>ctx.label+': '+_fmtMonto(ctx.parsed)}} } }
    });
  }
}
...
/* ═══════════════════════════════════════════
   CATEGORIZACIÓN MANUAL DE COBROS — usuarios del módulo Cartera (rol 'cobranza')
   Cada cobro que trae Orange se etiqueta a mano en una de estas 9 categorías,
   para poder armar el mismo reporte que ya se le manda a gerencia.
═══════════════════════════════════════════ */
const CARTERA_CATEGORIAS = [
  'Vigente Terreno', 'Mora Terreno', 'Adelanto Terreno',
  'Vigente Acción', 'Mora Acción',
  'Otros Ingresos', 'Venta Nueva', 'Desembolso Bancario', 'Pronto Pago',
];

// Semana ISO (número de semana del año) — para que coincida con el "Semana 32/33/34/35" del reporte a gerencia.
// Formato simplificado para no saturar la vista: $150K en vez de $150.000, $1.2M en vez de $1.200.000.
function _fmtMonto(v) {
  v = Math.round(v||0);
  if (Math.abs(v) >= 1000000) return '$'+(v/1000000).toLocaleString('es-BO',{maximumFractionDigits:1})+'M';
  if (Math.abs(v) >= 1000) return '$'+(v/1000).toLocaleString('es-BO',{maximumFractionDigits:1})+'K';
  return '$'+v.toLocaleString('es-BO');
}
// Igual que _fmtMonto pero en Bolivianos — Expensas se maneja en Bs, no en dólares.
function _fmtMontoBs(v) {
  v = Math.round(v||0);
  if (Math.abs(v) >= 1000000) return 'Bs'+(v/1000000).toLocaleString('es-BO',{maximumFractionDigits:1})+'M';
  if (Math.abs(v) >= 1000) return 'Bs'+(v/1000).toLocaleString('es-BO',{maximumFractionDigits:1})+'K';
  return 'Bs'+v.toLocaleString('es-BO');
}

function _semanaISO(date) {
...
async function asignarCategoriaCobro(cobroKey, categoria) {
  if (!categoria) return;
  await window._fbSetCategoriaCobro(cobroKey, { categoria, asignadoPorNombre: asesorActual?.nombre || 'Admin' });
  toastOk('Categorizado como "' + categoria + '".');
}

function renderCategorizacionCobros() {
  const tbody = document.getElementById('cartera-categ-tbody');
  const contador = document.getElementById('cartera-categ-pendientes');
  if (!tbody) return;
  let pendientes = orangeCarteraDetalleRaw.filter(r => !carteraCategoriasData[r.key]);
  if (contador) contador.textContent = pendientes.length + ' pendiente' + (pendientes.length!==1?'s':'') + ' por categorizar';

  const filTipo = document.getElementById('cartera-categ-fil-tipo')?.value || '';
  if (filTipo) pendientes = pendientes.filter(r => r.tipoPago === filTipo);
  // Más recientes primero — antes las Señas quedaban siempre al fondo del array (se agregan
  // después de Terreno/Acción) y con 11.000+ pendientes nunca llegaban a mostrarse en las 200 primeras.
  pendientes = [...pendientes].sort((a,b) => (b.fechaOrden||0) - (a.fechaOrden||0));

  if (!pendientes.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">✅ Nada pendiente' + (filTipo ? ' de tipo "'+filTipo+'"' : '') + '.</div></td></tr>';
    return;
  }
  const opciones = '<option value="">Elegir categoría...</option>' + CARTERA_CATEGORIAS.map(c=>`<option value="${c}">${c}</option>`).join('');
  tbody.innerHTML = pendientes.slice(0,200).map(r => `<tr>
    <td style="font-size:var(--fs-sm);">${esc(r.fecha)}</td>
    <td>${esc(r.cliente)}</td>
    <td style="font-family:monospace;font-size:var(--fs-sm);">${esc(r.terreno)}</td>
    ...
    <td>
      <select onchange="asignarCategoriaCobro('${r.key}', this.value)" style="padding:6px 10px;border:1.5px solid var(--line);border-radius:var(--r-sm);font-size:var(--fs-sm);font-family:'DM Sans',sans-serif;">
        ${opciones}
      </select>
    </td>
  </tr>`).join('') + (pendientes.length>200 ? `<tr><td colspan="6" style="text-align:center;color:var(--gris);padding:10px;">Mostrando 200 de ${pendientes.length} — categorizá estos y se van cargando los siguientes, o filtrá por tipo arriba.</td></tr>` : '');
}

/* ── Resumen mensual (igual estructura que el reporte a gerencia) ── */
function _sumaCategoria(cobros, categorias) {
  return cobros.filter(r => categorias.includes(carteraCategoriasData[r.key]?.categoria)).reduce((s,r)=>s+r.monto,0);
}

async function renderResumenMensualCartera() {
  const cont = document.getElementById('cartera-resumen-mensual');
  if (!cont) return;

  const mesesNombre = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  const mesFilNum = mesesNombre.indexOf(carteraFilMes);
  const delMes = orangeCarteraDetalleRaw.filter(r => {
    if (!r.fechaOrden) return false;
    const f = new Date(r.fechaOrden);
    return f.getFullYear()===carteraFilAnio && f.getMonth()===mesFilNum;
  });

  const metaVigente = await window._fbGetMetaOnce(carteraFilAnio, carteraFilMes, 'cartera_vigente') || 0;
  const metaMora    = await window._fbGetMetaOnce(carteraFilAnio, carteraFilMes, 'cartera_mora') || 0;
  const metaVM = metaVigente + metaMora;

  const cVigente = _sumaCategoria(delMes, ['Vigente Terreno','Vigente Acción']);
  const cMora    = _sumaCategoria(delMes, ['Mora Terreno','Mora Acción']);
  const cVigenteMora = cVigente + cMora;
  const cAdelanto = _sumaCategoria(delMes, ['Adelanto Terreno']);
  const cOtros    = _sumaCategoria(delMes, ['Otros Ingresos']);
  const cVentaNueva = _sumaCategoria(delMes, ['Venta Nueva']);
  const subAdelantoEtc = cAdelanto + cOtros + cVentaNueva;
  const cDesembolso = _sumaCategoria(delMes, ['Desembolso Bancario']);
  const subGrande = cVigenteMora + subAdelantoEtc + cDesembolso;
  const cProntoPago = _sumaCategoria(delMes, ['Pronto Pago']);
  const totalCobranza = subGrande + cProntoPago;
  const pctMeta = metaVM ? Math.round(cVigenteMora/metaVM*100) : null;

  const fila = (label, valor, destacado, esSub) => `
    <tr style="${destacado?'background:var(--ok-bg,#e8f5e9);font-weight:700;':''}">
      <td style="padding:8px 12px;${esSub?'padding-left:24px;':''}">${label}</td>
      <td style="padding:8px 12px;text-align:right;">${valor!==null ? '$'+Math.round(valor).toLocaleString('es-BO') : '—'}</td>
    </tr>`;

  cont.innerHTML = `
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:var(--ink-900);color:#fff;"><th style="padding:8px 12px;text-align:left;">Meta Cartera ${carteraFilMes} ${carteraFilAnio}</th><th style="padding:8px 12px;text-align:right;">$us</th></tr></thead>
      <tbody>
        ${fila('Meta Cobranza Vigente', metaVigente)}
        ${fila('Meta Cobranza Mora', metaMora)}
        ${fila('Meta Cobranza Vigente y Mora', metaVM, true)}
        ${fila('Cobranza Vigente', cVigente, false, true)}
        ${fila('Cobranza Mora', cMora, false, true)}
        ${fila('Cobranza Vigente y Mora' + (pctMeta!==null ? ' — '+pctMeta+'% de la meta' : ''), cVigenteMora, true)}
        ${fila('Cobranza Adelantado', cAdelanto, false, true)}
        ${fila('Otros Ingresos', cOtros, false, true)}
        ${fila('Venta Nueva', cVentaNueva, false, true)}
        ${fila('Cobranza Adelantado, Ventas Nuevas, Otros Ingresos', subAdelantoEtc, true)}
        ${fila('Desembolso Bancario', cDesembolso)}
        ${fila('Cobranza Vigente + Mora + Adelantado + Venta Nueva + Desembolso', subGrande, true)}
        ${fila('Cobranza Pronto Pago', cProntoPago)}
        ${fila('TOTAL COBRANZA', totalCobranza, true)}
      </tbody>
    </table>
    <div style="margin-top:12px;">
      <button onclick="abrirModalMetaCarteraCategorias()" class="btn-secondary btn-sm">⚙ Cargar meta Vigente/Mora de ${carteraFilMes}</button>
    </div>
    <p style="font-size:10px;color:#999;margin-top:10px;font-family:monospace;">🔍 Diagnóstico — buscando en: metas/${carteraFilAnio}/${carteraFilMes}/cartera_vigente (leyó: ${metaVigente}) y metas/${carteraFilAnio}/${carteraFilMes}/cartera_mora (leyó: ${metaMora})</p>`;
}

/* ── Semanal, con meta (igual al patrón que usamos en Expensas) ── */
let _chartSemanalCartera = null;
function renderSemanalCartera() {
  const tbody = document.getElementById('cartera-semanal-tbody');
  const canvas = document.getElementById('cartera-chart-semanal-meta');
  if (!tbody) return;
  ...
  delMes.forEach(r => {
    const { anio, semana } = _semanaISO(new Date(r.fechaOrden));
    const key = anio+'-'+semana;
    (porSemana[key] ||= { anio, semana, total:0, meta:0, cats:{} });
    porSemana[key].total += r.monto;
    const cat = carteraCategoriasData[r.key]?.categoria;
    const cuentaParaMeta = cat === 'Vigente Terreno' || cat === 'Mora Terreno' || cat === 'Vigente Acción' || cat === 'Mora Acción';
    if (cuentaParaMeta) porSemana[key].meta += r.monto;
  });

  const semanas = Object.values(porSemana).sort((a,b)=> a.semana-b.semana);
  if (!semanas.length) {
    tbody.innerHTML = '<tr><td colspan="5"><div class="empty-state">Sin cobros en ' + carteraFilMes + ' ' + carteraFilAnio + '.</div></td></tr>';
    return;
  }

  tbody.innerHTML = semanas.map(s => {
    const noConsiderada = s.total - s.meta;
    return `<tr>
      <td style="font-weight:600;">Semana ${s.semana}</td>
      <td style="font-weight:600;color:var(--cartera);">$${Math.round(s.total).toLocaleString('es-BO')}</td>
      <td>$${Math.round(s.meta).toLocaleString('es-BO')}</td>
      <td style="color:var(--ok);">$${Math.round(s.meta).toLocaleString('es-BO')}</td>
      <td style="color:var(--gris);">$${Math.round(noConsiderada).toLocaleString('es-BO')}</td>
    </tr>`;
  }).join('') + `<tr style="font-weight:700;background:var(--bg-soft);">
    <td>Total</td>
    <td>$${Math.round(semanas.reduce((s,x)=>s+x.total,0)).toLocaleString('es-BO')}</td>
    <td colspan="2"></td>
    <td>$${Math.round(semanas.reduce((s,x)=>s+(x.total-x.meta),0)).toLocaleString('es-BO')}</td>
  </tr>`;

  if (canvas) {
    if (_chartSemanalCartera) _chartSemanalCartera.destroy();
    _chartSemanalCartera = new Chart(canvas, {
      type: 'line',
      data: {
        labels: semanas.map(s=>'Sem. '+s.semana),
        datasets: [
          { label:'Cobranza total', data: semanas.map(s=>Math.round(s.total)), borderColor:'#E8590C', backgroundColor:'#E8590C', tension:.3 },
          { label:'Cobranza para meta', data: semanas.map(s=>Math.round(s.meta)), borderColor:'#2451FF', backgroundColor:'#2451FF', tension:.3 },
        ]
      },
      options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{position:'top'}, datalabels:{display:false} }, scales:{ y:{beginAtZero:true, ticks:{callback:v=>'$'+(v/1000)+'K'}} } }
    });
  }
}

/* ── Detalle por semana, agrupado por categoría (igual al reporte a gerencia) ── */
function renderDetalleSemanaCartera() {
  const tbody = document.getElementById('cartera-detalle-semana-tbody');
  if (!tbody) return;

  // Poblar selectores la primera vez, con la semana actual por defecto
  const selAnio = document.getElementById('fil-cartera-semana-anio');
  const selSemana = document.getElementById('fil-cartera-semana-num');
  const hoyIso = _semanaISO(new Date());
  if (selAnio && !selAnio.dataset.filled) {
    const anios = [hoyIso.anio, hoyIso.anio-1, hoyIso.anio-2];
    selAnio.innerHTML = anios.map(a=>`<option value="${a}">${a}</option>`).join('');
    selAnio.dataset.filled = '1';
  }
  if (selSemana && !selSemana.dataset.filled) {
    let opts = '';
    for (let i=1;i<=53;i++) opts += `<option value="${i}">Semana ${i}</option>`;
    selSemana.innerHTML = opts;
    selSemana.value = hoyIso.semana;
    selSemana.dataset.filled = '1';
  }

  const anio = parseInt(document.getElementById('fil-cartera-semana-anio')?.value) || new Date().getFullYear();
  const semana = parseInt(document.getElementById('fil-cartera-semana-num')?.value) || 1;

  const cobros = orangeCarteraDetalleRaw.filter(r => {
    if (!r.fechaOrden) return false;
    const iso = _semanaISO(new Date(r.fechaOrden));
    return iso.anio === anio && iso.semana === semana;
  });
  ...
  if (!cobros.length) {
    tbody.innerHTML = '<tr><td colspan="3"><div class="empty-state">Sin cobros en la Semana ' + semana + '/' + String(anio).slice(2) + '.</div></td></tr>';
    return;
  }

  const porCategoria = {};
  cobros.forEach(r => {
    const cat = carteraCategoriasData[r.key]?.categoria || 'Sin categorizar';
    (porCategoria[cat] ||= []).push(r);
  });
  const ordenCat = [...CARTERA_CATEGORIAS, 'Sin categorizar'];
  const catsOrdenadas = Object.keys(porCategoria).sort((a,b)=>ordenCat.indexOf(a)-ordenCat.indexOf(b));
  ...
  catsOrdenadas.forEach(cat => {
    ...
    const subtotal = items.reduce((s,r)=>s+r.monto,0);
    totalGeneral += subtotal;
    html += `<tr style="background:var(--bg-soft);"><td style="font-weight:700;" colspan="2">${esc(cat)}</td><td></td></tr>`;
    items.forEach(r => {
      html += `<tr><td></td><td>${esc(r.cliente)}</td><td style="text-align:right;">$${r.monto.toLocaleString('es-BO')}</td></tr>`;
    });
    html += `<tr style="background:var(--ok-bg,#e8f5e9);font-weight:600;"><td colspan="2">Sub Total</td><td style="text-align:right;">$${Math.round(subtotal).toLocaleString('es-BO')}</td></tr>`;
  });
  html += `<tr style="background:var(--ink-900);color:#fff;font-weight:700;"><td colspan="2">Total general</td><td style="text-align:right;">$${Math.round(totalGeneral).toLocaleString('es-BO')}</td></tr>`;
  tbody.innerHTML = html;
}

function abrirModalMetaCarteraCategorias() {
  const selAnio = document.getElementById('mcc-anio');
  if (!selAnio.options.length) {
    const anioHoy = new Date().getFullYear();
    selAnio.innerHTML = [anioHoy+1, anioHoy, anioHoy-1, anioHoy-2].map(a=>`<option value="${a}">${a}</option>`).join('');
  }
  selAnio.value = carteraFilAnio;
  document.getElementById('mcc-mes').value = carteraFilMes;
  cargarMetaCarteraCategoriasExistente();
  document.getElementById('modal-meta-cartera-categorias').classList.add('open');
}
async function cargarMetaCarteraCategoriasExistente() {
  const anio = document.getElementById('mcc-anio')?.value;
  const mes  = document.getElementById('mcc-mes')?.value;
  if (!anio || !mes) return;
  document.getElementById('mcc-vigente').value = await window._fbGetMetaOnce(anio, mes, 'cartera_vigente') || '';
  document.getElementById('mcc-mora').value = await window._fbGetMetaOnce(anio, mes, 'cartera_mora') || '';
}
function cerrarModalMetaCarteraCategorias() {
  document.getElementById('modal-meta-cartera-categorias')?.classList.remove('open');
}
async function guardarMetaCarteraCategorias() {
  const anio = document.getElementById('mcc-anio')?.value;
  const mes  = document.getElementById('mcc-mes')?.value;
  const vigente = parseFloat(document.getElementById('mcc-vigente')?.value) || 0;
  const mora = parseFloat(document.getElementById('mcc-mora')?.value) || 0;
  if (!anio || !mes) { toastErr('Elegí un período.'); return; }
  await window._fbSetMeta(anio, mes, 'cartera_vigente', vigente);
  await window._fbSetMeta(anio, mes, 'cartera_mora', mora);
  toastOk('Guardado en metas/' + anio + '/' + mes + '/cartera_vigente (' + vigente + ') y .../cartera_mora (' + mora + ')');
  cerrarModalMetaCarteraCategorias();
  carteraFilAnio = parseInt(anio);
  carteraFilMes = mes;
  const selAnioGlobal = document.getElementById('fil-cart-anio');
  const selMesGlobal = document.getElementById('fil-cart-mes-global');
  if (selAnioGlobal) selAnioGlobal.value = anio;
  if (selMesGlobal) selMesGlobal.value = mes;
  renderResumenMensualCartera();
  renderSemanalCartera();
...
function poblarFiltrosDetalleCartera() {
  const selMod = document.getElementById('fil-cart-modalidad');
  const selModu = document.getElementById('fil-cart-modulo');
  if (selMod) {
    const vals = [...new Set(orangeCarteraDetalleRaw.map(r=>r.modalidad))].sort();
    selMod.innerHTML = '<option value="">Todas las modalidades</option>' + vals.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
  }
  if (selModu) {
    const vals = [...new Set(orangeCarteraDetalleRaw.map(r=>r.modulo))].sort();
    selModu.innerHTML = '<option value="">Todos los módulos</option>' + vals.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
  }
}

function renderDetalleCobrosCartera() {
  const tbody = document.getElementById('orange-cart-detalle-tbody');
  if (!tbody) return;
  const term = (document.getElementById('orange-cart-detalle-buscar')?.value||'').trim().toLowerCase();
  const filTipo = document.getElementById('fil-cart-tipo')?.value || '';
  const filMod  = document.getElementById('fil-cart-modalidad')?.value || '';
  const filModu = document.getElementById('fil-cart-modulo')?.value || '';

  let datos = orangeCarteraDetalleRaw;
  if (term) datos = datos.filter(r => (r.cliente||'').toLowerCase().includes(term) || (r.terreno||'').toLowerCase().includes(term));
  if (filTipo) datos = datos.filter(r => r.tipoPago === filTipo);
  if (filMod)  datos = datos.filter(r => r.modalidad === filMod);
  if (filModu) datos = datos.filter(r => r.modulo === filModu);
  datos = [...datos].sort((a,b) => b.fechaOrden - a.fechaOrden);

  const total = datos.reduce((s,r)=>s+r.monto,0);
  setText('orange-cart-detalle-total', '$'+Math.round(total).toLocaleString('es-BO') + ' (' + datos.length + ' cobros)');

  if (!datos.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Sin datos — tocá "Cargar detalle de cobros" arriba.</div></td></tr>';
    return;
  }
  tbody.innerHTML = datos.slice(0,500).map(r => `<tr>
    <td style="font-size:var(--fs-sm);">${esc(r.fecha)}</td>
    <td>${esc(r.cliente)}</td>
    <td style="font-family:monospace;font-size:var(--fs-sm);">${esc(r.terreno)}</td>
    <td><span style="padding:2px 9px;border-radius:999px;font-size:var(--fs-xs);font-weight:600;${r.tipoPago==='Terreno'?'background:var(--ok-bg);color:var(--ok-ink);':r.tipoPago==='Acción'?'background:var(--info-bg,#eef4fb);color:var(--info);':'background:var(--warn-bg);color:var(--warn-ink);'}">${r.tipoPago}</span></td>
    <td style="font-size:var(--fs-sm);">${esc(r.modalidad)}</td>
    <td style="font-weight:600;color:var(--cartera);">$${r.monto.toLocaleString('es-BO',{minimumFractionDigits:2})}</td>
  </tr>`).join('') + (datos.length>500 ? `<tr><td colspan="6" style="text-align:center;color:var(--gris);font-size:var(--fs-sm);padding:10px;">Mostrando los primeros 500 de ${datos.length} — usá los filtros para acotar.</td></tr>` : '');
}
