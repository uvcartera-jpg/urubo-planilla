...
function renderDocumentosGuardados() {
  ...
  tbody.innerHTML = datos.map(d => `<tr>
    ...
    <td>
      <button class="btn-clear" style="padding:5px 10px;font-size:var(--fs-sm);" onclick="abrirDocumentoGuardado('${d._key}','mr')">📄 Materia</button>
      <button class="btn-clear" style="padding:5px 10px;font-size:var(--fs-sm);" onclick="abrirDocumentoGuardado('${d._key}','sc')">📄 Contrato</button>
    </td>
  </tr>`).join('');
}

function abrirDocumentoGuardado(key, cual) {
  ...
  // Rellenamos los campos ocultos del formulario "Nuevo" con lo guardado, para que
  // los generadores (que leen del formulario) usen estos datos exactos.
  ...
  if (cual === 'mr') generarMateriaReservada(); else generarSolicitudContrato();
}
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${titulo}</title><meta charset="utf-8">
    <style>
      ...
      *{ box-sizing:border-box; }
      body{ font-family:Arial,Helvetica,sans-serif; font-size:11px; line-height:1.35; color:#111; padding:14px; max-width:700px; margin:0 auto; }
      h1{ text-align:center; font-size:15px; text-decoration:underline; margin:0 0 12px; font-weight:700; }
      table{ width:100%; border-collapse:collapse; margin:4px 0; }
      td,th{ border:1px solid #333; padding:3px 7px; font-size:10.5px; }
      .header-band{ background:#1e3a5f; color:#fff; font-weight:700; padding:3px 7px; font-size:10.5px; }
      .no-border td{ border:none; padding:1.5px 0; }
      .right{ text-align:right; } .center{ text-align:center; }
      .sign-box{ height:52px; border:1px solid #333; }
...
function generarMateriaReservada() {
  ...
  const esModalidadActual = m => (r.formaPago||'').toLowerCase().includes(m.toLowerCase().split(' ')[0]);
...
function generarSolicitudContrato() {
  ...
  const modKey = Object.keys(modalidadesCheck).find(k => (r.formaPago||'').toLowerCase().includes(k.toLowerCase().split(' ')[0]));
  ...
  const html = `
    ...
    <table class="no-border">
      <tr>
        ...
        <td style="width:37%;">Contado <span style="border:1px solid #333;display:inline-block;width:16px;height:14px;text-align:center;">${modalidadesCheck['Contado']}</span></td>
        <td style="width:37%;">Crédito Directo <span style="border:1px solid #333;display:inline-block;width:16px;height:14px;text-align:center;">${modalidadesCheck['Crédito Directo']}</span></td>
      </tr>
      <tr>
        <td></td>
        <td>Contado Diferido <span style="border:1px solid #333;display:inline-block;width:16px;height:14px;text-align:center;">${modalidadesCheck['Contado Diferido']}</span></td>
        <td>Crédito Bancario <span style="border:1px solid #333;display:inline-block;width:16px;height:14px;text-align:center;">${modalidadesCheck['Crédito Bancario']}</span></td>
      </tr>
    </table>
