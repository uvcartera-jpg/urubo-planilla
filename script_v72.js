...
async function guardarDocumentoReserva() {
  ...
  const data = {
    ...
    precioVenta: r.precioVenta||0,
    guardadoPorNombre: asesorActual?.nombre || 'Admin',
  };
  await window._fbPushDocumentoReserva(data);
...
function renderDocumentosGuardados() {
  ...
  if (!datos.length) { tbody.innerHTML = '<tr><td colspan="7"><div class="empty-state">Sin documentos guardados todavía.</div></td></tr>'; return; }
  ...
  tbody.innerHTML = datos.map(d => `<tr>
    ...
    <td style="font-size:var(--fs-sm);">${esc(d.guardadoPorNombre||'—')}</td>
    <td>
