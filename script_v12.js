...
function autocompletarClienteExistente() {
  ...
  // le autocompletamos teléfono/procedencia con su registro más reciente — así no
  ...
  const proc = document.getElementById('f-procedencia');
  if (tel && !tel.value && masReciente.telefono) tel.value = masReciente.telefono;
  if (proc && !proc.value && masReciente.procedencia) proc.value = masReciente.procedencia;
  verificarVisitaRepetida();
}
...
async function guardarRegistro() {
  ...
  if (!nombre) {
    ...
    toastErr('Falta el nombre del cliente — el campo quedó marcado en rojo arriba.');
    return;
  }
  ...
  try {
    ...
    btnSuccess(btnR);
    toastOk('Registro de ' + nombre + ' guardado correctamente.');
    limpiarForm();
    const sb = document.getElementById('success-banner');
    if (sb) {
      sb.querySelector('p').textContent = '✅ Registro de "' + nombre + '" guardado — el formulario está listo para el próximo cliente';
